...
\begin{itemize}
  \item \textbf{Formula~A} (derivative-based, appendix\_FORMAL\_emergent\_metric.tex):
    ...
    \]
    % $\mathcal{N}$ is the scale-fixing positive scalar — see theorem\_chain.tex Def~\ref{def:theta_field}.
...
The normalization $\mathcal{N} > 0$ is a \emph{scale-fixing} positive scalar.
It determines the physical units of the metric but \textbf{does not determine
the signature}.  The Lorentzian signature $(-,+,+,+)$ follows from the algebraic
structure of $\mathbb{C}\otimes\mathbb{H} \cong \mathrm{Cl}_{1,3}(\mathbb{R})$
via AXIOM~B — see \texttt{step3\_signature\_theorem.tex}, Theorem~2.
Even if $\mathcal{N}$ were chosen differently (any positive value), the signature
would remain $(-,+,+,+)$.
\end{remark}
...
\begin{remark}[Relation to sigma models]
The construction $g_{\mu\nu} = \Re[\partial_\mu\Theta\cdot\partial_\nu\Theta^\dagger]$
resembles the induced metric in nonlinear sigma models, where a target-space metric
is pulled back via a field map $\phi: \mathcal{M} \to \mathcal{T}$.

The key differences in UBT are:
\begin{enumerate}
  \item The target space is $\mathbb{C}\otimes\mathbb{H}$ (an algebra, not a Riemannian
        manifold); the metric structure comes from the Clifford norm, not a fixed
        background metric.
  \item The signature of $g_{\mu\nu}$ is not imposed by a target-space metric
        but derived from the algebraic axioms (AXIOM~B, Theorem~2).
  \item The field $\Theta$ is not a map into an external space but a fundamental
        field whose dynamics determines spacetime geometry itself.
\end{enumerate}
In sigma models, gravity is a fixed background; in UBT, gravity \emph{is} the field.
\end{remark}
