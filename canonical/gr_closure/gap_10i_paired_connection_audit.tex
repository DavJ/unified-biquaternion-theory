...
action, to the single existing spin connection,
...
Schwarzschild with nonzero mass, and generic Einstein geometries, do not obey
this restriction.  Thus the no-new-field Lorentz pairing is rigorously closed
as a kinematic reduction but closed as a no-go for generic curved GR.  A
viable two-sided completion must derive a nontrivial relative bimodule action
from the canonical UBT action without adding propagating degrees of freedom.
\end{abstract}
...
\begin{proposition}[Schwarzschild exclusion]
The Schwarzschild metric with mass $M\ne0$ admits no vector satisfying
Eq.~\eqref{eq:concurrent}; indeed it admits no proper homothety
$\mathcal L_Vg=2g$.
\end{proposition}
...
\begin{remark}[Why constants in a potential do not remove the obstruction]
...
cannot provide the spacetime-dependent relative left/right action required to
evade the concurrent-vector restriction.  Any such relative component must
be derived algebraically from the canonical action and shown to carry no new
...
\medskip
\textbf{GAP-10I-PAIR-GR: CLOSED AS NO-GO [L1].}
That pure Lorentz pairing makes a nondegenerate torsion-free tetrad generated
by one $\Theta$ imply a concurrent vector and a proper homothety.  It therefore
cannot represent generic curved GR and excludes Schwarzschild with $M\ne0$.

\medskip
\textbf{GAP-10I-2S / GAP-10I-CURVED: NARROWED.}
The remaining admissible route must derive a nontrivial relative bimodule
connection from the canonical action, prove that it is composite or auxiliary
rather than propagating, and then establish self-consistent existence and
regularity.  Merely naming two independent connections is not a closure.

\medskip
\textbf{GAP-10D: NOT CLOSED BY THIS REDUCTION.}
The result removes a false no-new-field shortcut and identifies the exact
missing object in the variational bridge.  The Palatini/Lovelock endpoint
remains conditional until the relative action, its algebraic elimination, and
the induced metric variation are derived from canonical UBT.
\end{tcolorbox}
