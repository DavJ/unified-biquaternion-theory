...
(Step~3 convention).
The remaining sub-gap on the off-diagonal components $g_{0i}$ is formally closed
...
\end{equation}
Since $x_i$ is an odd function of $x_i$ and $\mathcal{V}$ is symmetric under
...
\begin{resultbox}
\textbf{Summary of new results (June 2026, v49):}
\begin{enumerate}
  ...
  \item \textbf{[L1 conditional]}: $\langle g_{0i}\rangle_{\mathcal{V}} = 0$ in the
        ...
        prescription.
\end{enumerate}
\textbf{GAP-C status after v49}: PARTIALLY CLOSED — Result~1 [L1],
Result~2 [MC], sub-gap $g_{0i}=0$ [L1 conditional on comoving-frame definition].
\end{resultbox}
