...
\end{align*}
using the antiderivative identities
$\tfrac{d}{ds}\mathrm{Si}(\omega s)=\sin(\omega s)/s$ and
$\tfrac{d}{ds}\mathrm{Ci}(\omega s)=\cos(\omega s)/s$
(so variation of parameters uses $\int\sin(\omega s)/s\,ds = \mathrm{Si}(\omega s)$
and $\int\cos(\omega s)/s\,ds = \mathrm{Ci}(\omega s)$ as antiderivatives,
consistent with $\mathrm{Si}(x)=\int_0^x\tfrac{\sin t}{t}\,dt$ and
