...
\end{equation}
i.e., it is a \emph{purely imaginary quaternion}.  Therefore, for any
biquaternionic factors $A, B$:
\begin{equation}
  \mathrm{Sc}(A[\mathbf{e}_a,\mathbf{e}_b]B)
  \;=\; 2\varepsilon_{abc}\,\mathrm{Sc}(A\,\mathbf{e}_c\,B)
  \;=\; 0,
  \label{eq:sc_correction_zero}
\end{equation}
since the product $A\,\mathbf{e}_c\,B$ has scalar part zero when $\mathbf{e}_c$
is sandwiched between arbitrary biquaternions and the outer projection is $\mathrm{Sc}$.

More directly, using the cyclic property
$\mathrm{Sc}(AB) = \mathrm{Sc}(BA)$ for all $A, B \in \mathbb{C}\otimes\mathbb{H}$
(standard: $\mathrm{Sc}(q) = (q + q^*)/2$ where $q^*$ is quaternion conjugate,
so $\mathrm{Sc}(AB) = \mathrm{Sc}(BA)$), the commutator correction to the
Euler--Lagrange equation under $\mathrm{Sc}[\cdot]$ is:
\begin{equation}
  \mathrm{Sc}\!\left[(\nabla\Theta)^\dagger [\mathbf{e}_a,\mathbf{e}_b] (\nabla\Theta)\right]
  \;=\; \mathrm{Sc}\!\left[[\mathbf{e}_a,\mathbf{e}_b]\,(\nabla\Theta)(\nabla\Theta)^\dagger\right]
  \;=\; 0,
  \label{eq:correction_vanishes}
\end{equation}
because $[\mathbf{e}_a,\mathbf{e}_b] \in \mathrm{Im}(\mathbb{H})$ and
$\mathrm{Sc}(X\cdot Y) = 0$ whenever $X \in \mathrm{Im}(\mathbb{H})$.

\begin{theorem}[FPE $\leftrightarrow$ E-L (all sectors), \Lzero]
\label{thm:fpe_all_sectors}
For $S[\Theta] = \int \mathrm{Sc}[(\nabla\Theta)^\dagger(\nabla\Theta)]\,d^4x$,
...
= 2\varepsilon_{abc}\mathbf{e}_c \in \mathrm{Im}(\mathbb{H})$.
Since $\mathrm{Sc}(X) = 0$ for all $X \in \mathrm{Im}(\mathbb{H})$,
and $S[\Theta]$ involves only $\mathrm{Sc}[\cdot]$, the correction
$\mathrm{Sc}[(\nabla\Theta)^\dagger[\mathbf{e}_a,\mathbf{e}_b]
(\nabla\Theta)] = 0$ identically.
\qed
\end{theorem}
