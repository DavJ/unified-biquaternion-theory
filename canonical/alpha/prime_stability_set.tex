...
The entropy term $q\ln q$ used in Sections~\ref{sec:pss:s1}--\ref{sec:pss:s7}
is the leading term of $\ln\Gamma(q+1)$ for large $q$.
A mathematically exact formulation replaces $q\ln q$ with
$\ln\Gamma(q+1) = \ln q!$, yielding the \emph{Gamma-entropy potential}:
\begin{equation}
  V_\Gamma(q;\,B) \;:=\; q^2 - B\,\ln\Gamma(q+1).
  \label{eq:pss:Vgamma}
\end{equation}
For every positive integer $q$, $\ln\Gamma(q+1) = \ln q!$ exactly.

\subsection{Stirling Expansion and Comparison with the Original Model}
...
\end{equation}
a quantity that grows as $q\to\infty$.  The two entropy choices are therefore
\emph{not} asymptotically equivalent for fixed $B$: the difference in $V$ is
\[
  V(q;B) - V_\Gamma(q;B) \;=\; B\bigl(\ln\Gamma(q+1) - q\ln q\bigr)
    \;\approx\; -B\!\left(q - \tfrac{1}{2}\ln(2\pi q)\right),
\]
which is $O(Bq)$ — comparable in magnitude to the gradient term $q^2$ for the
values of $B$ encountered here (see Table~\ref{tab:pss:stirling}).

\begin{table}[h]
\centering
\caption{Stirling approximation: $\ln\Gamma(p+1)$ vs.\ $p\ln p$
         and 2-term Stirling $p\ln p - p + \frac{1}{2}\ln(2\pi p)$ at key primes.
         The 3-term approximation (adding $+1/(12p)$) agrees to better than
         $10^{-8}$.}
\label{tab:pss:stirling}
\begin{tabular}{rrrrrr}
\toprule
$p$ & $p\ln p$ & $\ln\Gamma(p+1)$ & $p\ln p - \ln\Gamma(p+1)$
    & $\text{Stirling}_2$ & $|\epsilon_2|$ \\
\midrule
 127 &  615.212 &  491.553 & 123.658 &  491.553 & $6.6\times10^{-4}$ \\
 137 &  674.037 &  540.417 & 133.620 &  540.416 & $6.1\times10^{-4}$ \\
 389 & 2331.555 & 1934.733 & 396.822 & 1934.733 & $2.1\times10^{-4}$ \\
 421 & 2537.843 & 2126.889 & 410.954 & 2126.889 & $2.0\times10^{-4}$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Stationarity Condition under Gamma Entropy}
\label{sec:pss:gamma:stationary}

The continuous stationarity condition $\partial V_\Gamma/\partial q = 0$ gives
\begin{equation}
  2q \;=\; B\,\psi(q+1),
  \label{eq:pss:gamma-stat}
\end{equation}
where $\psi = \Gamma'/\Gamma$ is the digamma function.  The stationarity value is
\begin{equation}
  B^*_\Gamma(p) \;:=\; \frac{2p}{\psi(p+1)}.
  \label{eq:pss:BstarGamma}
\end{equation}
For large $p$, $\psi(p+1) = \ln p + \frac{1}{2p} - \frac{1}{12p^2} + O(p^{-4})$,
so $B^*_\Gamma(p) \approx 2p/\ln p$, compared to $B^*(p) = 2p/(\ln p+1)$ in the
original model.  The two stationarity values differ by a factor $1 + 1/\ln p$, which
is $\approx 20\%$ at $p = 100$ and falls below $5\%$ only for $p > 10^{20}$.

The leading-order upper stability condition $B(p) < B^*_\Gamma(p)$ becomes
\begin{equation}
  \frac{p+1}{3} \;<\; \frac{2p}{\ln p}
  \;\;\Longleftrightarrow\;\;
  (p+1)\ln p \;<\; 6p
  \;\;\Longrightarrow\;\;
  \ln p \;<\; 6 - \frac{\ln p}{p} \;\approx\; 6 \quad\text{for large }p,
  \label{eq:pss:gamma-threshold}
\end{equation}
shifting the stability threshold from $e^5 \approx 148$ to $e^6 \approx 403$.

\subsection{Stable Primes under Gamma Entropy}
\label{sec:pss:gamma:stable}

The nearest-prime stability bounds under $V_\Gamma$ are:
\begin{align}
  B^{\Gamma}_{\mathrm{low}}(p)  &\;=\;
    \frac{p^2 - p_-^2}{\ln\Gamma(p+1) - \ln\Gamma(p_-+1)},
  \label{eq:pss:BlowGamma} \\
  B^{\Gamma}_{\mathrm{high}}(p) &\;=\;
    \frac{p_+^2 - p^2}{\ln\Gamma(p_++1) - \ln\Gamma(p+1)}.
  \label{eq:pss:BhighGamma}
\end{align}
Exhaustive computation (all primes $q \leq 100{,}000$ checked for each candidate
prime $p \leq 10{,}000$) yields the following complete stable set:

\begin{table}[h]
\centering
\caption{All prime-stable primes under $V_\Gamma$.  $B(p)=(p+1)/3$;
         bounds set by nearest primes $p_\pm$;
         margins $\Delta_- = B(p)-B^{\Gamma}_{\mathrm{low}}$,\;
         $\Delta_+ = B^{\Gamma}_{\mathrm{high}}-B(p)$.}
\label{tab:pss:gamma:stable}
\begin{tabular}{rrrrrrrr}
\toprule
$p$ & $p_-$ & $p_+$ & $B(p)$ & $B^{\Gamma}_{\mathrm{low}}$ & $B^{\Gamma}_{\mathrm{high}}$
    & $\Delta_-$ & $\Delta_+$ \\
\midrule
...
421 & 419   & 431 & 140.6667 & 139.0396  & 140.6965 & 1.6271   & \textbf{0.0298} \\
\bottomrule
\end{tabular}
\end{table}

\begin{resultbox}
\textbf{Prime-stable set under $V_\Gamma$} (verified up to $10^5$):
\[
  \mathcal{S}_\Gamma \;=\; \{2,\;389,\;397,\;401,\;409,\;421\}.
\]
$|\mathcal{S}_\Gamma| = 6$.  The odd elements of $\mathcal{S}_\Gamma$ are
completely disjoint from those of $\mathcal{S} = \{127, 137, 139, 151, 157\}$.
\end{resultbox}

The stability window shifts from $[127,157]$ (original) to $[389,421]$ (Gamma),
a shift of $\approx 262$.  The last survivor, $p = 421$, has an upper margin of
only $0.0298$ in $B$-space — the same marginal-survival structure as $p = 157$
in the original model.

For $p > e^6$, the required gap condition becomes
\[
  g_+ \;\gtrsim\; \frac{p(\ln p - 6)}{3}.
\]
At $p = 431$ this gives $g_\text{req} \approx 9.5$, while the actual gap to
$p = 433$ is only $2$.  No prime beyond $421$ is stable under $V_\Gamma$.

\subsubsection*{V-difference spot-check at $p = 421$, $B = 140.6667$}

\begin{center}
\begin{tabular}{rc}
\toprule
$q$ & $V_\Gamma(q;\,140.6667) - V_\Gamma(421;\,140.6667)$ \\
\midrule
419 & $+19.66$ \\
...
\begin{warnbox}
\textbf{Revalidation required.}
The stable sets $\mathcal{S}$ and $\mathcal{S}_\Gamma$ are completely different
(aside from $p = 2$).  All modular-structure claims (Section~\ref{sec:pss:s7})
and pattern analysis (Section~\ref{sec:pss:s6}) must be repeated for
$\mathcal{S}_\Gamma$ before any physical conclusions are drawn from the Gamma
formulation.  The choice between $q\ln q$ and $\ln\Gamma(q+1)$ is not a
cosmetic rescaling: it shifts the entire stability window by $\approx 260$ in
prime-index space.
\end{warnbox}
...
\begin{enumerate}
  ...
  \item \textbf{Gamma extension (Section~\ref{sec:pss:gamma})}: Replacing
        $q\ln q$ with $\ln\Gamma(q+1)$ shifts the stability threshold to
        $\ln p < 6$ ($p < e^6 \approx 403$) and yields a completely different
        stable set $\mathcal{S}_\Gamma = \{2, 389, 397, 401, 409, 421\}$.
        The two formulations are not asymptotically equivalent for fixed $B$;
        the stable sets must be revalidated independently.
\end{enumerate}
\end{resultbox}
