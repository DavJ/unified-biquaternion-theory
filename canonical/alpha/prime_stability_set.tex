...
\begin{enumerate}
  ...
  \item \textbf{Gamma-consistency (Section~\ref{sec:pss:gamma}) \textnormal{[Numerical]}}: The
        ...
        in the canonical model.  The unrenormalised variant produces a different
        stable set $\{2,389,397,401,409,421\}$ near $e^6 \approx 403$; this
        is a separate non-canonical model and must not be interpreted as a
        correction or extension of the canonical result.
\end{enumerate}
\end{resultbox}

\begin{infobox}
\textbf{Scope and claims}: No claims about the Riemann Hypothesis, the fine
structure constant, or other deep results are derived here beyond the
prime-stability framework itself.  The coincidence of $137 \in \mathcal{S}$
with the approximate value of $1/\alpha$ is noted as a structural observation
only.
\end{infobox}
