...
as elements.  Because $\pi_1(S^1_\psi) = \ZZ$ is connected, a continuous
$\pi_1$-deformation can interpolate between the $n$-winding configuration and
the separated $(a,b)$ pair (winding-$a$ together with winding-$b$); both are
elements of $\ZZ$ and the deformation path passes through intermediate integer
windings.
...
\begin{equation}
  ...
  \quad \text{for any non-negative integers } n_k
  \text{ satisfying } \sum_{k=1}^{p-1} n_k \cdot k < p.
  % The constraint \sum n_k \cdot k < p restricts to linear combinations of
  % lower modes whose total winding is strictly less than p.  Without it the
  % claim would be trivially false (e.g.\ p copies of winding 1 sum to p).
  % The spectral non-overlap asserts that p cannot be reached by any
  % \emph{sub-p} combination of lower winding modes.
...
For this cross-term to be non-zero in the second variation $\delta^2 S$, the
commutator $[\nabla^\dagger\nabla, e^{in\psi}]$ must contain a mode at $|n - 2j|$.
(Here ``tree level'' refers to the classical-background level of the second
variation, which is the input to the one-loop functional determinant; the full
calculation is one-loop, and these cross-terms are generated at that order.)
...
For prime $n = p$, the commutator has no mode support at $j = 1, \ldots, p-1$
(since $p$ has no proper divisors in $\{1,\ldots,p-1\}$), so all cross-terms
vanish within the one-loop second-variation analysis.
\end{proof}
