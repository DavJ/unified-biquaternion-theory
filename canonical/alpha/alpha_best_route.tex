...
\else
  ...
  \title{\textbf{Best Route to $\alpha$ from UBT: Prime Attractor on $S^1_\psi$}\\
         \large Alpha Breakthrough Mission — Formal Derivation}
...
This document presents the \textbf{best available route} to deriving the
fine-structure constant~$\alpha \approx 1/137.036$ from the Unified Biquaternion
Theory (UBT), following the Alpha Breakthrough Mission (2026-04-28).
...
Conditional step & Step 5: Kac-Moody level $k = 1$ (Gap G3-k) \\
...
\begin{equation}
  ...
  \;\xrightarrow{\text{Gap G3-k}}\;
  \underbrace{B_{\mathrm{base}}=N_{\mathrm{eff}}^{3/2}}_{\text{[MC]}}
  \;\to\;
...
\begin{gapbox}
\textbf{Gap G3-k (the unique blocking gap)}: Prove that the Kac-Moody level of
...
\textbf{(ii) New analysis (Alpha Breakthrough Mission 2026-04-28)}:
...
\medskip
\textbf{Status of Gap G3-k}: All 27+ approaches exhausted except the modular
...
one-loop; only coefficient tracks Gap G3-k.\\
...
\end{equation}
With $B_{\mathrm{base}} \approx 41.57$, the continuous minimum solves
$2n = 41.57(\ln n + 1)$, giving $n^*_{\mathrm{continuous}} \approx 84$.
With $B_{\mathrm{full}} = B_{\mathrm{base}} \times R \approx 46.31$, the equation
$2n = 46.31(\ln n + 1)$ gives $n^*_{\mathrm{continuous}} \approx 105$.
...
\begin{enumerate}
  \item The physical value $\alpha^{-1} = 137.036$: the extra $\delta \approx 0.036$
  ...
  \item The value $k = 1$: Gap G3-k remains unproved.
\end{enumerate}
\end{warnbox}
...
5 & $B_{\mathrm{base}} = N_{\mathrm{eff}}^{3/2}$ & [MC] & No & \textbf{G3-k} & \S\ref{sec:ar:Bbase} \\
...
8 & $\alpha^{-1}_{\mathrm{bare}} = 137$ & CONDITIONAL & No & G3-k & \S\ref{sec:ar:identification} \\
\midrule
...
with zero free parameters.  Step 5 (Gap G3-k: $k=1$) is the unique remaining gap.
No circular input appears in Steps 1--8.  If Gap G3-k is resolved,
$\alpha^{-1}_{\mathrm{bare}} = 137$ is a zero-parameter result from UBT axioms.
\end{resultbox}

%──────────────────────────────────────────────────────────────────────────────
\section{Gap G3-k: Modular Bootstrap Attack}
\label{sec:ar:bootstrap}
%──────────────────────────────────────────────────────────────────────────────

\begin{gapbox}
\textbf{Gap G3-k}: Prove $k = 1$ (Kac-Moody level) from the UBT CFT on $T^2$.
\end{gapbox}
