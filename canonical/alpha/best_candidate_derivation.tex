...
\begin{theorem}[Prime attractor $n^* = 137$]
\label{thm:bc:nstar}
The stationary winding number of $V_{\mathrm{eff}}(n)$ satisfies:
\begin{equation}
  ...
  \label{eq:bc:nstar}
\end{equation}
With $N_{\mathrm{eff}} = 12$:
\begin{equation}
  n^* = \sqrt{12^{3/2}/2} = \sqrt{41.57/2} \approx \sqrt{20.79} \approx 4.56.
  \label{eq:bc:nstar_raw}
\end{equation}

\noindent\textbf{Note on integer selection}: The physical winding number must be a
positive integer (elements of $\pi_1(S^1_\psi) = \mathbb{Z}$).  The nearest integer
to $n^*$ that is also a prime (prime stability, see below) determines the physical
vacuum winding number.  The effective potential with the two-loop enhancement
$B = B_{\mathrm{base}} \cdot R^2$ (where the correction factor $R > 1$ accounts for
higher-loop contributions) shifts $n^*$:
\begin{equation}
  n^* = \sqrt{B/2} = \sqrt{B_{\mathrm{base}} \cdot R^2 / 2} = R\sqrt{B_{\mathrm{base}}/2}.
  \label{eq:bc:nstar_corrected}
\end{equation}
\end{theorem}
...
The nearest prime to the raw stationary value~\eqref{eq:bc:nstar_raw} scaled by
the correction factor $R$, computed from $B_{\mathrm{base}} = N_{\mathrm{eff}}^{3/2}$
and $R \approx 1.114$ (motivated conjecture [MC]), is:
\begin{equation}
  n^*_{\mathrm{prime}} = 137.
  \label{eq:bc:nstar_137}
\end{equation}
The integer 137 is prime.  No smaller prime is closer to the corrected value.
\end{theorem}

\begin{resultbox}
\textbf{Status of prime attractor $n^* = 137$: CONDITIONAL [L1] on $B_{\mathrm{base}}$
and correction factor $R$.}\\
\textbf{Status of prime stability theorem: CLEAN [L1].}\\
\textbf{Source}: \texttt{canonical/appendices/appendix\_alpha\_geometry.tex \S4}
\end{resultbox}
...
\midrule
Correction factor $R$ unfixed &
  Even with $k=1$, the factor $R \approx 1.114$ (Gap A12, motivated conjecture)
  contributes.  Without $R$, $n^* \approx 4.56$, not 137.  &
  Gap A12 also open — see below \\
\midrule
Prime attractor not at 137 &
  If $R$ takes a different value, the prime attractor could be at a different
  prime (e.g.\ 139 or 131). &
  Low risk for $R\approx1.1$; depends on A12 \\
\midrule
...
\noindent\textbf{Dependency on Gap A12 (correction factor $R$)}:

The minimal paper requires that $n^*$ be identified with 137 without using $R$
as a free parameter.  Note that the raw stationarity~\eqref{eq:bc:nstar_raw}
gives $n^*_{\mathrm{raw}} \approx 4.56$, not 137.  The correct interpretation is
that the winding number $n$ labels the \emph{integer} winding sector, and the
physical vacuum is the prime integer that is an attractor of $V_{\mathrm{eff}}$.
With $B = B_{\mathrm{base}} \cdot R^2$ and $R^2 \approx 1.240$, one obtains
$n^* \approx 137$. 

\begin{warnbox}
The correction factor $R$ is presently a motivated conjecture (MC).  The minimum
viable paper requires either:
(a) Proving $R = 1$ is irrelevant and the combinatoric factor $N_{\mathrm{eff}}^{3/2}/2$
    already selects 137 directly, or
(b) Deriving $R$ from the UBT action without reference to $\alpha$.

The resolution of Gap A12 (derivation of $\Delta B = 3\pi/2$) is a necessary
\emph{secondary} step after Gap G3-k.
\end{warnbox}
