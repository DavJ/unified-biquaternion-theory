...
\begin{warnbox}
\textbf{Legacy derivation-attempt banner (2026-05-10)}:
This file preserves an older best-candidate framing with Gap G3-k language.
For active canonical status, use \texttt{canonical/alpha/ALPHA\_MASTER\_STATUS.md}.\\
\textbf{Current blocker framing}: Gap \textbf{G137-B} (derive $B$ from $S[\Theta]$
without circular inputs).\\
alpha is \textbf{NOT derived}; $\alpha^{-1}_{\mathrm{bare}}=137$ is
\textbf{CONDITIONAL ONLY}; $\alpha^{-1}=137.036$ is \textbf{NOT derived}.
\end{warnbox}
...
Conditional step & 1 (derive $B$ from $S[\Theta]$, Gap G137-B) \\
Circular inputs & None in the bare-value claim \\
Overall status & \textbf{CONDITIONAL} on Gap G137-B \\
Blocking gap & Gap G137-B: first-principles $B$ derivation not yet proved \\
