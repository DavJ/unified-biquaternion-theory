...
This document presents the complete equation chain for three historical
alpha-route formulations, with explicit identification of
...
\begin{tcolorbox}[colback=red!4!white,colframe=red!65!black,
title=Canonical status overlay (2026-05-10)]
\textbf{Authoritative source}: \texttt{canonical/alpha/ALPHA\_MASTER\_STATUS.md}.\\
\textbf{Overall}: alpha is \textbf{NOT DERIVED}. Gap \textbf{G137-B} is \textbf{OPEN}.\\
\textbf{Route labels (canonical verdict)}:
\begin{itemize}
  \item Route A (Modular-Hecke): \textbf{OBSOLETE} as an active alpha-derivation route; legacy heuristic support only.
  \item Route B (V\(_{\mathrm{eff}}\) spectral): \textbf{CONDITIONAL} route to $\alpha^{-1}_{\mathrm{bare}}=137$, blocked by \textbf{OPEN} Gap G137-B.
  \item Route C (EW/GUT bridge): \textbf{OBSOLETE} as an alpha-derivation route in this track.
\end{itemize}
\textbf{Status terms used in this file}: \textbf{PROVED / CONDITIONAL / HEURISTIC / OBSOLETE / OPEN}.\\
\textbf{Deprecated claims}: constant $\Delta B_{\mathrm{wind}}\approx 18.5$ is \textbf{OBSOLETE/NO-GO};
$B_{\mathrm{best}}\approx 43.6$ is \textbf{OBSOLETE HEURISTIC}.
\end{tcolorbox}
...
but this route remains \textbf{OPEN} and this value is \textbf{not derived} from
UBT first principles in the current canonical verdict.
\end{tcolorbox}
...
which remains \textbf{not derived} as a UBT first-principles result and
stays \textbf{OPEN} under Gap G137-B framing.
