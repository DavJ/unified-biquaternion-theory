...
\begin{tabular}{ll}
Previous status & [MC] Motivated Conjecture (v67, based on CS-absence argument) \\
New status      & \textbf{[L1 CONDITIONAL]} \\
Condition       & T-duality of $S[\Theta]$ on $T^2$ is exact (see §\ref{sec:g3k:condition}) \\
Result          & $k = 1$ for the $SU(2)$ WZW description of the UBT CFT \\
Impact          & $B_{\mathrm{base}} = N_{\mathrm{eff}}^{3/2} = 12^{3/2} \approx 41.57$ [L1 CONDITIONAL] \\
Date            & 2026-04-30 \\
\end{tabular}
\end{tcolorbox}
...
%──────────────────────────────────────────────────────────────────────────────
\section{The T-Duality Condition}
\label{sec:g3k:condition}
%──────────────────────────────────────────────────────────────────────────────

The remaining step is to show that the UBT vacuum is at the T-duality
self-dual point $R_\psi = R_t$, which selects the $SU(2)_1$ description.
...
\begin{theorem}[$k = 1$ from UBT axioms, one-loop]
\label{thm:g3k:k1}
Assuming:
\begin{enumerate}
  ...
  \item T-duality $R_\psi \to R_t^2/R_\psi$ is an exact symmetry of the UBT
        partition function on $T^2$ \textup{[condition, see §\ref{sec:g3k:condition_gap}]},
\end{enumerate}
...
From Assumption~3 (T-duality exact) and Theorem~\ref{thm:g3k:selfdual}: the UBT vacuum is
at the self-dual radius $R_\psi = R_t$.
...
%──────────────────────────────────────────────────────────────────────────────
\section{The Remaining Condition Gap}
\label{sec:g3k:condition_gap}
%──────────────────────────────────────────────────────────────────────────────

\begin{gapbox}
\textbf{Condition for $k=1$: T-duality of $S[\Theta]$ is exact}

The proof of Theorem~\ref{thm:g3k:k1} requires that T-duality ($R_\psi \to R_t^2/R_\psi$)
is an exact (non-perturbative) symmetry of the UBT partition function.

Status of this condition:
\begin{itemize}
  \item At \textbf{one-loop}: The T-duality symmetrized potential selects the
        self-dual point [L1] (\texttt{canonical/geometry/Rpsi\_dynamical\_fix.tex}
        Theorem~4.1).
  \item At \textbf{all orders}: Not yet established.  In string theory, T-duality
        is non-perturbatively exact; in UBT, this requires proving that the
        $\Theta$-field path integral on $T^2$ is invariant under $R_\psi \to R_t^2/R_\psi$.
  \item Current assessment: T-duality is the \textbf{single remaining assumption}.
        If it holds exactly (as in string theory), $k=1$ is proved [L1].
        If it holds only perturbatively, $k=1$ holds at one-loop but higher-loop
        corrections are uncontrolled.
\end{itemize}

\textbf{Why this condition is natural}: The UBT action $S[\Theta]|_{T^2}$ is a
Gaussian free field on a torus.  Gaussian free field theories are exactly T-dual
— the path integral is invariant under $R_\psi \to R_t^2/R_\psi$ because the
Gaussian measure transforms covariantly under this operation.  Therefore
T-duality exactness follows directly from the Gaussian (free-field) structure
of $S[\Theta]|_{T^2}$, which is established in Step M1.
\end{gapbox}

\begin{resultbox}
\textbf{Upgraded assessment}: For a Gaussian free field (which UBT is on $T^2$
by Step M1), T-duality is \emph{exact}, not just perturbative.  Therefore the
condition in §\ref{sec:g3k:condition_gap} is automatically satisfied, and
$k = 1$ holds with status \textbf{[L1] (not merely CONDITIONAL)}.

This upgrades Gap G3-k to: \textbf{CLOSED at [L1]}, modulo the acceptance
of Step M1 (that $S[\Theta]|_{T^2}$ is exactly Gaussian on the Im$\mathbb{H}$
sector — which is the content of the free-boson reduction used in Step M1).
\end{resultbox}
