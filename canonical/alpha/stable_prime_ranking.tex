...
The twin prime $p = 139$ hits a \emph{mirror sector}
(``Set~B''; see \texttt{prime\_137\_status.md}, Section~5): it achieves
a Hecke lepton-ratio match in a different triple of modular forms, but
the exact eigenvalues and errors for this mirror match are not yet
documented in the canonical record.
No Hecke lepton-ratio signal has been found for $p \in \{127, 151, 157\}$
in the same exhaustive search.
...
\caption{Hecke lepton-ratio scores.
  ...
  $1 =$ partial (mirror/set-B match documented but eigenvalues not published);
...
139 & Mirror ``Set~B'' hit documented; exact errors not yet canonical & \textbf{1} \\
...
\caption{Theta spectral compatibility.  $p \bmod 4$ and $p \bmod 8$
  ...
  Explicit representations: $137 = 4^2 + 11^2 = 10^2 + 6^2 + 1^2$;\;
  $157 = 6^2 + 11^2 = 11^2 + 4^2 + 4^2 + 4^2$ (4 sq.\ only for the
  direct check; $157 = 6^2 + 11^2$ is already a two-square representation).}
\label{tab:spr:c4}
...
\begin{infobox}
\textbf{Twin-prime complementarity}: The pair $(137, 139)$ exhibits a
``gauge duality'': $p = 137$ has $\nu_2 = 2,\,\nu_3 = 0$ (pure EW) while
$p = 139$ has $\nu_2 = 0,\,\nu_3 = 2$ (pure QCD).  This mirrors the
Set~A / Set~B Hecke structure documented in \texttt{prime\_137\_status.md},
Section~5.  The complementarity follows from $137 \equiv 2\pmod{3}$ and
$139 \equiv 1\pmod{3}$, i.e.\ consecutive values modulo~3.
\end{infobox}
...
\begin{enumerate}
  \item $B(2) = 1$: the one-loop coefficient equals the minimal integer;
        no non-trivial logarithmic structure ($2\ln 2 - B \approx 0.386 \neq 0$).
...
\begin{description}
  \item[$p = 139$ (rank 2)] The twin-prime partner of $137$.  Its partial
        Hecke score (Set~B mirror sector) and its closeness to $\alpha^{-1}$
        ($\delta = 1.964$) suggest it is a ``shadow'' of the attractor in
        the twin-prime structure.  Its gauge sector ($\nu_2 = 0$, $\nu_3 = 2$)
        is complementary to $137$ (pure QCD), consistent with a
        colour-singlet / colour-charged duality.
...
\begin{enumerate}
  ...
  \item \textbf{Gap G137-Hk} [\OPEN]: Derive why levels $76, 7, 208$ and
        ...
        (C2 \MC).
  \item \textbf{Gap G-twin} [\OPEN]: Derive the Set~A / Set~B twin-prime
        duality between $p = 137$ (electroweak) and $p = 139$ (QCD) from the
        gauge-algebra structure of UBT
        (C5 suggests it; not yet derived).
