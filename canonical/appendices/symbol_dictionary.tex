...
\hline
$\mathcal{G}_{\mu\nu}$ & Biquaternionic metric tensor & Fundamental metric (in $\mathbb{B}$) \\
\hline
$g_{\mu\nu}$ & Real metric tensor & DERIVED: $g_{\mu\nu} := \text{Re}(\mathcal{G}_{\mu\nu})$ (GR projection) \\
\hline
...
\begin{enumerate}
    ...
    \item ❌ Defining metric without reference to biquaternionic metric $\mathcal{G}_{\mu\nu}$
\end{enumerate}
