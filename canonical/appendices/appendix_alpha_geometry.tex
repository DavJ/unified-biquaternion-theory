...
\begin{warnbox}
\textbf{Correction (2026-05-02)}: An earlier version of this appendix wrote
$V_{\mathrm{eff}}(n) = n^2 - B\ln n$ (missing the factor $n$).
That is form V4, which leads to $n^* = \sqrt{B/2} \approx 4.8$ for
$B \approx 46.3$ — far from 137.
The correct form is $n^2 - B\,n\ln n$ (form V1), consistent with the
one-loop Coleman-Weinberg/heat-kernel calculation.
See \texttt{reports/alpha\_veff\_sanity\_audit.md} and
\texttt{canonical/alpha/veff\_corrected\_statement.tex}.
\end{warnbox}
...
The continuous minimum of $V_{\mathrm{eff}}(n) = n^2 - B\,n\ln n$ is the
prime-stable winding number $n^* = 137$, where $n^*$ satisfies the
\textbf{transcendental stationarity condition}:
\begin{equation}\label{eq:stationarity}
  \frac{\partial V_{\mathrm{eff}}}{\partial n}\bigg|_{n^*} = 0
  \;\;\Longrightarrow\;\;
  \boxed{2n^* = B\,\bigl(\ln n^* + 1\bigr).}
\end{equation}
This equation has no elementary closed form and is solved numerically.
For $B \approx 46.284$, the solution is $n^*_{\mathrm{continuous}} = 137.000$;
the prime-stable minimiser is $n^* = 137$.
\end{theorem}

\begin{warnbox}
\textbf{Correction (2026-05-02)}: A previous version of this theorem
stated $n^* = \sqrt{B/2}$, which is the stationarity condition of the
\emph{wrong} potential $n^2 - B\ln n$.
For $B \approx 46.3$, $\sqrt{B/2} \approx 4.8$, which is far from 137.
The correct condition is the transcendental
equation~\eqref{eq:stationarity} above.
See \texttt{canonical/alpha/veff\_corrected\_statement.tex} for the
complete corrected derivation.
\end{warnbox}

\begin{proof}
Differentiating $V_{\mathrm{eff}}(n) = n^2 - B\,n\ln n$:
\[
  \frac{\partial V_{\mathrm{eff}}}{\partial n}
  = 2n - B\,(\ln n + 1).
\]
Setting this to zero gives the transcendental
equation~\eqref{eq:stationarity}.
For $B = B_{\mathrm{required}} \approx 46.284$
(see \texttt{canonical/alpha/veff\_corrected\_statement.tex}~\S4),
the numerical solution is $n^*_{\mathrm{continuous}} = 137.000$.
The prime-stability constraint (from homotopy theory,
\texttt{docs/STATUS\_ALPHA.md \S3}) selects $n^*$ to be prime; 137 is
prime, and the next nearest prime (139) has $V(139) - V(137) \approx +3.16$.
$\square$
\end{proof}
...
The decomposition~\eqref{eq:B} combined with the stationarity
condition~\eqref{eq:stationarity} gives the implicit relation:
\begin{equation}\label{eq:alpha_formula}
  2\alpha^{-1}_{\mathrm{bare}}
  = B\,\bigl(\ln \alpha^{-1}_{\mathrm{bare}} + 1\bigr),
  \qquad B = B_{\mathrm{required}} \approx 46.284.
\end{equation}
There is no elementary closed-form solution; $n^* = 137$ is the numerical
solution and prime minimiser for $B \in [46.00, 46.50]$.
