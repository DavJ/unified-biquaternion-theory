...
\subsection{Non-canonical $p=139$ material moved to speculative extension}

The p=139 mirror-sector / Set B material is not treated as a canonical
prediction in this appendix.  The detailed note has been moved to
\texttt{speculative\_extensions/alpha\_139\_mirror\_sector/} until a
canonical UBT mechanism is derived.

\noindent\textbf{Status}: \textbf{[SPECULATIVE / MOVED]}.
...
The scalar phase $\phi$ of $\Theta$ would be physical on a branch with a
canonically derived nonzero product $\rho r$.  The former two-mode Hermitian
example does not realize this condition: its metric channel is real and the
reported complex gauge ratio comes only from a sketch formula.

\noindent\textbf{Status}: \textbf{[OPEN]} (analytic diagnostic derived; explicit canonical realization missing) \\
...
Mirror-sector note at $p=139$ & moved to speculative extension & SPECULATIVE & No \\
