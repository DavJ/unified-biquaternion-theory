...
\begin{tabular}{lll}
  ...
  \midrule
  $B_\mathrm{base} = \Neff^{3/2}$ & $12^{3/2} \approx 41.57$ & Motivated Conjecture [with gap] \\
...
%──────────────────────────────────────────────────────────────────────────────
\section{Critical Audit Update (2026-05-10)}
\label{sec:audit_update_20260510}
%──────────────────────────────────────────────────────────────────────────────

\textbf{Superseding note for this file:}
The formal audit in \texttt{canonical/n\_eff/step2\_AUDIT.tex} is authoritative
for convention reconciliation.

\begin{itemize}
  \item $N_{\mathrm{phases}}=3$ remains proved.
  \item The independent multipliers $N_{\mathrm{helicity}}=2$ and explicit
  $N_{\mathrm{charge}}=2$ are \emph{not} currently proved in this scalar-loop
  derivation path.
  \item The loop-safe audited value is currently
  $N_{\mathrm{eff}}^{\mathrm{audit}}=3$, yielding $B_0^{\mathrm{audit}}=2\pi$ in this
  normalization.
  \item The previous $\Neff=12\Rightarrow B_0=8\pi$ line remains a hypothesis
  pending explicit non-redundant derivation of both extra factors.
\end{itemize}

\paragraph{Bridge note.}
The mapping from vacuum-polarization $B_0$ to the $B$ in
$V_{\mathrm{eff}}(n)=n^2-Bn\ln n$ is treated separately in
\texttt{canonical/alpha/B\_identification\_proof.tex} and is currently open.
