...
\textbf{Audit verdict for Step B:}
\begin{itemize}
  \item $N_{\mathrm{helicity}}=2$ is \textbf{not proved} as an independent multiplier
  in the current scalar-action derivation chain.
  \item Status: \textbf{OPEN / [MC]} in the present canonical files.
\end{itemize}
...
$N_{\mathrm{helicity}}$ & 2 (claimed) & No explicit Dirac-loop derivation in audited path & OPEN / [MC] \\
...
$N_{\mathrm{eff}}$ & 3 (loop-safe) & Minimal scalar-action one-loop interpretation & AUDIT RESULT \\
$B_0$ & $2\pi$ & $B_0=(2\pi/3)N_{\mathrm{eff}}$ with $N_{\mathrm{eff}}=3$ & AUDIT RESULT \\
\bottomrule
\end{tabular}
\end{center}
