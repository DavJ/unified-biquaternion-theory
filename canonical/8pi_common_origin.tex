...
\[
  16\pi = 4 \times 4\pi = \dim_\mathbb{R}(\mathbb{H}) \times \mathrm{vol}(S^2),
\]
where $\dim_\mathbb{R}(\mathbb{H}) = 4$ appears through $N_{\rm eff} = 12 = 4 \times 3$
and $\mathrm{vol}(S^2) = 4\pi$ is the 2-sphere volume (same as in
\eqref{eq:16piG}).
