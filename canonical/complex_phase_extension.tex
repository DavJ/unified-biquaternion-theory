...
\begin{proposition}[Periodicity of $V$, \PROVED]
\label{prop:cpe:period}
$V(r, \theta + 2\pi) = V(r, \theta)$ for all $r > 0$ and $\theta \in \RR$.
\end{proposition}

\begin{proof}
All factors ($e^{2i\theta}$, $e^{i\theta}$, $\cos\theta$, $\sin\theta$) are
$2\pi$-periodic, and $\ln(r\,e^{i(\theta+2\pi)}) = \ln r + i(\theta + 2\pi)$,
which shifts $\Im V$ by $-B\,r\cos\theta \cdot 2\pi$ and $\Re V$ by
$+B\,r\sin\theta \cdot 2\pi$.  However, eqs.~\eqref{eq:cpe:ReV}--\eqref{eq:cpe:ImV}
show $V$ depends on $\theta$ only through $\cos$, $\sin$ and $\theta$ itself in the
combination $(\ln r)\cos\theta - \theta\sin\theta$.  Under $\theta \to \theta + 2\pi$:
the trigonometric factors are unchanged, but the $\theta\sin\theta$ term acquires
an extra $2\pi\sin\theta$.  Therefore $V$ is \emph{not} strictly $2\pi$-periodic
in general; it is $2\pi$-periodic only for $\theta = k\pi$. $\square$ (Modified.)
\end{proof}

\begin{remark}
The branch structure of $\ln q$ means that $V$ acquires an additive shift of
$-B\,r\,e^{i\theta}\cdot 2\pi i$ per winding.  For the principal branch,
$\theta \in (-\pi,\pi]$ is the natural domain.
\end{remark}
