...
Interpretation of \(\tau=t+i\psi\) in the active canonical theory is fixed in
\texttt{canonical/THEORY/complex\_time\_canonical\_choice.tex}: \(\psi\) is
treated as the compact internal fiber \(S^1_\psi\), not as a second
macroscopic time.

\section{Einstein Field Equations (Real Sector)}

Let the metric be the constant-normalized compact-$\psi$ pairing of the
existing field $\Theta(q,t+i\psi)$.  Direct variation of the canonical
composite action gives
\begin{equation}
 \mathscr F_\Theta+
 \frac{1}{\kappa\mathcal N_0}\nabla_\nu
 \left[(G^{\mu\nu}-\kappa T^{\mu\nu})\partial_\mu\Theta\right]=0.
 \label{eq:pure_theta_el}
\end{equation}
For a single fixed-$\psi$ section, generic closure is impossible by a rank
bound.  On a regular fiber-free compact-$\psi$ profile, the vacuum equation
implies $G_{\mu\nu}=0$.  With matter/gauge fields it implies
\begin{equation}\label{eq:Einstein}
 G_{\mu\nu}=\kappa T_{\mu\nu}
\end{equation}
conditional on the direct internal equation $\mathscr F_\Theta=0$.

\noindent\textbf{Status}: exact first variation and local vacuum fiber-free
closure PROVED; matter closure CONDITIONAL; GAP-10S/J/R/G OPEN.\\
...
\section{Pure-$\Theta$ Closure Ledger}

\begin{itemize}
 \item GAP-10K: local kinematic/rank closure is proved on regular fiber-free
 compact-$\psi$ profiles; a fixed-$\psi$ section is a rank no-go.
 \item GAP-10S: separating the direct internal and metric equations from one
 total stationarity equation remains open.
 \item GAP-10J: fiber-freeness of the dynamically selected Jacobi-theta family
 remains open.
 \item GAP-10G: global existence, topology, and preservation of the rank
...
Einstein eq.\ \eqref{eq:Einstein} & Gravity & \texttt{pure\_ubt\_fiber\_closure.tex} & local vacuum PROVED; matter CONDITIONAL \\
...
Pure-$\Theta$ closure ledger & Gravity & \texttt{pure\_ubt\_fiber\_closure.tex} & GAP-10K closed locally; S/J/G open \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
  ...
  \item Complex-time choice: \texttt{THEORY/complex\_time\_canonical\_choice.tex}
