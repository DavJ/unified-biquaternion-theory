...
\begin{itemize}
    ...
    \item $\circ$ denotes the biquaternionic product (non-commutative, non-associative in general)
    \item $\Gamma^\lambda_{\mu\nu}$ are the Christoffel symbols (derived from the metric)
\end{itemize}

\textbf{Do NOT simplify} the commutators or assume associativity. The full biquaternionic structure must be preserved.
