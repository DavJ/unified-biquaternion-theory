...
\begin{tcolorbox}[colback=yellow!5!white,colframe=orange!60!black,title=Signature Convention Note]
\textbf{Canonical signature for UBT}: $(-,+,+,+)$ (timelike negative).
This file historically uses $(+,-,-,-)$. For consistency with the rest of canonical theory
and wiki, all new derivations should use $(-,+,+,+)$.
The two conventions are physically equivalent under overall sign flip.
\textbf{Note}: The existing formulas in this file are written in $(+,-,-,-)$; when comparing
with other canonical files using $(-,+,+,+)$, apply an overall sign flip to the metric.
Full signature unification is a separate task.
\end{tcolorbox}
...
g_{\mu\nu}(\Theta) = \frac{\left\langle \partial_\mu\Theta,\partial_\nu\Theta \right\rangle_\eta}{\mathcal{N}}
\end{equation}
...
\begin{itemize}
    ...
    \item $\langle A,B\rangle_\eta := \text{Re}\,\text{Tr}(A\,\eta_{\mathcal{B}}\,B^\dagger)$ is the admissible Lorentzian bilinear form
    \item $\eta_{\mathcal{B}}$ is an external Lorentzian bilinear-form operator on the selected 4D real projection subspace (equivalently representation-level metric operator), not an element of $\mathbb{B}$
    \item $\mathcal{N} := -\langle \partial_0\Theta,\partial_0\Theta\rangle_\eta > 0$
\end{itemize}
...
encodes phase curvature and couples to internal auxiliary sector (dark energy; speculative extensions in \texttt{speculative\_extensions/}).
...
\textbf{Canonical resolution}: Use Eq.~\ref{eq:canonical:metric_from_theta} with signature $(+,-,-,-)$ and index convention $\mu,\nu = 0,1,2,3$ consistently throughout.

\begin{tcolorbox}[colback=blue!5!white,colframe=blue!40!black,title=Signature convention note]
The canonical signature here is $(+,-,-,-)$ (mostly minus, spacelike negative).
Some older files in the repository use the equivalent ``mostly plus'' convention
$(-,+,+,+)$ (e.g.\ parts of \texttt{unified\_biquaternion\_theory/} and
\texttt{STATUS\_THEORY\_ASSESSMENT.md} entries predating v60).

The two conventions are physically equivalent up to an overall sign of the metric
and a corresponding sign flip in any Lagrangian density.  No physics claim
is affected.  When comparing equations across files, verify which convention is in use.
\end{tcolorbox}
