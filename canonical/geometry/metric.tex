% Canonical Metric Tensor Definition
% Version: 1.0
% Date: 2025-11-14
% Status: Canonical - DO NOT DUPLICATE

\section{The Metric Tensor $g_{\mu\nu}$}
\label{sec:canonical:metric}

\subsection{Canonical Definition}

The spacetime metric tensor emerges from the fundamental biquaternion field $\Theta(q,\tau)$ via:
...
The metric determines the Christoffel symbols (affine connection):
...
In the limit $\psi \to 0$ (imaginary time component vanishes), the metric derived from $\Theta$ \textbf{exactly reproduces} the Einstein metric satisfying:

\begin{equation}
\label{eq:canonical:einstein_equation}
R_{\mu\nu} - \frac{1}{2} g_{\mu\nu} R = 8\pi G T_{\mu\nu}
\end{equation}

where $T_{\mu\nu}$ is the stress-energy tensor (see Section~\ref{sec:canonical:stress_energy}).

\textbf{This ensures UBT generalizes GR rather than contradicting it.}
