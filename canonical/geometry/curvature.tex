...
\textbf{Note:} This classical formula is valid because the real projection commutes with
\emph{real} derivatives ($\text{Re}(\partial_\mu f) = \partial_\mu \text{Re}(f)$ for
real $\partial_\mu$) and is \emph{linear}: $\text{Re}(A + B) = \text{Re}(A) + \text{Re}(B)$.
\textbf{Caution}: $\text{Re}$ does \emph{not} commute with general biquaternionic
products: $\text{Re}(AB) \neq \text{Re}(A)\,\text{Re}(B)$ in general.
The above formula is valid specifically because the Christoffel symbols are derived from
the real metric $g_{\mu\nu}$ using only real arithmetic once the real projection has
been applied; the formula does not generalise to arbitrary biquaternionic products.
