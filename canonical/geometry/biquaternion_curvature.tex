...
\begin{itemize}
    ...
    \item $\star$ denotes the full biquaternionic product (associative, non-commutative)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Associative}: $(AB)C = A(BC)$ always (tensor product of associative algebras)
\end{itemize}

Associativity is used explicitly in the proof of the Bianchi identity (Section~\ref{subsec:bianchi_biquaternion}).
The full non-commutative structure must be preserved; simplifications valid only in the real limit must be labeled ``GR limit''.
\end{tcolorbox}
...
\begin{itemize}
    ...
    \item $\mathcal{G}_{\mu\nu}$ is the biquaternionic \textbf{metric} (distinct from $\mathcal{E}_{\mu\nu}$)
\end{itemize}

\begin{tcolorbox}[colback=yellow!5!white,colframe=orange!75!black,title=Symbol Convention]
$\mathcal{G}_{\mu\nu}$ always denotes the biquaternionic \textbf{metric tensor}.
$\mathcal{E}_{\mu\nu}$ always denotes the biquaternionic \textbf{Einstein tensor}.
These are distinct objects; using the same symbol for both caused confusion in earlier versions.
\end{tcolorbox}
...
\label{subsec:bianchi_biquaternion}

The biquaternionic curvature satisfies generalized Bianchi identities, which we now prove
explicitly in the non-commutative (but associative) setting.

\paragraph{Setup.}
Define the curvature 2-form $\boldsymbol{\mathcal{R}} = d\Omega + \Omega \wedge \Omega$
where the wedge product carries the ordering convention
\begin{equation}
(\Omega \wedge \Omega)_{\mu\nu} = \tfrac{1}{2}[\Omega_\mu, \Omega_\nu]_\star
    = \tfrac{1}{2}(\Omega_\mu \Omega_\nu - \Omega_\nu \Omega_\mu).
\end{equation}
The covariant exterior derivative is $D := d + [\Omega, \cdot]$ with
$[A, B] := A \wedge B - B \wedge A$.

\begin{theorem}[Second Bianchi identity in $\mathbb{B}$]
\label{thm:bianchi_biquaternion}
$D\boldsymbol{\mathcal{R}} = 0$.
\end{theorem}

\begin{proof}
Compute $D\boldsymbol{\mathcal{R}} = d\boldsymbol{\mathcal{R}} + [\Omega, \boldsymbol{\mathcal{R}}]$.

\textbf{Step 1} ($d\boldsymbol{\mathcal{R}}$): Differentiate $\boldsymbol{\mathcal{R}} = d\Omega + \Omega \wedge \Omega$:
\begin{equation}
d\boldsymbol{\mathcal{R}} = d(d\Omega) + d(\Omega \wedge \Omega) = 0 + (d\Omega) \wedge \Omega - \Omega \wedge (d\Omega).
\end{equation}

\textbf{Step 2} ($[\Omega, \boldsymbol{\mathcal{R}}]$):
\begin{equation}
[\Omega, \boldsymbol{\mathcal{R}}] = \Omega \wedge \boldsymbol{\mathcal{R}} - \boldsymbol{\mathcal{R}} \wedge \Omega
    = \Omega \wedge (d\Omega + \Omega \wedge \Omega) - (d\Omega + \Omega \wedge \Omega) \wedge \Omega.
\end{equation}
Expanding and using \textbf{associativity} $(AB)C = A(BC)$ in $\mathbb{B}$:
\begin{equation}
[\Omega, \boldsymbol{\mathcal{R}}] = \Omega \wedge d\Omega + \Omega \wedge (\Omega \wedge \Omega)
    - d\Omega \wedge \Omega - (\Omega \wedge \Omega) \wedge \Omega.
\end{equation}
By associativity, $\Omega \wedge (\Omega \wedge \Omega) = (\Omega \wedge \Omega) \wedge \Omega$, so these cancel.

\textbf{Step 3}: Combining Steps 1 and 2:
\begin{align}
D\boldsymbol{\mathcal{R}} &= (d\Omega) \wedge \Omega - \Omega \wedge (d\Omega)
    + \Omega \wedge d\Omega - d\Omega \wedge \Omega = 0.
\end{align}
\end{proof}

\paragraph{First Bianchi identity.}
The first (algebraic) Bianchi identity in its component form states:
\begin{equation}
...
\paragraph{Contracted form and conservation.}
After real projection, the contracted second Bianchi identity gives:
\begin{equation}
\nabla^\mu G_{\mu\nu} = 0
\end{equation}
where $G_{\mu\nu} = \mathrm{Re}(\mathcal{E}_{\mu\nu})$ is the classical Einstein tensor,
with $\mathcal{E}_{\mu\nu} := \mathcal{R}_{\mu\nu} - \tfrac{1}{2}\mathcal{G}_{\mu\nu}\mathcal{R}$
the biquaternionic Einstein tensor
(see Appendix~R for the full derivation and the conditions under which imaginary
cross-terms vanish under the projection).
...
\begin{itemize}
    ...
    \item Couples to internal auxiliary sector (see \texttt{speculative\_extensions/})
    \item Responsible for:
    \begin{itemize}
        \item Dark energy effects (cosmological acceleration)
        \item Internal auxiliary sector dynamics (speculative; see \texttt{speculative\_extensions/})
...
\noindent where:
\begin{itemize}
    \item $\mathcal{E}_{\mu\nu} = \mathcal{R}_{\mu\nu} - \frac{1}{2}\mathcal{G}_{\mu\nu}\mathcal{R}$ is the biquaternionic Einstein tensor (Eq.~\ref{eq:canonical:biq_einstein_tensor})
...
\begin{equation}
\mathcal{E}_{\mu\nu} = \kappa \mathcal{T}_{\mu\nu} \quad \text{(biquaternionic; $\mathcal{E}_{\mu\nu}$: Einstein tensor, $\mathcal{G}_{\mu\nu}$: metric)}
\end{equation}
