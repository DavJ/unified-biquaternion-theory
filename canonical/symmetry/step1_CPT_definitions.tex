...
Under $CPT$:
\begin{enumerate}
  \item $P$ sends $q \to \bar{q}$, which inverts only the spatial part:
    $\mathbf{q} \to -\mathbf{q}$ while $q^0$ is unchanged.
  \item $T_{\mathrm{UBT}}$ sends $\tau \to -\tau$.
    In the real-time embedding $q^0 = t$, this gives $q^0 \to -q^0$
    (since $\tau = t = q^0$ at $\psi = 0$).
    Together, items 1 and 2 give the full four-vector inversion
    $(q^0, \mathbf{q}) \to (-q^0, -\mathbf{q})$;
    note that $q \to \bar{q}$ alone does \emph{not} invert the time
    component.
...
\end{equation}
where $t \to -t$ arises from $T_{\mathrm{UBT}}$ (via $\tau \to -\tau$ with
$q^0 = t$) and $\mathbf{x} \to -\mathbf{x}$ arises from $P$ (via
$q \to \bar{q}$).
This is CPT-compatible in the real-time limit, pending the
antiunitary-$T$ inner-product resolution (Gap S1).
\end{proposition}
...
\begin{itemize}
  ...
  \item $P = P_2$ on field values + $q \to \bar{q}$: spatial parity
    (inverts $\mathbf{q}$ only; $q^0$ unchanged).
  \item $T_{\mathrm{UBT}}$: field unchanged + $\tau \to -\tau$:
    full complex time reversal; in the real-time embedding $q^0=t$,
    this gives $q^0 \to -q^0$.
  \item $CPT = (P_1 \circ P_2)$ on field values + $(q,\tau) \to (\bar{q},-\tau)$:
    full discrete symmetry; CPT-compatible in the real-time limit
    ($q^0 \to -q^0$ from $T$, $\mathbf{q} \to -\mathbf{q}$ from $P$),
    pending the antiunitary-$T$ inner-product resolution.
\end{itemize}
