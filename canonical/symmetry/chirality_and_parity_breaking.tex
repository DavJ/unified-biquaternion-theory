...
$\Theta_R \leftrightarrow$ grade-even (scalar) follows from the standard
physics convention in which the left-handed Weyl fermion transforms as a
spinor (odd under $P$) and the right-handed transforms as its conjugate.
In the $P_2$-eigenspace decomposition, grade-odd components change sign
under $P$, matching left-handed spinor behaviour.
\end{remark}
...
\end{align}
This naive argument gives $P[\mathcal{L}_{\rm weak}]=\mathcal{L}_{\rm weak}$,
which is not the correct result.  The resolution lies in the fact that the
\emph{right-handed} coupling is \emph{absent}:
\begin{equation}
...
\begin{resultbox}
\textbf{Summary.}
The left-chiral coupling \eqref{eq:chir:Lweak}:
\begin{itemize}
  \item Preserves $C$: ✓ (charge conjugation maps $W$-coupling to
    conjugate right-handed coupling, but both are physical if the full theory
    includes both chiralities for QED; the weak sector has only left)
  \item Breaks $P$: ✗ (explicitly, by absence of right-chiral $W$-coupling)
  \item Preserves $T$: ✓ (no imaginary phases in the coupling constant)
  \item Breaks $CP$: ✗ (same as $P$ breaking with $C$ invariant)
  \item Preserves $CPT$: ✓ (proof above; general CPT theorem applies)
\end{itemize}
\end{resultbox}
