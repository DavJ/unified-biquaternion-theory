...
\end{equation}
where both forms are equal since $1/i = -i$.  Verification: applying to
$\Phi_n(\psi) \propto e^{in\psi/R_\psi}$ gives
$-iR_\psi \cdot (in/R_\psi) \Phi_n = n\Phi_n$ as required by~\eqref{eq:chron:Nhat}.
...
The equal weighting of $\mathbf{I}$, $\mathbf{J}$, $\mathbf{K}$ reflects the
\emph{isotropic UBT vacuum}: the physical inner product on
$\operatorname{Im}\mathbb{H}$ is the Euclidean metric (all three directions are
equivalent under the $SO(3)$ automorphism of $\operatorname{Im}\mathbb{H}$), and
the imaginary time $i\psi$ couples equally to all three.
This choice is not arbitrary — it is the unique $SO(3)$-equivariant projection
of $\operatorname{Im}\mathbb{H}$ onto a one-dimensional complex direction.
Explicitly:
\begin{equation}
  \Pi(\mathbf{I}) = i, \quad
  \Pi(\mathbf{J}) = i, \quad
