...
% Version: 2.0 - Phase 2 (Per COPILOT_INSTRUCTIONS)
% Date: 2025-11-14
...
The fundamental time coordinate in Unified Biquaternion Theory is \textbf{complex-valued}:
...
\textbf{Note}: This is the canonical formulation per Phase 2 instructions. Biquaternion time extensions ($T_B = t + i\psi + j\chi + k\xi$) are discussed in research extensions but complex time $\tau$ is the primary framework for UBT.
...
\begin{itemize}
    ...
    \item $m_\psi$ is an effective mass scale for $\psi$ excitations (psychon mass)
...
\begin{itemize}
    \item Einstein field equations recovered exactly
...
\subsection{Extension: Full Biquaternion Time (Research Topic)}

\textbf{Note}: The canonical UBT formulation uses complex time $\tau = t + i\psi$. However, extensions to full biquaternion time are explored in research contexts.

\subsubsection{Biquaternion Time Definition}

In certain research extensions, time can be generalized to:

\begin{equation}
T_B = t + i\psi + j\chi + k\xi
\end{equation}

where $i, j, k$ are quaternion units and $\chi, \xi$ are additional imaginary time components.

\subsubsection{When Biquaternion Time May Be Needed}

Full biquaternion time may be relevant for:
\begin{itemize}
    \item Spacetime torsion effects
    \item Directional dark matter distributions
    \item Anisotropic consciousness modes
    \item Non-Abelian gauge theories beyond SM
\end{itemize}

\subsubsection{Reduction to Complex Time}

The canonical complex time is recovered when:
\begin{equation}
\chi, \xi \to 0 \quad \Rightarrow \quad T_B \to \tau = t + i\psi
\end{equation}

For most UBT applications, complex time $\tau$ is sufficient. Biquaternion extensions $T_B$ are treated as advanced topics in research appendices.
