...
\section{The Fundamental Biquaternion Field $\Theta(q,T_B)$}
...
\boxed{\Theta(q,T_B) \in \mathcal{B} = \mathbb{H} \otimes \mathbb{C}}
\end{equation}

\noindent where:
\begin{itemize}
    \item $q \in \mathcal{B}$ is a biquaternion coordinate with 4 degrees of freedom
    \item $T_B = t + i\psi + j\chi + k\xi$ is the biquaternion time (see Section~\ref{sec:canonical:biquaternion_time})
    \item In isotropic limit: $\tau = t + i\psi$ (complex time simplification)
...
\Theta(q,T_B) = \Theta_0(q,T_B) + \Theta_1(q,T_B) i + \Theta_2(q,T_B) j + \Theta_3(q,T_B) k
\end{equation}

where $\Theta_a(q,T_B) \in \mathbb{C}$ ($a = 0,1,2,3$) are \textbf{complex-valued} component fields.
