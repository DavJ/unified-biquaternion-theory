...
\begin{enumerate}
    \item \textbf{Geometric structure}: Biquaternionic tetrads $E_\mu$ emerge from $\Theta$ configurations, giving biquaternionic metric $\mathcal{G}_{\mu\nu} = \text{Sc}(E_\mu E_\nu^\dagger)$; observable metric $g_{\mu\nu} := \text{Re}(\mathcal{G}_{\mu\nu})$ is the GR projection
