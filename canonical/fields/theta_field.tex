...
\noindent where:
\begin{itemize}
    \item $q \in \mathcal{B}$ is a biquaternion coordinate (full coordinate model: 8 real parameters)
...
\noindent\textbf{Coordinate bookkeeping note.}
In this repository, \(q\) can denote either (1) the full biquaternionic coordinate \(q=z^\mu e_\mu\in\mathbb{C}\otimes\mathbb{H}\), or (2) the real-projected GR coordinate \(q_{\mathrm{GR}}=x^\mu e_\mu\) used in the classical GR recovery chain.
The canonical complex-time coordinate/fiber is \(\tau=z^0=t+i\psi\).
The GR-sector recovery assumes classical complex holomorphy only in tau.
...
\textbf{Important}: $\Theta$ is a biquaternion (NOT a matrix). Matrix representations are computational tools only.
