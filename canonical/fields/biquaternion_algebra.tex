...
\begin{enumerate}
    ...
    \item \textbf{Metric emergence}: Biquaternionic metric $\mathcal{G}_{\mu\nu} \in \mathbb{B}$ emerges from $\text{Sc}[(\partial_\mu\Theta)(\partial_\nu\Theta^\dagger)]$; the physical spacetime metric is its real projection $g_{\mu\nu} := \text{Re}(\mathcal{G}_{\mu\nu})$
...
\begin{enumerate}
    ...
    \item \textbf{Associative}: $(\Theta \Phi) \Psi = \Theta (\Phi \Psi)$
