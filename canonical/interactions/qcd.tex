...
\textbf{Status: Embedded — Track A (involution approach) canonical}\\
This section embeds standard QCD in UBT-compatible notation.

\medskip
\noindent\textbf{$SU(3)$ derivation:}
$SU(3)_c$ is \textbf{Proved [L0]} from $\C \otimes \H$ via the involution $P_2$
and carrier space $V_c = \ker(P_2+\mathbf{1}) \cong \C^3$
(Theorems G.A--G.D; see \texttt{consolidation\_project/SU3\_derivation/}).
An alternative route via octonionic extension ($\C \otimes \O$, Track B) also
works but is not required; see \texttt{research\_tracks/octonionic\_completion/}.

\medskip
\noindent\textbf{Algebraic confinement:}
Free quarks are \textbf{Conjectured [L0]} to be algebraically inadmissible in
$\mathcal{H}_{\mathrm{phys}} = \mathrm{Im}(\Pi_{\mathrm{color}})$
(colour-singlet condition on the 3-qubit colour space $\C^2\!\otimes\!\C^2\!\otimes\!\C^2$).
All colour-neutral hadrons (baryons, mesons, tetraquarks, pentaquarks) satisfy
the admissibility condition; isolated quarks do not.
See \texttt{consolidation\_project/confinement/algebraic\_confinement.tex}.\\
\textit{Note: this is distinct from the Clay Millennium Prize (Yang--Mills mass
gap); UBT does not claim to solve that problem.}
\end{minipage}}
\end{center}
