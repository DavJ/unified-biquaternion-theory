...
\begin{itemize}
    ...
    \item Coupling to internal auxiliary sector (speculative; see \texttt{speculative\_extensions/})
...
From this Lagrangian, UBT \textbf{aims to derive} the following
(see \texttt{canonical/qed\_phi\_const/} for current status):

\begin{itemize}
    \item \textbf{Electron mass}: $m_e \approx 0.511$ MeV reproduced using
      fitted parameter $R = 1/m_e$ (\textbf{not a zero-parameter prediction})
    \item \textbf{Fine structure constant}: $\alpha \approx 1/137.036$
      (\textbf{semi-empirical}; structural vacuum $n^*=137$ proved,
      coupling $B$ not yet derived from first principles)
    \item \textbf{Magnetic moment}: Anomalous magnetic moment of electron
      (Schwinger term $a_e = \alpha/(2\pi)$ \textbf{proved} at $\varphi=\text{const}$)
    \item \textbf{Lamb shift}: Energy level corrections in hydrogen
      (\textbf{sketch} — explicit path-integral computation pending, Gap L1)
\end{itemize}
