...
Step 1: SU(3) colour lattice $\Rightarrow Y\in\frac{1}{3}\ZZ$ & \PROVED\ \Lone &
  \texttt{colour\_charge\_lattice.tex}: closed via topological $S^1_\psi$ EM integrality \\
...
\midrule
\textbf{Gap C2 fully closed} & \textbf{YES} &
  Status is \textbf{[L1]} after Step 1 closure \\
\bottomrule
\end{tabular}
\end{center}

\medskip
\textbf{What is proved}: Step~1 is now closed from a topological theorem:
U(1)$_{\mathrm{EM}}$ integrality $Q\in\ZZ$ on compact $S^1_\psi$ implies
$3y\in\ZZ$ for colour triplets, hence $Y\in\frac{1}{3}\ZZ$ (PDG normalisation).
Together with Step~2 (SU(2) lattice) and Step~3 (gcd arithmetic), Gap~C2 is closed.

\textbf{Residual caveat}: hypercharge normalisation conventions
($Q=T_3+Y/2$ vs.\ $Q=T_3+Y_{\mathrm{PDG}}$) must stay explicit when quoting values.
\end{tcolorbox}
...
Step~1 (SU(3) colour lattice $\to \frac{1}{3}\ZZ$) & \PROVED\ \Lone \\
...
\textbf{Gap C2 fully closed} & \textbf{YES [L1]} \\
\midrule
