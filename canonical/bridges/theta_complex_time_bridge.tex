...
\paragraph{Status.}
Bridge / non-canonical derivation. This note is a mathematically motivated bridge between a reduced Jacobi-theta construction and the canonical UBT framework. It does not replace the canonical definitions of $\tau$ or $\Theta(q,\tau)$.

\subsection{Motivation}

\paragraph{Canonical anchor.}
Canonical UBT uses complex time
\begin{equation}
\tau = t + i\psi.
\end{equation}
The earlier biquaternion time extension $T_B = t + i\psi + j\chi + k\xi$ is deprecated as a
noncanonical/historical formulation and is not part of the canonical minimal theory.
...
\paragraph{Canonical anchor.}
The canonical field is $\Theta(q,\tau)$ and the canonical time is $\tau = t + i\psi$.
Biquaternion time $T_B = t + i\psi + j\chi + k\xi$ is a deprecated/historical extension.
