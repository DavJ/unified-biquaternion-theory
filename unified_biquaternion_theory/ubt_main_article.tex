...
% Include theory status disclaimer
...
\textbf{Important:} This document contains both established physics (General Relativity recovery) and speculative extensions (consciousness modeling). Speculative sections are clearly marked. See repository documentation for detailed theory status.

\paragraph{Acknowledgement of Prior Work.} UBT builds upon a rich tradition of quaternionic physics extending from Lanczos (1929), Gürsey (1956), Finkelstein et al. (1962), Adler (1995), and De Leo (2000), alongside recent complex-quaternion Dirac models (2022). UBT's originality lies in the synthesis of biquaternionic time, theta-function attractors, emergent gauge structure, and speculative consciousness integration—extensions beyond prior quaternionic reformulations.

\paragraph{Fine-Structure Constant Status.} The fine-structure constant $\alpha \approx 1/137.036$ is treated as an empirical input in the CORE formulation. While exploratory work examines connections between biquaternionic topology and $\alpha$, no ab initio derivation has been achieved. Current approaches include empirically fitted renormalization factors.
\end{abstract}

\section*{Note on General Relativity Compatibility}

The Unified Biquaternion Theory (UBT) \textbf{generalizes Einstein's General Relativity} by embedding the metric tensor within a biquaternionic field defined over biquaternionic time $\tau = t + i\psi$. In the real-valued limit (where the imaginary time component vanishes), UBT \textbf{exactly reproduces Einstein's field equations}:
\[
R_{\mu\nu} - \tfrac{1}{2} g_{\mu\nu} R = 8\pi G \, T_{\mu\nu},
\]
