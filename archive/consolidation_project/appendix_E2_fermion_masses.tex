...
\begin{verbatim}
   ...
   |  * Compactification: psi ~ psi + 2pi         |
   |  * UV cutoff: Lambda = 1/R_psi               |
   |  * Mode count: N_eff = 12                    |
   |  * Complex time structure: tau = t + i*psi   |
   +------------+----------------------------------+
                |
                +---> [One-Loop Vacuum Polarization]
                |         |
                |         +---> [beta-function: d(1/alpha)/d ln mu = B/(2pi)]
                |         |         |
                |         |         v
                |         |    +-------------------------+
                |         |    |  alpha(mu_0) = 1/137    |
   ...
   |  * g_1(mu), g_2(mu), g_3(mu) couplings     |
   |  * Uses alpha(mu) as input                 |
   +------------+-------------------------------+
                |
                v
   +--------------------------------------------+
   |  YUKAWA TEXTURE FROM Theta-INVARIANTS      |
   |  * Y_f[Theta, nabla Theta, R(Theta)]       |
   |  * Coefficients a_1, a_2, a_3 from action  |
   |  * NO dependence on alpha                  |
   +------------+-------------------------------+
                |
                v
   +--------------------------------------------+
   |  FERMION MASSES                            |
   |  * m_e = M_Theta x Y_e[Theta]              |
   |  * m_mu, m_tau, quarks similarly           |
   |  * DOWNSTREAM of alpha derivation          |
   +--------------------------------------------+
\end{verbatim}
