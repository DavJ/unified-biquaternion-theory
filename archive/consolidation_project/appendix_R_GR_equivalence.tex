...
\begin{remark}[Equivalence with derivative-based formula]
\label{rem:bridge_R}
The tetrad-based formula $g_{\mu\nu} = \Re(\mathrm{Sc}(E_\mu E_\nu^\dagger))$ used here
is equivalent to the derivative-based formula
$g_{\mu\nu} = \Re[\partial_\mu\Theta\cdot\partial_\nu\Theta^\dagger/\mathcal{N}]$
from \texttt{appendix\_FORMAL\_emergent\_metric.tex}, under the natural identification
