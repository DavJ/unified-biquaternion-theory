...
\textbf{Novel Feature:} UBT's core formulation uses canonical complex time $\tau = t + i\psi$ (AXIOM B). Extended formalisms explore biquaternionic time with two equivalent representations for specialized contexts:
\begin{itemize}
...
\end{itemize}
These extended formalisms are discussed in specialized appendices (e.g., Appendix N2) but are not part of the canonical axiomatic structure.
...
\textbf{Novel Feature (Candidate):} UBT provides a candidate mechanism for the Standard Model gauge group $\text{SU}(3) \times \text{SU}(2) \times \text{U}(1)$ from the internal phase structure of the biquaternionic manifold. A full derivation remains open (see \texttt{docs/sm\_embedding\_roadmap.md}); in particular, how color SU(3) emerges from internal phase degrees of freedom is a working conjecture requiring further rigorous development.
