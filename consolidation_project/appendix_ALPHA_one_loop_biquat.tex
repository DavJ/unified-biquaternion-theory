...
% TODO(alpha-B-inconsistency): The one-loop derivation yields B_0 = 8pi ~ 25.1,
%   but B ~ 46.3 is required for n_min = 137. The ratio 46.3/25.1 ~ 1.844 is far
%   too large for a perturbative two-loop QED correction (O(alpha/pi) ~ 0.002).
%   The 46.3 value appears in ALPHA_SYMBOLIC_B_DERIVATION.md via a separate
%   N_eff^(3/2) formula that is not reconciled with the standard one-loop result.
%   Until the discrepancy is resolved by an explicit CT two-loop calculation,
%   B/A must be treated as a measured input parameter following SM convention.
%   See Section~\ref{sec:B-alpha-inconsistency} for the full status.
...
% TODO(alpha-B-inconsistency): B_0 ~ 25.1 is derived; B ~ 46.3 needed for alpha^{-1}=137 is NOT yet derived. Treat B as measured input pending resolution.
\item The one-loop $\beta$-function coefficient $B_0 = 2\pi N_{\mathrm{eff}}/3 \approx 25.1$ is derived symbolically from mode counting.
\item The value $B \approx 46.3$ required by the effective-potential topological selection (Sec.~\ref{sec:alpha-value}) has \emph{not} yet been derived from first principles. Until the discrepancy is resolved, $B$ and $A/B$ are treated as measured input parameters following SM convention (see Sec.~\ref{sec:B-alpha-inconsistency}).
...
\hline
\textbf{Overall} & 100\% fitted & Partially derived (see below) \\
\hline
\end{tabular}
\end{center}

% TODO(alpha-B-inconsistency): the gap is larger than the "10%" stated here; see Sec.~\ref{sec:B-alpha-inconsistency}.
The derivation of the full coefficient $B \approx 46.3$ from first principles (specifically, closing the factor-of-$1.844$ gap between the one-loop result $B_0 \approx 25.1$ and the empirically required value) represents the main outstanding open problem; see Sec.~\ref{sec:B-alpha-inconsistency}.

\subsection{Open Problem: B/\texorpdfstring{$\alpha$}{alpha} Inconsistency}
\label{sec:B-alpha-inconsistency}

% TODO(alpha-B-inconsistency): This section documents the open inconsistency
%   and must be resolved before claiming a parameter-free derivation of alpha.

\paragraph{Statement of the inconsistency.}
The one-loop calculation in Sections~\ref{sec:one-loop}--\ref{sec:renormalization} yields:
\begin{equation}
B_0 = \frac{2\pi N_{\mathrm{eff}}}{3} = \frac{2\pi \times 12}{3} = 8\pi \approx 25.1.
\label{eq:B0-one-loop}
\end{equation}
However, the effective-potential topological selection (Section~\ref{sec:alpha-value}) requires $B \approx 46.3$ to produce $n_{\min} = 137$, hence $\alpha^{-1} = 137$. The discrepancy is:
\begin{equation}
\frac{B_{\mathrm{required}}}{B_0} = \frac{46.3}{25.1} \approx 1.844.
\label{eq:B-ratio}
\end{equation}

\paragraph{Why the gap cannot be closed by a perturbative two-loop correction.}
Standard two-loop QED corrections to the $\beta$-function coefficient are of order $\alpha/\pi \approx 0.002$, giving:
\begin{equation}
\mathcal{R}_{\mathrm{2\text{-}loop}}^{\mathrm{QED}} = 1 + \mathcal{O}(\alpha/\pi) \approx 1.001.
\label{eq:two-loop-QED-small}
\end{equation}
The required enhancement factor $1.844$ is three orders of magnitude larger than Eq.~\eqref{eq:two-loop-QED-small} and therefore \emph{cannot} be attributed to a perturbative two-loop CT correction. An alternative derivation route present in the repository (\texttt{ALPHA\_SYMBOLIC\_B\_DERIVATION.md}) uses the formula $B = N_{\mathrm{eff}}^{3/2} \times C_{\mathrm{geo}} \times R_{\mathrm{loop}}(\mu/\Lambda)$, which gives $12^{3/2} \approx 41.6$ and, with a running-coupling factor $R_{\mathrm{loop}} \approx 1.11$, yields $B \approx 46.3$. This formula and the standard one-loop formula~\eqref{eq:B0-one-loop} have not been reconciled.

\paragraph{Current status: $B$ as a measured input parameter.}
Following Standard Model convention---where the QED coupling $\alpha(m_e)$ is a measured input, not a predicted output---we treat $B$ (and the ratio $A/B$) as a \emph{measured input parameter} until an explicit derivation closes the gap:
\begin{equation}
B = 46.3 \quad \text{(measured input; SM convention)}.
\label{eq:B-SM-convention}
\end{equation}
All UBT predictions that depend on $B$ are therefore \emph{conditional} on this input. The topological selection mechanism $n_{\min} = 137 \Leftrightarrow \alpha^{-1} = 137$ is valid given this input, but does not yet constitute a parameter-free derivation of $\alpha$.

\paragraph{Resolution pathway.}
The inconsistency will be resolved when one of the following is completed:
\begin{enumerate}
  \item An explicit complex-time (CT) two-loop calculation is performed that gives $\mathcal{R}_{\mathrm{UBT}} \approx 1.844$, together with a derivation of why this large enhancement factor arises in the UBT framework.
  \item The formula $B = N_{\mathrm{eff}}^{3/2} \times C_{\mathrm{geo}} \times R_{\mathrm{loop}}$ is rigorously derived from the UBT action and shown to supersede the one-loop result~\eqref{eq:B0-one-loop}.
  \item An alternative topological-selection mechanism is found that yields $\alpha^{-1} = 137$ with $B \approx 25.1$.
\end{enumerate}
Until one of these resolutions is achieved, any claim that $\alpha$ is \emph{derived} without free parameters in UBT is premature.

% ================================================================
% §B.3 — KK Mode Sum Correction to B
% Status: [HONEST GAP] — three independent approaches tested;
%         none reproduces N_eff^{3/2} scaling.  Documented for
%         completeness and as a record for future investigators.
% See: tools/compute_B_KK_sum.py for numerical verification.
% ================================================================
\subsection{KK Mode Sum Correction to $B$ — Three Approaches}
\label{sec:B-KK-correction}

\textbf{[HONEST GAP — no derivation found; documented as open problem]}

This section reports the results of testing three independent approaches to
deriving $B_{\mathrm{base}} = N_{\mathrm{eff}}^{3/2}$ from first principles.
All numerical computations are implemented in \texttt{tools/compute\_B\_KK\_sum.py},
which may be executed to reproduce these results.  No input uses the target
value $n^* = 137$ or $\alpha = 1/137$; the only inputs are $N_{\mathrm{eff}}$
and the compactification geometry.

\subsubsection*{B.3.1 — KK Mode Sum (Euler-Maclaurin approach)  \textbf{[HONEST GAP]}}
\label{sec:B3-KK-euler}

The compact $\psi$-circle $S^1$ with radius $R_\psi$ introduces a Kaluza-Klein
(KK) tower of virtual fermion modes running in the one-loop vacuum polarization
loop~\cite{Appelquist1983}.  The KK masses are
\begin{equation}
m_k = k/R_\psi, \quad k \in \mathbb{Z} \setminus \{0\},
\label{eq:KK-masses}
\end{equation}
(setting $m_f = 0$ for the massless limit).  Mode-by-mode, the standard
Feynman-parameter form of the vacuum polarization contributes
\begin{equation}
\Pi_k(q^2) \propto \int_0^1 dz\, z(1-z)\ln\!\left[1 + \frac{q^2 z(1-z)}{m_k^2}\right].
\label{eq:Pi-k}
\end{equation}
\textbf{[SKETCH — applies the standard vacuum polarization integral
mode-by-mode to the KK tower; see Appelquist-Chodos (1983) for the full
Kaluza-Klein quantum correction derivation.]}

Setting the renormalization scale $q^2 = \Lambda^2 = 1/R_\psi^2$ (geometric UV
cutoff) and defining
\begin{equation}
f(k) = \int_0^1 dz\, z(1-z)\ln\!\left[1 + \frac{z(1-z)}{k^2}\right],
\label{eq:f-k}
\end{equation}
the compact-dimension one-loop sum is
$S_{\mathrm{KK}}(N_{\mathrm{eff}}) = (N_{\mathrm{eff}}/3\pi)\sum_{k \neq 0} f(k)$.

\paragraph{Numerical results} (see \texttt{tools/compute\_B\_KK\_sum.py}, convergence
verified at $K_{\max} = 100$):

\begin{equation}
\frac{S_{\mathrm{KK}}(N_{\mathrm{eff}})}{B_0(N_{\mathrm{eff}})} \approx 0.0052
\quad \text{for all } N_{\mathrm{eff}} \in \{4, 8, 12, 24\},
\label{eq:KK-ratio}
\end{equation}
whereas the ratio required for $B_{\mathrm{base}} = N_{\mathrm{eff}}^{3/2}$ would be
$N_{\mathrm{eff}}^{1/2}/(2\pi/3) \approx 1.65$ for $N_{\mathrm{eff}} = 12$.
The KK mode sum gives a result nearly 300 times smaller than required.

\textbf{Verdict}: The massless-KK-mode sum does \emph{not} reproduce the
$N_{\mathrm{eff}}^{3/2}$ scaling.  \textbf{[HONEST GAP]}

\subsubsection*{B.3.2 — Zeta-Function Regularization  \textbf{[HONEST GAP]}}
\label{sec:B3-zeta}

An alternative route uses the Riemann zeta regularization $\sum_{k=1}^\infty k \to \zeta(-1) = -1/12$:
\begin{equation}
\Delta B_\zeta = \frac{N_{\mathrm{eff}}}{3} \cdot 2\zeta(-1) \cdot \left(\frac{2\pi}{R_\psi\Lambda}\right)^2.
\label{eq:DeltaB-zeta}
\end{equation}
\textbf{[SKETCH — this is a formal zeta regularization; physical applicability to
the compact-dimension KK correction requires careful treatment of boundary
conditions.]}

\paragraph{Numerical results} ($R_\psi = \Lambda = 1$):
\begin{equation}
B_\zeta(N_{\mathrm{eff}} = 12) = B_0 + \Delta B_\zeta \approx 25.1 - 26.3 = -1.2.
\label{eq:Bzeta-12}
\end{equation}
The large factor $(2\pi)^2 \approx 39.5$ combined with $\zeta(-1) = -1/12$ gives a
correction $\Delta B_\zeta \approx -26.3$, which moves $B$ \emph{away} from the
required value $B_{\mathrm{base}} = 41.6$.

\textbf{Verdict}: Zeta regularization of the KK sum gives the wrong sign and
magnitude.  \textbf{[HONEST GAP]}

\subsubsection*{B.3.3 — Gauge Orbit Volume (Weyl Formula)  \textbf{[DEAD END]}}
\label{sec:B3-gauge-volume}

The hypothesis $B_{\mathrm{base}} \propto \mathrm{Vol}(\mathrm{SU}(N_{\mathrm{eff}}))$
was checked using the Weyl integration formula~\cite{Brocker1985}:
\begin{equation}
\mathrm{Vol}(\mathrm{SU}(N)) = \frac{(2\pi)^{N(N+1)/2}}{\prod_{k=1}^{N-1} k!}.
\label{eq:Vol-SUN}
\end{equation}
A power-law fit over $N \in \{4, 5, 6, 8, 10, 12\}$ gives
\begin{equation}
\mathrm{Vol}(\mathrm{SU}(N)) \sim C \cdot N^\alpha, \quad \alpha \approx 42.3
\label{eq:Vol-power-law}
\end{equation}
due to the super-exponential growth from the factorial denominator
($\mathrm{Vol}(\mathrm{SU}(N)) \sim (2\pi e/N)^{N^2/2}$ by Stirling's approximation).
This scaling is far from $N^{3/2}$.

\textbf{Verdict}: The Weyl formula gives super-exponential growth, not $N^{3/2}$.
The gauge orbit volume hypothesis is \textbf{[DEAD END]} as a direct derivation
of $B_{\mathrm{base}} = N_{\mathrm{eff}}^{3/2}$.

\subsubsection*{B.3.4 — Honest Summary  \textbf{[OPEN HARD PROBLEM]}}
\label{sec:B3-summary}
...
\hline
KK mode sum (Euler-Maclaurin) & Ratio $\approx 0.005$, not $1.65$ & [HONEST GAP] \\
Zeta-function regularization  & $B_\zeta \approx -1.2$, wrong sign & [HONEST GAP] \\
Gauge orbit volume             & $\mathrm{Vol}(SU(N)) \sim N^{42}$  & [DEAD END] \\
\hline
\textbf{Required} & $B_{\mathrm{base}} = N_{\mathrm{eff}}^{3/2} = 41.57$ & \\
\hline
\end{tabular}
\end{center}

None of the three approaches gives $N_{\mathrm{eff}}^{3/2}$ scaling.  The
formula $B_{\mathrm{base}} = N_{\mathrm{eff}}^{3/2} = 41.57$ \textbf{remains
phenomenological}.  The correctly derived one-loop value $B_0 = 25.1$ predicts
$n^*(B_0) = 67 \neq 137$.  This is an \textbf{OPEN HARD PROBLEM}.  The
$\alpha$ derivation at bare level requires $B_{\mathrm{base}}$ to be treated as
a fitted parameter until a geometric derivation is found.

\textbf{No-circularity verification}: see \texttt{validation/validate\_B\_coefficient.py},
which confirms this status independently.

\subsubsection*{B.3.5 — Twin-Prime Candidate ($B = p_{\rm twin}/N_{\rm color}$)
  \textbf{[DEAD END]}}
\label{sec:B3-twin-prime}

A new numerical direction was explored in March 2026: whether $B$ can be expressed
in terms of the twin-prime pair $(137, 139)$ of the target $n^* = 137$.

\paragraph{Hypothesis.}
Let $p_{\rm twin} = 139$ (twin prime of $n^* = 137$) and $N_{\rm color} = 3$.
Define the candidate
\begin{equation}
  B_{\rm twin} \;=\; \frac{p_{\rm twin}}{N_{\rm color}} \;=\; \frac{139}{3} \;\approx\; 46.333.
  \label{eq:B-twin-prime}
\end{equation}
With $A = 1$ (normalised kinetic coefficient) the exact $B$ required so that $n^* = 137$
satisfies $2An^* = B(\ln n^* + 1)$ is
\[
  B_{\rm required} = \frac{2A \cdot 137}{\ln 137 + 1} = \frac{274}{5.9195} \approx 46.284.
\]
The candidate~\eqref{eq:B-twin-prime} deviates from $B_{\rm required}$ by $+0.11\%$.

\paragraph{Implied $R$ factor.}
If $B = 139/3$ and $B_{\rm base} = N_{\rm eff}^{3/2} = 41.57$, then
\[
  R = \frac{B_{\rm twin}}{B_{\rm base}}
    = \frac{139}{3 \times 12^{3/2}}
    = \frac{139}{3\sqrt{1728}}
    = 1.1146,
\]
which is within $0.06\%$ of the phenomenological value $R = 1.114$.

\paragraph{Self-consistency.}
Newton iteration with $B = 139/3$ and $A = 1$ gives $n^* = 137.18$, i.e.\ $0.13\%$
above the target.  The result is not an exact integer.

\paragraph{Numerical scan.}
The script \texttt{scripts/padic/sage\_B\_derivation.sage} searched all algebraically
motivated exponents $\alpha$ for $B_{\rm required} = N_{\rm eff}^\alpha$:
no value in $\{3/2,\,8/5,\,\sqrt{2},\,\pi/2,\,\ldots\}$ gives an exact match.
A grid scan over $N_{\rm eff} \in \{4, 8, 12, 24\}$ finds that $p = 139$ is the
closest prime to $3 B_{\rm required}$ for \emph{all} values, independent of
$N_{\rm eff}$ — confirming the match is to $n^* = 137$ directly.

\paragraph{Why this is not a derivation.}
\begin{enumerate}
  \item The formula $B = 139/3$ has no geometric derivation within UBT.
        There is no mechanism in $S[\Theta]$ that selects $p_{\rm twin}/N_{\rm color}$.
  \item Even if accepted, the formula does not explain Problem~A:
        the ratio $B_{\rm base}/B_0 = N_{\rm eff}^{1/2}/(2\pi/3) \approx 1.654$ remains
        unexplained whether or not $B_{\rm twin}$ replaces $B_{\rm base}$.
  \item The numerical near-coincidence is of the same character as Eddington's numerology
        for 137 (1929) and must not be presented as a derivation.
\end{enumerate}

\begin{center}
\begin{tabular}{lll}
\hline
\textbf{Expression} & \textbf{Value} & \textbf{Error vs $B_{\rm req}$} \\
\hline
$139/3$               & 46.333  & $+0.11\%$ \\
$(137+139)/6$         & 46.000  & $-0.61\%$ \\
$\sqrt{137\times139}/3$ & 45.999 & $-0.62\%$ \\
$N_{\rm eff}^{3/2}$   & 41.569  & $-10.2\%$ \\
$B_0 = 2\pi N_{\rm eff}/3$ & 25.133 & $-45.7\%$ \\
\hline
\end{tabular}
\end{center}

\textbf{Verdict}: \textbf{[DEAD END]}.
The twin-prime approach gives an interesting numerical near-coincidence but no derivation.
Problems~A and~B remain \textbf{OPEN HARD PROBLEMS}.
...
Zeta-function regularization  & $B_\zeta \approx -1.2$, wrong sign & [HONEST GAP] \\
Gauge orbit volume             & $\mathrm{Vol}(SU(N)) \sim N^{42}$  & [DEAD END] \\
Twin-prime ($139/3$)           & $0.11\%$ off, no geometric basis   & [DEAD END] \\
Power-law scan ($N_{\rm eff}^\alpha$) & $\alpha = 1.543$, not algebraic & [DEAD END] \\
\hline
\textbf{Required} & $B_{\rm base} = N_{\rm eff}^{3/2} = 41.57$ & \\
\hline
\end{tabular}
\end{center}
...
\label{eq:V-eff}
\end{equation}
% TODO(alpha-B-inconsistency): B ~ 46.3 is used here as a measured input; the one-loop derivation gives B_0 ~ 25.1 only. See Sec.~\ref{sec:B-alpha-inconsistency}.
where $A = 1$ (normalized kinetic energy) and $B \approx 46.3$ (measured input parameter following SM convention). Note that the one-loop derivation gives only $B_0 \approx 25.1$; the origin of the larger value is an open problem documented in Sec.~\ref{sec:B-alpha-inconsistency}.
...
% TODO(alpha-B-inconsistency): beta_2-loop ~ 1.844 needed here is not derived; B ~ 46.3 must be treated as a measured input parameter until CT two-loop calculation is complete.
\item \textbf{Coefficient $B$:} The one-loop result is $B_0 \approx 25.1$ (Sec.~\ref{sec:one-loop}). The value $B \approx 46.3$ used in the effective potential (Sec.~\ref{sec:alpha-value}) requires a two-loop correction factor $\beta_{\text{2-loop}} \approx 1.844$ that has not been derived. Pending this derivation, $B$ is treated as a measured input parameter following SM convention.
...
% TODO(alpha-B-inconsistency): B ~ 46.3 is a measured input, not yet derived from first principles.
The quantities $\Lambda$, $N_{\mathrm{eff}}$, and $R_\psi$ are determined purely from geometry and mode counting. The coefficient $B_0 \approx 25.1$ is derived from these inputs at one loop. The phenomenologically required value $B \approx 46.3$ is currently treated as a \emph{measured input parameter} (SM convention) pending resolution of the open inconsistency in Sec.~\ref{sec:B-alpha-inconsistency}.
...
% TODO(alpha-B-inconsistency): B is NOT fully derived; see Sec.~\ref{sec:B-alpha-inconsistency}.
We have derived the one-loop coefficient $B_0 \approx 25.1$ from first principles within UBT, using only the biquaternionic field structure, geometric compactification, and standard QFT techniques. The UV cutoff $\Lambda$ is set by the compact imaginary-time radius $R_\psi$. However, the effective-potential calculation requires $B \approx 46.3$ to yield $\alpha^{-1} = 137$; this value has \emph{not} yet been derived—the ratio $46.3/25.1 \approx 1.844$ is far too large for a perturbative two-loop QED correction. Until an explicit derivation bridges this gap, $B$ and the ratio $A/B$ are treated as measured input parameters following SM convention (see Sec.~\ref{sec:B-alpha-inconsistency}).
