...
\textbf{Calculation of $\mathcal{R}_{\text{UBT}}$ is in progress} (requires full two-loop computation in biquaternionic field theory).
...
\textbf{Current status:} The value $\mathcal{R}_{\text{UBT}} \approx 1.84$ is determined by requiring $B \approx 46.3$ to match the observed $\alpha^{-1} = 137$. Full calculation of $\mathcal{R}_{\text{UBT}}$ from biquaternionic two-loop diagrams is in progress.
