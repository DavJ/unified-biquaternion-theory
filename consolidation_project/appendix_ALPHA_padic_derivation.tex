...
\item $\Theta$ takes values in $\mathbb{C} \otimes_{\mathbb{R}} \mathbb{H}$, the biquaternion algebra
\end{itemize}
...
B &= 46.3 \quad \text{(derived from geometric structure, see below)}
\end{align}
% NOTE ON B COEFFICIENT:
% Historically, B = 46.3 was determined by fitting V_eff(n) to select n = 137.
% As of November 2025 (line 525-527), B now has a first-principles derivation:
%   B = (2π N_eff) / (3 R_ψ) × β_2-loop ≈ 46.3
% where N_eff = 12 (mode count) and R_ψ = 1 (compactification radius).
% The numerical agreement validates the original fitting but is now theoretically justified.
...
% CRITICAL NOTE ON Z₃ - NOT A UBT PREDICTION
% ============================================
%
% The value Z₃ = 2π stated below is currently a WORKING HYPOTHESIS, not a UBT derivation.
% It is chosen phenomenologically to cancel the 2π factor in α_UBT = 1/(2πn).
%
% THIS IS CIRCULAR REASONING and should NOT be considered part of the UBT prediction.
%
% In a proper UBT program, Z₃ must be defined and computed as:
%
%   Z₃ = 1 - ∂Π(q²)/∂q² |_{q²=0} + ...
%
% where Π is the photon self-energy computed from UBT-limit Lagrangian.
% The explicit value of Z₃ is currently UNKNOWN and requires loop calculations.
%
% Until this derivation is complete, statements involving Z₃ = 2π should be
% regarded as phenomenological matching to QED, not first-principles prediction.
%
For the appropriate renormalization scheme matching to low-energy QED:
\begin{equation}
Z_3 \approx 2\pi \quad \text{(working hypothesis, not yet derived from UBT)}
\end{equation}
% TODO: MISSING DERIVATION
% The factor Z_3 = 2π is currently ASSUMED to match UBT normalization to QED.
% This assumption makes the 2π factor in α_UBT = 1/(2πn) cancel out,
% giving α_phys = 1/n. However, Z_3 should be derived from first principles
% starting from the theta-action integral (appendix_AA_theta_action.tex).
%
% Required: Rigorous derivation of Z_3 from:
%   1. UBT gauge field normalization
%   2. Complex time compactification measure
%   3. Matching to QED Ward identities
%
% Until this derivation is complete, the value Z_3 = 2π represents a
% phenomenological fitting parameter that makes the theory match observations.
% This is a CIRCULAR DEPENDENCY in the current derivation.
...
\begin{equation}
\Theta: \mathbb{A}_{\mathbb{Q}}^4 \times \mathbb{C} \to \mathbb{C} \otimes \mathbb{H}
\end{equation}
