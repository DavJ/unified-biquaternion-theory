...
% When included, the preamble and document environment are skipped
\ifdefined\UBTSTANDALONECOMPILE
  % Do nothing - preamble already loaded
\else
...
\begin{itemize}
    ...
    \item Minimal length scale $\ell_P$ from biquaternionic time periodicity
...
\textbf{UBT}: Fundamental object is 0-dimensional field $\Theta$ with rich internal algebraic structure. Spacetime emerges from field projections. Extended dimensions (biquaternionic time, internal phase) have geometric interpretation. Fewer free parameters, more constrained predictions.
...
\begin{itemize}
    ...
    \item[$\checkmark$] \textbf{Fine structure constant}: Geometric derivation of $\alpha^{-1} \approx 137$ from biquaternionic time topology
\end{itemize}
...
\begin{itemize}
    ...
    \item Proof of unitarity and causality with biquaternionic time
\end{itemize}
