...
by embedding the metric tensor in a biquaternionic field $\Theta(q,T_B)$ defined over 
biquaternionic time $T_B = t + i(\psi + \mathbf{v} \cdot \boldsymbol{\sigma})$. The real part of this field corresponds to the classical 
metric tensor, ensuring that \textbf{GR is fully contained within UBT as a special case}.

In the real-valued limit (or equivalently, when the imaginary time components $\psi, \mathbf{v} \to 0$), 
...
biquaternionic manifold, and curvature is encoded in the covariant derivatives of the
biquaternion field $\Theta(q,T_B)$. The gravitational interaction is therefore not an
