...
\item $\delta_\psi$ is the phase correction amplitude: $\delta_\psi \approx 10^{-8}$ to $10^{-6}$ (strain-independent)
...
where:
\begin{itemize}
\item $\xi_{\text{QG}}$ is the quantum gravity parameter: $\xi_{\text{QG}} = 1.2 \pm 0.3$
\item $E_{\text{Planck}} = \sqrt{\hbar c^5/G} \approx 1.22 \times 10^{19}$ GeV
...
Lamb shift & $\delta_{\psi} = 2.3 \times 10^{-6}$ & 0 & 2-5 years \\
