...
For the biquaternionic Unified \Theta-field, we face an analogous challenge: the action
\begin{equation}
...
The biquaternionic field $\Theta(q,\tau)$ is defined over biquaternionic time $\tau = t + i\psi$ and takes values in $\C \otimes \H$. The complete action must include:
\begin{itemize}
...
The bulk action for the \Theta-field is:
\begin{equation}
...
\begin{definition}[GHY Boundary Term for \Theta-Field]
...
\begin{corollary}[\Theta-Field Equation]
...
\begin{itemize}
\item \textbf{Biquaternionic \Theta-field}: Remains the single unifying object
