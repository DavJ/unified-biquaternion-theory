...
\textbf{Caution regarding complex time approximation}: Unlike QED, QCD is a non-Abelian gauge theory where gluon self-interactions lead to $[\Theta_i, \Theta_j] \neq 0$ for chromodynamic field components. Therefore, the full biquaternionic or quaternionic time structure may be required for accurate QCD calculations, particularly in the strongly-coupled regime or for confinement physics. The complex time approximation $\tau = t + i\psi$ used in some derivations should be understood as a first-order treatment valid only in the weakly-coupled perturbative regime. Future work should verify this approximation or reformulate QCD derivations using the full time structure. See Appendix~\ref{sec:biquaternion_vs_complex_time} for criteria.
