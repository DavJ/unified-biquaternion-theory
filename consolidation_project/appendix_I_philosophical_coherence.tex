...
The key insight is that biquaternionic time does not break causality—it \emph{enriches} the kinematic structure of spacetime by adding a phase dimension orthogonal to ordinary time flow.
...
\paragraph{Origin.} Theta functions are the canonical solutions to heat/diffusion equations on toroidal manifolds. Since UBT posits that biquaternionic time has topology $\mathbb{T}^2$ (2-torus), any field defined on this space will generically admit theta-function expansions.
...
\item Holomorphic properties (analyticity in biquaternionic time)
\end{itemize}

These features make theta functions the natural language for describing fields that live in biquaternionic time. Their use in UBT is analogous to using plane waves $e^{ikx}$ in ordinary quantum mechanics—they are the simplest solutions respecting the symmetries of the problem.

\paragraph{Connection to statistical physics.} Theta functions also appear as partition functions in statistical mechanics and as solutions to the quantum harmonic oscillator in imaginary time (Euclidean path integrals). This connects UBT's biquaternionic time formulation to well-established quantum field theory techniques (thermal field theory, instanton calculus).

\subsubsection{Drift-Diffusion as Conscious Dynamics}

One of UBT's more speculative proposals is that the evolution of conscious states can be modeled as a drift-diffusion process in biquaternionic time:
\begin{equation}
...
This "invisibility" is not a weakness but a feature: it explains why biquaternionic time has not been detected experimentally while remaining consistent with all current observations.
...
Figure~\ref{fig:dual_reality} (if available) would illustrate the dual nature of biquaternionic time, showing:
\begin{itemize}
