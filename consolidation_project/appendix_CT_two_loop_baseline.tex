...
\section{Biquaternionic Time vs. Complex Time (Formal Note)}
\label{app:bq-vs-ct}

On the Hermitian slice of the biquaternions \(\mathbb H_{\mathbb C}\), Lorentz transformations act as \(X\mapsto A X A^\dagger\) with \(A\in \mathrm{SL}(2,\mathbb C)\), and the determinant on self-adjoint elements yields the Minkowski invariant. This provides a natural invariant measure used for geometric locking. The complex time \(T=t+i\psi\) is introduced as an analytic/renormalization parameter within the same \(\mathbb H_{\mathbb C}\) setting; it does not abandon the biquaternionic structure. In the real-time limit \(\psi\to 0\), CT amplitudes reduce to standard QED, preserving macroscopic causality. The \(\psi\neq 0\) regime, if considered at all, is kept separate as speculative and is subject to null tests and upper bounds.

\paragraph{Necessity for the fit-free $\alpha$ derivation.}
Within the biquaternionic setting on the Hermitian slice, introducing the complex time
$T=t+i\psi$ is a necessary technical device: it provides a controlled analytic continuation
and fixes the order of limits (analyticity first, then $q^2\to 0$), while enforcing gauge
constraints via Ward identities and the Thomson normalization. Exactly these ingredients
yield the two–loop baseline $\boxed{\mathcal R_{\mathrm{UBT}}=1}$ and thus an $\alpha$
extraction without fitted parameters.
