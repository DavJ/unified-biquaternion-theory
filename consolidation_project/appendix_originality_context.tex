...
\subsubsection{Biquaternionic Time Extension}

\textbf{Novel Feature:} UBT extends time from a single real parameter to a full biquaternionic structure with two equivalent representations:
\begin{itemize}
\item \textbf{Algebraic form:} $T = t_0 + i t_1 + j t_2 + k t_3$ (global geometric structure)
\item \textbf{Operator form:} $T_B = t + i(\psi + \mathbf{v} \cdot \boldsymbol{\sigma})$ (local Hamiltonian dynamics)
\end{itemize}
Complex time $\tau = t + i\psi$ emerges as a 2D projection when vector components are negligible.

\textbf{Distinction from Prior Work:} Earlier quaternionic theories (Lanczos, Gürsey, Finkelstein) used quaternions to reformulate \emph{existing} physical equations but did not extend the temporal dimension itself into a biquaternionic structure. Recent biquaternionic Dirac models \cite{ComplexQuaternion2022a,ComplexQuaternion2022b} use complex quaternions algebraically but do not posit fundamental biquaternionic time with geometric and topological significance.

\textbf{UBT's Innovation:} The unification of \emph{physical} fields (gravity, gauge interactions) and \emph{conscious/informational} fields through a shared biquaternionic-time substrate represents a conceptual leap beyond traditional quaternionic reformulations.

\subsubsection{Hamiltonian-in-Exponent Theta Function Formulation}

\textbf{Novel Feature:} UBT introduces the Hamiltonian exponent formulation (Appendix~\ref{app:hamiltonian_theta_exponent}):
\begin{equation}
\Theta(Q, T) = \sum_{n=-\infty}^{\infty} \exp\!\left[\pi\,\mathbb{B}(n) \cdot \mathbb{H}(T)\right]
\end{equation}
where the Hamiltonian operator $\mathbb{H}(T)$ is embedded directly within the exponential of the Jacobi theta function, transforming it from a static mathematical series into a dynamical propagator.

\textbf{Distinction from Prior Work:} While theta functions have been used extensively in string theory and mathematical physics, \textbf{no known published theory embeds a biquaternionic Hamiltonian operator within the theta-function exponent} to model multiversal spectral structure. This represents a fundamental mathematical innovation specific to UBT.

\textbf{UBT's Innovation:} This formulation creates a "Hamiltonian multiverse" where each term represents a spectral branch of the solution space, with observable reality emerging from interference between branches. This goes beyond both classical theta function applications and standard quantum field theory path integral formulations.

\subsubsection{Theta-Function Attractors and Drift-Diffusion Dynamics}

\textbf{Novel Feature:} UBT employs Jacobi theta functions $\Theta(q,\tau)$ as natural solutions to the unified field equations, arising from the toroidal topology $\mathbb{T}^2$ of biquaternionic time. These theta functions act as \emph{attractors} in the phase-space dynamics, providing a mechanism for quantization and stability.
...
\item \textbf{An extension beyond} prior quaternionic theories through the introduction of biquaternionic time, theta-function attractors, and emergent gauge structure.
...
Each approach has its strengths and limitations. UBT offers a distinct perspective through its emphasis on biquaternionic structure and biquaternionic time, but it does not claim superiority over these alternative programs—merely complementarity and a different set of guiding principles.
