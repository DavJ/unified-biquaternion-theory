...
$\alpha = 1/(4\pi)$ (unit coupling at the UBT scale, i.e.\ the
left-hand side of the self-consistency equation equals $1$
after multiplying through by $4\pi$):
\begin{align}
  1 &= \frac{3}{2\pi R_\psi^2} + \frac{1}{\pi^2}
  \notag\\
  \frac{3}{2\pi R_\psi^2} &= 1 - \frac{1}{\pi^2} = \frac{\pi^2-1}{\pi^2}
  \notag\\
  R_\psi^2 &= \frac{3\pi}{2(\pi^2-1)}
  \qquad\Longrightarrow\qquad
  R_\psi^{\mathrm{SC}}
  = \sqrt{\frac{3\pi}{2(\pi^2-1)}}
  \approx 0.729.
  \label{eq:Rpsi_selfcons}
\end{align}
The ratio to the algebraic candidate is
$R_\psi^{\mathrm{alg}}/R_\psi^{\mathrm{SC}}
= (\pi/\sqrt{12})/0.729 \approx 1.24$;
...
$R_\psi^{\mathrm{SC}} \approx 0.729 \neq \pi/\sqrt{N_{\mathrm{eff}}} \approx 0.907$
(discrepancy $\approx 24\%$);
...
\begin{tabular}{lp{4.8cm}p{3.6cm}l}
  ...
  1(b): proved combos
    ...
    & \textbf{[DEAD END]} \\[6pt]
  1(c): SC det'
    & $R_\psi^{\mathrm{SC}} = \sqrt{3\pi/(2(\pi^2-1))} \approx 0.729$;\par
      $\alpha=1/137$ requires $\Phi_{\mathrm{UBT}}<0$
    & $R_\psi^{\mathrm{SC}} \neq \pi/\sqrt{N_{\mathrm{eff}}}$ (24\% off);\par
...
\begin{gap}
  ...
  \begin{enumerate}
    \item \emph{Algebraic origin of $R_\psi$ --- all four routes are dead ends.}
          ...
          gives $R_\psi^{\mathrm{SC}} = \sqrt{3\pi/(2(\pi^2-1))} \approx 0.729
          \neq \pi/\sqrt{N_{\mathrm{eff}}} \approx 0.907$ (a $\approx 24\%$ discrepancy),
