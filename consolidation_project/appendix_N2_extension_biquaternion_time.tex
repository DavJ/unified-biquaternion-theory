...
\subsubsection{Θ-Field Commutation Criterion (v9 UPDATE)}

The geometric criterion above relates to the time structure. A complementary \textbf{field-theoretic criterion} determines when complex time is valid based on the properties of the Θ-field itself.

\paragraph{Commutator-Based Transition Rule.}

Define the commutator of Θ-field components:
\begin{equation}
...
In this limit, the Θ-field components commute and the system can be described using complex time $\tau = t + i\psi$.
...
For gauge fields embedded in the Θ-field (see appendix\_E\_SM\_geometry.tex), the commutator is related to the field strength:
\begin{equation}
