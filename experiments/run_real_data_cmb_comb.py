...
Canonical root-level implementation restored without an ARCHIVE runtime dependency.
"""
...
# Canonical root-level modules live in experiments/ and scripts/.  Add those
# directories explicitly so direct CLI execution is independent of CWD and of
# the preserved ARCHIVE tree.
_script_dir = Path(__file__).resolve().parent
for _path in (_script_dir, repo_root / "scripts", repo_root):
...
def validate_data_manifest(manifest_path, dataset_type, obs_file=None, model_file=None, base_dir=None):
    ...
    if not success and fallback_desc and obs_file is not None:
        legacy_base = Path(obs_file).resolve().parent
        print(
            "WARNING: Fallback manifest did not resolve from the repository base; "
            f"retrying legacy bare paths relative to the observation directory: {legacy_base}"
        )
        success = validate_manifest.validate_manifest(resolved_path, base_dir=legacy_base)
