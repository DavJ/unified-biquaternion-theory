...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Internal auxiliary sector (speculative; see \texttt{speculative\_extensions/})
...
\begin{enumerate}
    ...
    \item \textbf{Phase-sector coupling}: Auxiliary interaction terms (speculative extensions moved to \texttt{speculative\_extensions/})
...
The fundamental field equation of UBT is defined at the biquaternionic level:
...
\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,title=Prohibition: Classical Einstein Equations Not Fundamental]
\textbf{The equation $G_{\mu\nu} = 8\pi G T_{\mu\nu}$ is NOT the fundamental field equation.}

Einstein's field equations arise ONLY after taking the real part of the fundamental biquaternionic equation:

\begin{equation}
\text{Re}(\mathcal{E}_{\mu\nu}) = \kappa \text{Re}(\mathcal{T}_{\mu\nu}) \quad \Rightarrow \quad G_{\mu\nu} = 8\pi G T_{\mu\nu}
\end{equation}

\textbf{This is labeled as ``GR limit'' or ``real projection'', never as a fundamental axiom.}
\end{tcolorbox}
...
\subsection{$\Theta$ Field to Geometry Coupling}

\textbf{Critical principle:} The $\Theta$ field configuration directly determines the geometry.

The biquaternionic metric is a functional of the $\Theta$ field:

\begin{equation}
\label{eq:canonical:metric_theta_functional}
\mathcal{G}_{\mu\nu} = \mathcal{G}_{\mu\nu}[\Theta]
\end{equation}

Explicitly, via the tetrad (Section~\ref{sec:canonical:biquaternion_tetrad}):

\begin{equation}
E_\mu = \partial_\mu \Theta \cdot \mathcal{N} \quad \Rightarrow \quad \mathcal{G}_{\mu\nu} = \text{Sc}(E_\mu E_\nu^\dagger)
\end{equation}

where $\mathcal{N}$ is a normalization operator.

\textbf{Variation of metric under $\Theta$ perturbations:}

\begin{equation}
\delta \mathcal{G}_{\mu\nu} \sim \langle D_\mu \Theta, \delta \Theta \rangle + \langle \delta \Theta, D_\mu \Theta \rangle
\end{equation}

This shows that:
\begin{itemize}
    \item Geometry is NOT background-independent of $\Theta$
    \item $\Theta$ configurations contribute dynamically to $\mathcal{G}_{\mu\nu}$
    \item Stress-energy $\mathcal{T}_{\mu\nu}$ arises from $\Theta$ gradients: $\mathcal{T}_{\mu\nu} \sim \langle D_\mu\Theta, D_\nu\Theta \rangle$
    \item There is NO external matter source—energy-momentum is geometric
\end{itemize}

\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Stress-Energy is Geometric]
In UBT, stress-energy is NOT an external matter source added to field equations.

Instead, $\mathcal{T}_{\mu\nu}$ is the \textbf{geometric phase response} of the $\Theta$ field:

\begin{equation}
\mathcal{T}_{\mu\nu} = \langle D_\mu\Theta, D_\nu\Theta \rangle_\mathbb{B} - \frac{1}{2}\mathcal{G}_{\mu\nu}\langle D\Theta, D\Theta \rangle
\end{equation}

The field equation $\mathcal{E}_{\mu\nu} = \kappa \mathcal{T}_{\mu\nu}$ is a self-consistency condition on geometry, not a source equation.
\end{tcolorbox}

\subsection{Computational Formula}

For numerical calculations, expand the biquaternionic metric. The full expansion includes cross-terms from products of complex and quaternionic units:
...
where $m^a_{\mu\nu}$ (with $a=1,2,3$) are mixed complex-quaternionic components arising from products like $\mathbf{I} \times j_a$.

In practice, these mixed terms often vanish or are subdominant for most physical configurations.
...
\subsection{Self-Consistency of the $\Theta \to \mathcal{G} \to \Omega \to \Theta$ Loop}
\label{subsec:theta_self_consistency}

The UBT construction involves a potential fixed-point loop:
\begin{equation}
\Theta \;\xrightarrow{\mathrm{(i)}}\; \mathcal{G}_{\mu\nu} = \mathrm{Sc}(E_\mu E_\nu^\dagger)
  \;\xrightarrow{\mathrm{(ii)}}\; \Omega_\mu \;\xrightarrow{\mathrm{(iii)}}\; \text{dynamics of } \Theta.
\end{equation}
Existence of solutions to this loop is a necessary condition for UBT consistency.

\paragraph{Trivial flat solution.}
For constant $\Theta = \Theta_0 \in \mathbb{B}$ (uniform field), we have
$E_\mu = \partial_\mu\Theta_0 = 0$ (vanishing derivatives), so $\mathcal{G}_{\mu\nu} = \eta_{\mu\nu}$ (Minkowski) in the linearized limit.
The connection $\Omega_\mu = 0$, and the field equation reduces to $\Box\Theta = 0$, satisfied by $\Theta = \Theta_0$.
This confirms a trivial flat solution exists.

\paragraph{Linearized perturbation analysis.}
Let $\Theta = \Theta_0 + \varepsilon\,\theta(x)$ with $|\varepsilon| \ll 1$.
At linear order:
\begin{align}
\delta\mathcal{G}_{\mu\nu} &= \mathrm{Sc}((\partial_\mu\theta)(\partial_\nu\Theta_0^\dagger) + (\partial_\mu\Theta_0)(\partial_\nu\theta^\dagger)), \\
\delta\Omega_\mu &\sim \partial_\mu(\delta E_\nu)\circ\tilde{E}^\nu_0 \quad\text{(from tetrad postulate)}, \\
\delta(\Box\Theta) &= \delta\Omega_\mu\circ\partial^\mu\Theta_0 + \Omega^\mu_0\circ\partial_\mu\theta + \cdots
\end{align}
These are wave-like equations for $\theta(x)$ on the Minkowski background,
which admit plane-wave solutions $\theta \propto e^{ik\cdot x}$.
This demonstrates existence of linearized (graviton-like) excitations.

\paragraph{Open problem.}
Non-perturbative existence (beyond linear order), uniqueness, and regularity
of solutions to the full $\Theta\to\mathcal{G}\to\Omega\to\Theta$ loop are open problems.
A rigorous fixed-point theorem in an appropriate function space is required.
