...
\medskip
\textbf{Scope note (important):} This derivation is the \emph{Hilbert variation}:
the metric $g^{\mu\nu}$ is treated as an \emph{independent} dynamical variable,
varied independently of $\Theta$.  This is \textbf{not} the $\Theta$-only closure,
in which $g = g[\Theta]$ is substituted before varying.  The $\Theta$-only formulation
is addressed separately in \texttt{GR\_closure/step2\_theta\_only\_closure.tex}
and requires an additional injectivity assumption; it holds on-shell but not
unconditionally off-shell.
\end{abstract}
