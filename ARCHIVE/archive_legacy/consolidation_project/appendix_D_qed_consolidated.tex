...
\paragraph{Note on Canonical Complex Time in QED.}
\textbf{Complex time $\tau = t + i\psi$ is the canonical time formulation in UBT (AXIOM B)}: QED in UBT uses canonical complex time. The electromagnetic field is Abelian, satisfying $[\Theta_i, \Theta_j] \approx 0$ for QED components, which aligns with the canonical complex time framework. Extended biquaternionic time formalism (discussed in Appendix~\ref{sec:biquaternion_vs_complex_time}) represents a theoretical exploration for specialized contexts such as extremely high-energy QED processes coupled to strong gravitational fields or rotating reference frames.
