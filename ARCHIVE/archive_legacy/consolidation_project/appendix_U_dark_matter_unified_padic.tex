...
\paragraph{Note on Canonical Complex Time in Dark Matter.}
\textbf{Complex time $\tau = t + i\psi$ is the canonical time formulation in UBT (AXIOM B)}: The dark matter derivations in this appendix use canonical complex time. The topological hopfion configurations are primarily spatial structures where the canonical complex time framework is appropriate. The phase coordinate $\psi$ plays a key role in decoupling dark matter from visible sector interactions, and for non-relativistic dark matter (which constitutes most cosmological DM), the complex time framework is the standard formulation. Extended biquaternionic time formalism (discussed in Appendix~\ref{sec:biquaternion_vs_complex_time}) represents a theoretical exploration for specialized contexts such as ultra-relativistic dark matter or strong-field scenarios near black holes.
