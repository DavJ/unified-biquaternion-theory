...
\label{gap:remaining}
  ...
  \begin{enumerate}
    ...
    \item[(b)] \textbf{Protection of the exponent at higher loops.}
      ...
      \textit{Status: \textbf{[PARTIAL PROGRESS] (v61).}}
      Approach E3 (holomorphic factorisation, \texttt{b\_base\_new\_directions.tex} §3)
      shows that the factor $1/2$ in the exponent is structurally forced by the
      \emph{odd real dimension} $d_\mathbb{R} = 3$ of $\mathrm{Im}(\mathbb{H})$:
      replacing the real Gaussian by a holomorphic integral over the complexification
      $\mathrm{Im}(\mathbb{H})_\mathbb{C} \cong \mathbb{C}^3$ would change the
      exponent from $3/2$ to $3$ (giving $B = N_{\mathrm{eff}}^3$), which is wrong.
      Since the real structure of $\mathrm{Im}(\mathbb{H})$ is an algebraic fact
      preserved at all loop orders, the factor $1/2$ is protected against complexification.
      This is a structural (not dynamical) protection mechanism;
      a full non-renormalisation theorem is not yet established.
  \end{enumerate}
