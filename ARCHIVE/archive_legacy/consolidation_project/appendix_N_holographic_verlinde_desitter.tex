...
\input{appendix_N5A_bekenstein_postulate_unruh}
\input{appendix_N5B_bekenstein_ubt_kms_sketch}
\input{appendix_N3_entropic_full_derivation}

\subsection{Entropic Gravity Limit in de Sitter Background}
\label{subsec:entropic_ds_limit}

The following subsections present a formal derivation of the entropic gravity limit of UBT in a de Sitter cosmological background. Starting from the biquaternionic field $\Theta(q,\tau)$, we construct an entropic density via the determinant of $\Theta^\dagger \Theta$ and show that an emergent gravitational potential arises from entropy gradients in the presence of de Sitter temperature $T_{\text{dS}} = \hbar H/(2\pi k_B)$. The construction naturally reduces to General Relativity in the weak-field limit while incorporating phase-sector contributions from the imaginary components of the biquaternionic structure. All derivations maintain gauge invariance and dimensional consistency with existing UBT formalism.

\input{appendix_N1_entropic_ds_limit}
\input{appendix_N4_two_newtonian_limits}

\input{appendix_N2_extension_biquaternion_time}
