...
\subsection{Metric Convention and Projection}

\paragraph{Fundamental biquaternionic metric.} Throughout this document, the fundamental geometric object is the \textbf{biquaternionic metric tensor} $\mathcal{G}_{\mu\nu}(q,\tau) \in \mathbb{B}$, where $\mathbb{B} = \mathbb{H} \otimes \mathbb{C}$ is the biquaternion algebra. This metric encodes the full geometric structure of spacetime including phase curvature and internal degrees of freedom.

\paragraph{Physical metric projection.} The \textbf{physical metric} $g_{\mu\nu}$ that couples to ordinary matter and appears in Einstein's field equations is obtained by the real projection:
\begin{equation}
\boxed{g_{\mu\nu} := \text{Re}(\mathcal{G}_{\mu\nu})}
\label{eq:metric_projection_main}
\end{equation}
This projection ensures compatibility with General Relativity: in the limit where all imaginary components of $\mathcal{G}_{\mu\nu}$ vanish, UBT exactly reproduces Einstein's equations. Any appearance of $g_{\mu\nu}$ in this document should be understood as this real projection unless explicitly stated otherwise.
...
\begin{equation}
\alpha_{\text{exp}}^{-1}(Q^2 \to 0) = \AlphaInvBest(21)
\end{equation}
...
In the full biquaternionic spacetime, the \textbf{fundamental biquaternionic metric} takes the form:
\begin{equation}
\mathcal{G}_{\mu\nu} = g^{(0)}_{\mu\nu} + i g^{(1)}_{\mu\nu} + j g^{(2)}_{\mu\nu} + k g^{(3)}_{\mu\nu}
\label{eq:biquaternion_metric}
\end{equation}
where $i, j, k$ are the quaternion imaginary units and each component $g^{(a)}_{\mu\nu}$ is a complex-valued tensor (yielding 8 independent real components per index pair):
\begin{itemize}
\item $g^{(0)}_{\mu\nu} \in \mathbb{C}$ is the scalar (real) part
\item $g^{(1)}_{\mu\nu} \in \mathbb{C}$ is the $i$-component (complex phase curvature)
\item $g^{(2)}_{\mu\nu} \in \mathbb{C}$ is the $j$-component (quaternionic structure)
\item $g^{(3)}_{\mu\nu} \in \mathbb{C}$ is the $k$-component (quaternionic structure)
\end{itemize}

Equivalently, using vector notation for quaternionic components:
\begin{equation}
\mathcal{G}_{\mu\nu} = g^{(0)}_{\mu\nu} + \vec{g}_{\mu\nu} \cdot \vec{\sigma}
\end{equation}
where $\vec{g}_{\mu\nu} = (g^{(1)}_{\mu\nu}, g^{(2)}_{\mu\nu}, g^{(3)}_{\mu\nu})$ and $\vec{\sigma} = (i, j, k)$.

\paragraph{Physical metric projection.} The physical metric $g_{\mu\nu}$ that couples to ordinary matter and appears in Einstein's equations is obtained by the real projection:
\begin{equation}
\boxed{g_{\mu\nu} := \text{Re}(\mathcal{G}_{\mu\nu}) = g^{(0)}_{\mu\nu}}
\label{eq:metric_projection}
\end{equation}
This projection ensures that UBT embeds general relativity: when all imaginary components vanish, $\mathcal{G}_{\mu\nu} \to g_{\mu\nu}$ exactly reproduces Einstein's field equations.

\subsubsection{Reduction to Complex Metric}

When the projection criterion \eqref{eq:projection_criterion} is satisfied, the biquaternionic metric reduces to a complex-valued metric:
\begin{equation}
\mathcal{G}_{ab}(\tau) = g^{(0)}_{ab} + i g^{(1)}_{ab}
...
Let $\Theta: M \times S^1 \to \mathbb{C} \otimes \mathbb{H}$ be a biquaternion field on spacetime $M$ with compact imaginary time $S^1$. Let $A_\mu$ be a $U(1)$ gauge connection with coupling $g$. Then for $\Theta$ to be globally well-defined, the gauge coupling must satisfy:
\begin{equation}
...
\subsection{Notation for the Complex-Time Correction}

We denote by $\Delta_{\mathrm{CT}}$ the finite archimedean correction from the complex-time sector,
computed as the two-loop contribution obtained by matching the UBT current correlator
to the QED vacuum polarization in the Thomson limit, without fitted parameters.

\bigskip

\noindent\textbf{Context.}
Let $\Theta(q,\tau)$ be the biquaternion field, with spatial covariant derivative $D_\mu$,
complex-time derivative $D_\tau$, and curvature coupling $\Lambda\!-\!R_{\Theta}$.
In the archimedean limit, define the dimensionless ratio of invariants
\begin{equation}
\alpha \;=\; \frac{\mathrm{Re}\!\left[\Theta^\dagger D_\mu \Theta\right]}
                  {\mathrm{Im}\!\left[\Theta^\dagger D_\tau \Theta\right]}\,.
\label{eq:alpha-ratio}
\end{equation}

\begin{lemma}[Place-covariance normalization]\label{lem:place-covariance}
Let $\Theta(q,\tau)$ be the biquaternion field on the place-covariant background with curvature coupling $\Lambda\!-\!R_{\Theta}$, and let $\alpha$ be given by \eqref{eq:alpha-ratio}.
Then $\alpha$ is uniquely normalized (i.e.\ contains no free fit parameter) under place-covariant rescalings of fields and measure. Moreover, in the prime-level sector $k=p$ (theta mode) it admits the representation
\begin{equation}
\alpha^{-1} \;=\; p \;+\; \Delta_{\mathrm{CT}}\,,
\label{eq:alpha-main}
\end{equation}
where $p$ is the prime anchor from the level quantization and $\Delta_{\mathrm{CT}}$ is the archimedean correction determined by the UBT$\leftrightarrow$QED matching.
\end{lemma}

\begin{proof}[Proof sketch]
(i) Place-covariance fixes the relative normalization of the two invariants via the $\Lambda\!-\!R_{\Theta}$ coupling, eliminating arbitrary scale factors. %
(ii) Prime-level quantization of the theta mode selects $k=p$ and yields the discrete anchor for $\alpha^{-1}$ in the place-agnostic measure. %
(iii) Passing to the archimedean limit, the complex-time sector contributes the finite correction $\Delta_{\mathrm{CT}}$ obtained by matching the UBT current two-point function to the QED vacuum polarization at low momentum. %
Combining (i)--(iii) gives \eqref{eq:alpha-main}.
\end{proof}

\subsection{Ward Identity and Thomson Limit}\label{sec:ward-thomson}

Let $\Pi_{\mu\nu}(q)$ be the current-current correlator extracted from the UBT action in the archimedean limit.
Gauge invariance implies current conservation and the Ward identity
\begin{equation}
q^\mu \, \Pi_{\mu\nu}(q) \;=\; 0 \,,
\end{equation}
...
\Pi_{\mu\nu}(q) \;=\; \big(q_\mu q_\nu - q^2 g_{\mu\nu}\big)\, \Pi(q^2)\,.
\end{equation}
In the Thomson limit ($q^2\!\to\!0$) the renormalization condition fixes the finite part of $\Pi(q^2)$, and the emergent fine-structure constant reads
\begin{equation}
\alpha^{-1} \;=\; p \;+\; \Delta_{\mathrm{CT}}, 
\qquad
\Delta_{\mathrm{CT}} \;=\; \lim_{q^2\to 0} \Pi(q^2)\,,
\end{equation}
where the discrete anchor $p$ comes from the prime-level quantization of the theta mode and $\Delta_{\mathrm{CT}}$ is the two-loop archimedean correction. No fitted parameter enters this matching.

\begin{theorem}[Emergent fine-structure constant]\label{thm:alpha-emergent}
Under Lemma~\ref{lem:place-covariance} and the Ward identity in the Thomson limit,
the fine-structure constant in the UBT framework satisfies
\begin{equation}
\boxed{\;\alpha^{-1} \;=\; p \;+\; \Delta_{\mathrm{CT}}\;}
\end{equation}
with $p$ the prime-level anchor and $\Delta_{\mathrm{CT}}$ the archimedean two-loop correction
fixed by UBT$\leftrightarrow$QED matching. No fitted parameters are required.
\end{theorem}

\paragraph{Numerical check.}
The value of $\Delta_{\mathrm{CT}}$ is computed in the companion code (\texttt{alpha\_core\_repro/alpha\_two\_loop.py})
and logged to \texttt{alpha\_core\_repro/out/alpha\_two\_loop.csv}. Plugging the result into
$\alpha^{-1} = p + \Delta_{\mathrm{CT}}$ with $p=137$ reproduces the experimental inverse value
...
\section{Renormalization–group running of $\alpha(\mu)$}
\label{app:RG-alpha}
For completeness we record the low–energy QED running that relates the static UBT value
$\alpha_p$ (defined via the Ward$\to$Thomson matching at $\mu\!\to\!0$) to its value at a finite
scale $\mu$. At one loop in QED,
\begin{equation}
\frac{d\,\alpha}{d\ln\mu} \;=\; \beta(\alpha) \;=\; \frac{2}{3\pi}\Big(\sum_f Q_f^2\Big)\,\alpha^2 \,,
\qquad\Rightarrow\qquad
\alpha^{-1}(\mu) \;=\; \alpha^{-1}(\mu_0) \;-\; \frac{2}{3\pi}\Big(\sum_f Q_f^2\Big)\ln\!\frac{\mu}{\mu_0}\,,
\label{eq:RG-alpha}
\end{equation}
where the sum runs over active charged fermions $f$ with $Q_f$ in units of $e$.
Including fermion thresholds yields the familiar piecewise–logarithmic running via decoupling.

\paragraph{Connection to the UBT sector prediction.}
In a given Hecke sector $p$ we predict the static anchor
\begin{equation}
\alpha_p^{-1} \;=\; p \;+\; \Delta_{\mathrm{CT}}(p)\,,
\end{equation}
with $\Delta_{\mathrm{CT}}(p)$ the finite two–loop archimedean correction obtained by matching the UBT current
two–point function to QED in the Thomson limit. Equation~\eqref{eq:RG-alpha} then transports $\alpha_p$ from
$\mu\!\to\!0$ to experimental scales $\mu$ without any additional free parameters.

\paragraph{Ward identity and vacuum polarization.}
The UBT matching uses the QED vacuum polarization $\Pi(q^2)$ and the Ward identity to fix the finite part
\(
\Delta_{\mathrm{CT}}(p) \,=\, \lim_{q^2\to 0}\Pi(q^2)
\)
in strict analogy to the low–energy normalization of the QED coupling. Higher–loop QED/RG effects can
be included systematically; the sector anchor $p$ remains discrete and RG–invariant.

\paragraph{Practical note.}
In our numerics we adopt $\mu_0$ in the deep infrared (Thomson limit); to compare against data reported at
$\mu\!=\!m_Z$ or $\mu\!=\!m_e$, use \eqref{eq:RG-alpha} with the appropriate set of active fermions between
the thresholds and the chosen scale.


\section{Fermion Masses: MSbar Scheme and QED Pole Mass}
\label{app:masses}

Having obtained the fine structure constant $\alpha$ from fit-free UBT principles (Ward$\to$Thomson matching
at two loops), we extend the framework to fermion masses. This section describes the computational pipeline
implemented in \texttt{ubt\_masses/}.

\subsection{Scheme and Scale Choice}

Lepton masses are defined in the $\overline{\mathrm{MS}}$ renormalization scheme at scale $\mu = \bar{m}_\ell(\mu)$.
This ``on-shell-like'' prescription eliminates large logarithms $\ln(\mu^2/m^2)$ in the perturbative
QED conversion between $\overline{\mathrm{MS}}$ mass $\bar{m}_\ell(\mu)$ and pole mass $m_\ell^{\text{pole}}$.

\paragraph{Why $\overline{\mathrm{MS}}$ for masses?}
In the $\overline{\mathrm{MS}}$ scheme, ultraviolet divergences are absorbed into coupling and mass counterterms
via minimal subtraction in dimensional regularization. The $\overline{\mathrm{MS}}$ mass $\bar{m}(\mu)$ is
scale-dependent and must satisfy a renormalization group equation (RGE). At one loop in QED,
\begin{equation}
\mu\frac{d\bar{m}}{d\mu} \;=\; \gamma_m(\alpha(\mu))\,\bar{m}\,,\qquad
\gamma_m \;=\; -\frac{3\alpha}{4\pi} + \mathcal{O}(\alpha^2)\,.
\label{eq:mass-rge}
\end{equation}
The anomalous dimension $\gamma_m$ is small for leptons; RG running between scales is logarithmic
and sub-percent across typical mass ranges.

\subsection{QED Conversion: MSbar $\leftrightarrow$ Pole Mass}

The physical (pole) mass $m^{\text{pole}}$ is related to the $\overline{\mathrm{MS}}$ mass by QED self-energy corrections.
At one loop,
\begin{equation}
m_\ell^{\text{pole}} \;=\; \bar{m}_\ell(\mu) \left[1 + \frac{\alpha(\mu)}{\pi}\left(1 + \frac{3}{4}\ln\frac{\mu^2}{\bar{m}_\ell^2}\right) + \mathcal{O}(\alpha^2)\right].
\label{eq:pole-from-msbar}
\end{equation}
When we choose $\mu = \bar{m}_\ell$, the logarithm vanishes and the formula simplifies to
\begin{equation}
m_\ell^{\text{pole}} \;\approx\; \bar{m}_\ell(\bar{m}_\ell)\left(1 + \frac{\alpha(\bar{m}_\ell)}{\pi}\right) + \mathcal{O}(\alpha^2)\,.
\label{eq:pole-simple}
\end{equation}
This prescription is numerically stable and well-suited for precision calculations.

\paragraph{TODO: Two-loop QED.}
The current implementation uses the one-loop formula \eqref{eq:pole-from-msbar}. For target precision
$|\Delta m/m| < 10^{-5}$ we need the two-loop QED self-energy correction,
\begin{equation}
m^{\text{pole}} \;=\; \bar{m}(\mu)\left[1 + \frac{\alpha}{\pi} + \left(\frac{\alpha}{\pi}\right)^2 C_2 + \ldots\right],
\label{eq:pole-2loop}
\end{equation}
where $C_2$ includes $\pi^2$ and higher-order logarithmic terms. This refinement is planned for future work.

\subsection{Fit-Free Mass Pipeline}

The mass calculation pipeline is \emph{fit-free}: it consumes only the two-loop $\alpha(\mu)$ from
\texttt{alpha\_core\_repro} (strict mode, no mocks). Specifically:

\begin{enumerate}
\item \textbf{α provider:} The function \texttt{ubt\_alpha\_msbar}($\mu$) calls the two-loop UBT calculation
      in strict mode. No manual constants, no fitted parameters.

\item \textbf{MSbar mass operator:} The function \texttt{ubt\_mass\_operator\_electron\_msbar}($\alpha(\mu)$)
      returns $\bar{m}_e(\mu)$ from UBT geometric invariants (currently a placeholder; will be derived
      from $\Theta$ field VEV, complex time compactification, and Yukawa structure).

\item \textbf{Self-consistency:} We solve the fixed-point equation $\mu = \bar{m}_\ell(\mu)$ iteratively
      (typically converges in 2--5 steps) to eliminate residual scale dependence.

\item \textbf{Pole mass:} Apply \eqref{eq:pole-from-msbar} with $\alpha(\mu)$ from step~1 to obtain
      the physical mass $m_\ell^{\text{pole}}$.
\end{enumerate}

\paragraph{Current precision.}
For the electron, we achieve relative error $|\Delta m_e / m_e| < 10^{-4}$ using one-loop QED.
The TODO for two-loop QED (cf. \eqref{eq:pole-2loop}) will tighten this to $< 10^{-5}$.

\subsection{Error Budget}

The total error in the electron pole mass has three main contributions:

\begin{enumerate}
\item \textbf{Input $\alpha(\mu)$:} The two-loop UBT calculation of $\alpha$ has intrinsic uncertainty from
      higher-order (three-loop+) corrections, currently estimated at $\mathcal{O}(\alpha^3/\pi^3) \sim 10^{-7}$.
      This is subdominant.

\item \textbf{QED truncation:} Using one-loop instead of two-loop QED conversion introduces an error
      $\sim (\alpha/\pi)^2 \approx 5 \times 10^{-5}$. This is the dominant source and will be removed
      by implementing \eqref{eq:pole-2loop}.

\item \textbf{UBT mass operator:} The placeholder formula for $\bar{m}_e$ will be replaced by the
      actual UBT derivation from biquaternion dynamics. The expected precision depends on the matching
      to Standard Model Yukawa couplings and is under investigation.
\end{enumerate}

Once two-loop QED is implemented, the limiting factor will shift to the UBT mass operator itself,
requiring deeper understanding of the Higgs-like mechanism in complex-time biquaternionic formalism.

\subsection{References and Code}

All mass calculations are implemented in the \texttt{ubt\_masses/} Python package:
\begin{itemize}
\item \texttt{core.py}: Fit-free $\alpha(\mu)$ provider, MSbar mass operator, fixed-point solver
\item \texttt{qed.py}: QED pole mass conversion (1-loop; TODO: 2-loop)
\end{itemize}
Tests are in \texttt{tests/test\_electron\_mass.py}, validating fit-free identity, self-consistency,
and precision against PDG 2024 data. Run via \texttt{make masses-tests}.

For muon and tau, the same QED structure applies; implementation is pending (\texttt{NotImplementedError}).
For quarks, we must include QCD running and pole mass definitions at 3--4 loops, with careful treatment
of thresholds and scheme matching.
...
% === Per-sector numeric table from CSV ===
% requires: amsmath, amsthm, booktabs, pgfplotstable, hyperref
\input{UBT_alpha_per_sector_patch}

\section{Hecke presentation options: A/B/C}
We summarize three equivalent ways to realize the Hecke structure relevant for UBT. 
All three yield the same prime–indexed factorization and thus the same sector anchor $p$ in the result 
$\alpha_p^{-1}=p+\Delta_{\mathrm{CT}}(p)$; we adopt Variant~C in the main derivation for conceptual clarity.

\paragraph{Variant A (theta constants, half–integral weight, Kohnen plus).}
Construct $\vartheta(\tau)$ from Jacobi theta constants $\theta_2,\theta_3,\theta_4$ at $z=0$ so that 
$\vartheta$ transforms with weight $k=\tfrac12$ on $\Gamma_0(4N)$ (nebentypus $\chi$) in Kohnen’s plus space. 
Hecke acts by $T(p^2)$ on Fourier coefficients.

\paragraph{Variant B (theta series of binary quadratic forms).}
Let $\vartheta_Q(\tau)=\sum_{x\in\mathbb Z^2} e^{\pi i\, Q[x]\tau}$ for a positive definite binary quadratic form $Q$. 
Again $k=\tfrac12$, level $4N$, nebentypus $\chi$, and Hecke uses $T(p^2)$.

\paragraph{Variant C (automorphic/L–function route; used here).}
Take $\vartheta(\tau)$ as above, pass to $L(\vartheta,s)$ by Mellin transform (with the standard gamma factor), 
and use the Euler product $L(\vartheta,s)=\prod_p L_p(\vartheta,s)$ to identify the local place $p$ (Hecke sector). 
This makes the prime anchor explicit and keeps the physics matching (Ward $\to$ Thomson) purely archimedean.

\begin{proposition}[Presentation invariance of the prime anchor]
Let $\vartheta$ be realized by A (theta constants) or B (theta series). 
Then $L(\vartheta,s)$ obtained by Mellin transform admits the same Euler product $\prod_p L_p(\vartheta,s)$ 
as in Variant~C. Consequently, the identification of the Hecke sector as the local place $p$ and the formula
$\alpha_p^{-1}=p+\Delta_{\mathrm{CT}}(p)$ are independent of the chosen presentation.
\end{proposition}

\paragraph{Remark on bad primes.}
At $p=2$ and primes dividing the level, local formulas for Hecke factors differ by standard sign/ramification details. 
These do not affect the archimedean correction $\Delta_{\mathrm{CT}}(p)$ nor the sector anchoring by $p$.

\paragraph{Choice in this work.}
We adopt Variant~C for transparency (theta $\to$ Mellin $\to$ $L(\vartheta,s)$ with Euler product), 
and include A/B constructions in the Appendix for completeness.

% === Hecke route via L-functions (Variant C) ===
% provides theta -> Mellin -> L(ϑ,s) and prime-indexed local factors
\input{tex/UBT_hecke_L_route}

\paragraph{Remark on bad primes ($p=2$ and primes dividing the level).}
At $p=2$ and primes $p\mid 2N$ the local Hecke theory for half–integral weight requires modified
operators and signs (ramification, Atkin–Lehner). The resulting local Euler factors differ by standard
conventions (\cite{Kohnen1980,Iwaniec1997}), but this does \emph{not} affect the archimedean correction
$\Delta_{\mathrm{CT}}(p)$ nor the sector anchoring by the prime index itself: the UBT matching in the Thomson
limit remains identical, hence $\alpha_p^{-1}=p+\Delta_{\mathrm{CT}}(p)$ holds sector–wise without fitted parameters.


\subsection{Hecke eigenform and presentation invariance}
\label{subsec:eigenform-invariance}
Let $\vartheta(\tau)=\sum_{n\ge0} a_n q^n$ be our half–integral weight object in Kohnen’s plus space
$S_{1/2}(\Gamma_0(4N),\chi)$ built from theta constants (Variant~A) or from binary quadratic forms (Variant~B).
For primes $p\nmid 2N$ the Hecke operator $T(p^2)$ acts on Fourier coefficients by
\[
(T(p^2)\,\vartheta)(\tau)=\sum_{n\ge0}\!\Big(a_{p^2 n}+\chi(p)\,p^{-1/2}\,a_n+p^{0}\,a_{n/p^2}\Big)q^n, 
\quad a_{n/p^2}=0\text{ if }p^2\nmid n,
\]
i.e. the half–integral weight $k=\tfrac12$ exponents are standard (\cite{Shimura1973,Kohnen1980,Iwaniec1997}).

\begin{proposition}[Eigenform and sector anchoring]
Assume $\vartheta$ is a Hecke eigenform for $T(p^2)$ at all primes $p\nmid 2N$, i.e.
$T(p^2)\,\vartheta=\lambda_p\,\vartheta$. Then the Mellin transform $L(\vartheta,s)=\sum_{n\ge1}a_n n^{-s}$
admits an Euler product $L(\vartheta,s)=\prod_p L_p(\vartheta,s)$ whose local factor at $p\nmid 2N$ is
\[
L_p(\vartheta,s)=\left(1-\lambda_p\,p^{-s}+\varepsilon_p\,p^{-2s}\right)^{-1}, 
\]
with $\varepsilon_p\in\{\pm1\}$ determined by the space and normalization. Consequently, selecting the local place $p$
defines a Hecke sector and the UBT prediction
\[
\boxed{\alpha_p^{-1}=p+\Delta_{\mathrm{CT}}(p)}
\]
is presentation–invariant: it is the same whether $\vartheta$ is realized via Variant~A, Variant~B, or accessed
through the automorphic $L$–route (Variant~C).
\end{proposition}

\begin{proof}[Sketch]
Kohnen’s plus space is stable under $T(p^2)$ for $p\nmid 2N$; eigenforms exist and correspond under the Shimura lift
to integral–weight eigenforms on $\Gamma_0(N)$, matching Hecke eigenvalues (\cite{Shimura1973,Kohnen1980,Serre1987}).
Mellin($\vartheta$) produces $L(\vartheta,s)$ with Euler factors determined by the $T(p^2)$ action. 
Since UBT’s archimedean matching (Ward$\to$Thomson) defining $\Delta_{\mathrm{CT}}(p)$ is place–agnostic, the sector
anchor is the prime $p$ itself, independent of how $\vartheta$ is presented.
\end{proof}
