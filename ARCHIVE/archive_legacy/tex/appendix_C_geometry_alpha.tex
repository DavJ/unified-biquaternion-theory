...
\paragraph{Metric convention.} All curvature data are computed from the biquaternionic metric $\mathcal{G}_{\mu\nu}(\Theta)$ and its Ricci tensor $\mathcal{R}_{\mu\nu}$. Whenever the real projection is referenced we use
\[
g_{\mu\nu}:=\Re(\mathcal{G}_{\mu\nu}),\qquad
R_{\mu\nu}:=\Re(\mathcal{R}_{\mu\nu}),
\]
explicitly marking the General Relativity limit rather than assuming a classical spacetime metric.
