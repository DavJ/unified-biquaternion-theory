...
The CTC framework emerges when we apply a **projection** that simplifies the geometry. We consider phenomena where the internal structures of the three spatial coordinates can be neglected, and the only relevant internal dimension is the one associated with time, \( \psi \). This reduces the manifold to a space with three real spatial coordinates (\(x, y, z\)) and one biquaternionic time coordinate, \( \tau = t + i\psi \).

\paragraph{Holographic Projection Interpretation:}
More precisely, the biquaternionic time \( \tau \) used in CTC can be understood as a \emph{holographic projection} of the full biquaternionic time structure. In the complete UBT formalism, time is described by:
\begin{equation}
...
\begin{itemize}
    ...
    \item The variable `\(\tau\)` is directly related to our biquaternionic time, defining the geometry (or "modulus") of the toroidal phase space.
\end{itemize}
