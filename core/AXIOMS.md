...
**Note — Generalized Phase Projection**: The formula $g_{\mu\nu} = \text{Re}(\mathcal{G}_{\mu\nu})$ is the **$\varphi = 0$ special case** of the general operator $P_\varphi[\mathcal{G}_{\mu\nu}] := \text{Re}(e^{-i\varphi}\mathcal{G}_{\mu\nu})$ defined in `core/canonical/geometry/phase_projection.tex`. The choice $\varphi = 0$ is the standard observer gauge convention. GR is recovered for *any* spatiotemporally constant $\varphi$ (see `core/canonical/geometry/gr_as_limit.tex`). **Axiom C is unchanged**; this note records the broader framework.
**Cross-References**:
...
- `core/canonical/geometry/phase_projection.tex` (generalized $P_\varphi$ operator)
- `core/canonical/geometry/gr_as_limit.tex` (GR for any constant $\varphi$)
