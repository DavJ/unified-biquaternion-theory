...
|---|---|---|---|
| Local classical single-Theta GR recovery from the adopted one-constant gravity dynamical postulate | PROVED | `canonical/gr_closure/gravity_dynamical_postulate.en.md`, `canonical/gr_closure/gr_recovery_status.yaml`, `research_tracks/action_selection/poincare_one_constant_gr_closure.en.md`, `canonical/gr_closure/gap_10t_split_jet_right_inverse.tex`, `canonical/gr_closure/gap_10t_palatini_torsion_dynamics.tex` | **Local classical GR recovery is CLOSED** on regular non-null split-jet patches after adoption of the Poincare-contracted fifth-channel Palatini law. `Theta` is the only fundamental physical field; there is no independent tetrad. Surjective algebraic jet variations impose the complete tetrad Einstein equation, the physical connection reduces to Levi-Civita in spinless vacuum, and the sole continuous gravitational constant is `kappa`. The branch fixes `Lambda_bare=0`. This does not claim that kinematics alone forced the dynamical postulate, nor does it close the full gauge/matter/quantum action, global/null-patch completion, effective dark energy, or UV psi stability. |
