...
|---|---|---|---|
| Lokální klasická obnova GR z jediného Theta po přijetí jednokonstantního gravitačního dynamického postulátu | PROVED | `canonical/gr_closure/gravity_dynamical_postulate.cs.md`, `canonical/gr_closure/gr_recovery_status.yaml`, `research_tracks/action_selection/poincare_one_constant_gr_closure.cs.md`, `canonical/gr_closure/gap_10t_split_jet_right_inverse.tex`, `canonical/gr_closure/gap_10t_palatini_torsion_dynamics.tex` | **Lokální klasická obnova GR je CLOSED** na regulárních nenulových split-jet patchích po přijetí Poincarého kontrahovaného pátokanálového Palatiniho zákona. `Theta` je jediné fundamentální fyzikální pole; nezávislá tetráda neexistuje. Surjektivní algebraické jet variace ukládají úplnou tetrádovou Einsteinovu rovnici, fyzická konexe se ve spinless vakuu redukuje na Levi–Civitovu a jedinou spojitou gravitační konstantou je `kappa`. Větev fixuje `Lambda_bare=0`. Toto netvrdí, že samotná kinematika vynutila dynamický postulát, ani neuzavírá úplnou gauge/hmotovou/kvantovou akci, globální/nulové patche, efektivní temnou energii či UV stabilitu psi. |
...
| Split-jet pomocná geometrie a variační přenos tetrádové rovnice | PROVED | `canonical/gr_closure/gap_10t_split_jet_right_inverse.tex`, `canonical/gr_closure/gap_10t_split_jet_auxiliary_completion.tex`, `research_tracks/action_selection/split_jet_palatii_variational_lift.cs.md`, `tools/verify_split_jet_palatii_variational_lift.py` | Lokální pravá inverze je exaktní na `X^2 != 0`; jet proměnné jsou algebraické/nepropagující; a po vložení dovnitř přijatého gravitačního funkcionálu je jejich variační zobrazení hodnosti čtyři surjektivní na všechny tetrádové směry. Stacionarita proto ukládá úplnou tetrádovou Eulerovu formu, nikoli projektovanou rovnici. Globální/nulové patche jsou samostatný problém. |
| Indukovaný Einsteinův koeficient z Hessiánu Theta (alternativní/historická cesta) | DERIVED_WITH_ASSUMPTIONS | `canonical/gr_closure/gap_10d_induced_gravity_endgame.tex`, `canonical/gr_closure/gap_10_gr_effective_completion.tex`, `tools/verify_gr_endgame_completion.py` | `GAP-10D-UNDERDETERMINATION: CLOSED AS NO-GO [L1]`; `GAP-10D-A2-FORM` a `GAP-10D-SPECTRAL-IR` zůstávají podmíněnými tvrzeními pro specifikovaný Hessián a míru. Tato cesta už není nutná pro lokální klasické uzavření GR po přijetí jednokonstantního dynamického postulátu, ale zůstává relevantní pro kvantové/UV odvození a případný výpočet efektivních koeficientů. |
| Schwarzschildovo řešení v přijaté gravitační větvi UBT | PROVED | `canonical/gr_closure/gravity_dynamical_postulate.cs.md`, `canonical/gr_closure/gr_recovery_status.yaml`, `canonical/geometry/schwarzschild_claim_status.yaml`, `papers/UBT_GR_Submission_canonical_correction.cs.tex` | `GAP-U2Theta: CLOSED LOCALLY`. Schwarzschild je vakuovým řešením přijaté Palatiniho/Einsteinovy větve s nulovou bare `Lambda` a má explicitní lokální split-jet lift. Starší přímý ansatz v `biquaternionic_vacuum_solutions.tex` zůstává neplatný a nahrazený. Globální pokračování přes horizont je samostatný problém globálního dokončení. |
