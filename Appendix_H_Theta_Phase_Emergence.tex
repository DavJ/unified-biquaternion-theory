...
% Layer: L1
% ================================================================
\subsection*{H.7a — Prime Attractor Theorem}

\textbf{Layer:} [L1] — biquaternionic geometry.

\bigskip

\noindent\textbf{Setup.} The Kaluza-Klein expansion
$\Theta(x,\psi) = \sum_n a_n(x)\,e^{in\psi}$
substituted into $\nabla^\dagger\nabla\Theta = \kappa\mathcal{T}$ with
self-interaction $U(\Theta) = \lambda|\Theta|^4$ yields the mode equation:
...
\textbf{[DERIVED]} from substitution of the KK expansion if the coupling is
\emph{multiplicative} ($k \cdot m = n$). \textbf{[DEAD END if additive ($k+m=n$):}
every integer $n = 1 + (n-1)$ has submodes, destroying the prime argument.]

The coupling structure must be verified from the explicit field equation (see
\texttt{tests/test\_prime\_attractor\_stability.py}, \texttt{test\_coupling\_structure}).
...
$\mathrm{Re}(\mu) \leq -\gamma\cdot n + \lambda A < 0$ when $\gamma > \lambda A/n \leq \lambda A/1$.
...
``Composite winding modes decay by splitting into their prime factors.''
...
\texttt{test\_prime\_modes\_stable\_with\_coupling}).

Update \texttt{DERIVATION\_INDEX.md}: ``Prime attractor'' $\to$ Proven (conditional on multiplicative coupling, Step~1).
