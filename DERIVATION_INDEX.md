...
| B_base = N_eff^{3/2} = 41.57 | **Motivated Conjecture** [with explicit gap] | `STATUS_ALPHA.md §9`, `consolidation_project/appendix_ALPHA_one_loop_biquat.tex §B.3`, `tools/compute_B_KK_sum.py`, `consolidation_project/alpha_derivation/b_base_spinor_approach.tex`, `consolidation_project/alpha_derivation/b_base_hausdorff.tex`, `tools/explore_b_exponent.py` | v58: Approach A2 (Hausdorff) expanded with Lemma (mode space Im(ℍ), d=3, PROVED), Proposition (Gaussian det^{-1/2}, mathematical identity), Conjecture (exponent = d/2 = 3/2, MOTIVATED CONJECTURE), Gap §4 (requires: (a) explicit det(S'') computation; (b) higher-loop protection — both OPEN). Factor 3 = dim_ℝ(Im ℍ) from algebra; factor 2 from Gaussian integration — neither is free. Dead ends: KK sum, zeta, gauge orbit volume. A3 (numerical: x = log(46.284)/log(12) = 1.543, nearest fraction 17/11, deviation from 3/2 entirely explained by R ≠ 1) |
