...
- The compact-ψ profile route remains noncanonical under the current Axiom C, but now has a local free-embedding completion candidate: `research_tracks/T1_GR/free_fiber_completion/gap_10r_free_fiber_embedding_completion.tex`. Its companion `gap_10s_covariant_profile_geometry.tex` supplies a flat pairing-compatible ambient derivative, proves induced Levi--Civita/Gauss geometry and preserves free rank under profile-frame changes. It also separates the central complex symmetric channel `gamma=g+ih` from the antisymmetric biquaternionic bivector. Profile selection, canonical connection notation, canonical-action origin, holomorphic/Jacobi restrictions, matter separation, and global completion remain open.
