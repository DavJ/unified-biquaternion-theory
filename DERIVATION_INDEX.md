...
| Gap H1 via radiative Hosotani: V(θ_W) one-loop | **Partially Proved [L1]** — v66: N_eff=8 at n_R=138 (even winding → bosonic, all 8 DOF of ℂ⊗ℍ bosonic, N_fer=0) [L1]; SSB at θ_W=π/2 confirmed [L1]; λ numerical estimate [Open estimate] | `research_tracks/research/hosotani_higgs.tex §3.3,§6`; `tools/compute_hosotani_lambda.py`; `research_tracks/research/higgs_yukawa_scan.md §2.4` | v66 §3.3: KK parity rule (fermionic_statistics_bridge.tex): even n→bosonic, odd n→fermionic; n_R=138 (even)→N_b=8 (dim_ℝ(ℂ⊗ℍ)=8), N_f=0; N_eff=N_b-4N_f=8>0 [L1]; SSB confirmed; λ_5D≈-6.8×10⁻⁷ GeV³ (from eq.lambda_formula with N_eff=8, g=0.65, ζ(3)=1.202, R_ψ=14.18 GeV⁻¹); 5D→4D reduction pending; see tools/compute_hosotani_lambda.py |
