...
| B_base = N_eff^{3/2} = 41.57 | **Motivated Conjecture** [L0] | `STATUS_ALPHA.md §9`, `consolidation_project/appendix_ALPHA_one_loop_biquat.tex §B.3`, `tools/compute_B_KK_sum.py`, `consolidation_project/alpha_derivation/b_base_spinor_approach.tex`, `consolidation_project/alpha_derivation/b_base_hausdorff.tex`, `tools/explore_b_exponent.py` | Dead ends: KK sum, zeta, gauge orbit volume. New v56 approaches: A1 (spinor unitarity counting → N_eff^{3/2} heuristic, [MOTIVATED CONJECTURE]); A2 (Hausdorff: exponent = dim_ℝ(Im ℍ)/2 = 3/2, zero free params, [MOTIVATED CONJECTURE] — most structurally motivated); A3 (numerical: x = log(46.284)/log(12) = 1.543, nearest fraction 17/11, deviation from 3/2 entirely explained by R ≠ 1) |
