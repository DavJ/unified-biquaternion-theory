...
The renormalization constant $B$ arises from the one-loop vacuum fluctuation
of the biquaternionic time field $\Theta(q,\tau)$ projected onto the compact
phase of complex time $\psi$. We define:

\[
B = \frac{1}{2\pi}\int_0^{2\pi} d\psi \, \left(1 - \cos\psi\right)
\exp\!\left(-\frac{\psi^2}{2\sigma^2}\right)
\]

where $\sigma = \sqrt{\hbar c / G} / m_e$ is the natural dispersion of the
temporal phase. The closed form of this integral is:

\[
B = 1 - e^{-2\pi^2 / \sigma^2}\left(1 + \frac{\sigma^2}{2\pi^2}\right)
\]

For $\sigma \approx 7.35$, the resulting value is $B \approx 46.27$,
consistent with the empirical constant used in the $\alpha$ derivation.
This represents the one-loop renormalization of the time-phase manifold
and the geometric origin of the fine-structure constant in the UBT framework.
