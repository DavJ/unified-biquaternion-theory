...
where $\mathcal{G}_{\mu\nu}$ is the fundamental biquaternionic metric and $g_{\mu\nu}$ is the projected metric observed in the physical (GR) sector.

% ============================================================
% Projection operator P_* and allowed permutations (UBT core)
% ============================================================

\subsection{Projection operator $P_\star$ and allowed permutations}

\paragraph{Scope note.}
This subsection formalizes the projection-selection logic that maps the fundamental biquaternionic metric $\mathcal{G}_{\mu\nu}$ onto the observed 4D metric $g_{\mu\nu}$.
The projection $P_\star$ is selected by an extremal integrity principle and remains fixed (superselection) under local field variations.
This formalism does not yet provide a complete dynamical theory of projection evolution $P(t,x)$; such extensions would require additional theoretical development beyond the scope of this appendix.

\paragraph{Biquaternionic degrees of freedom.}
Let $\mathbb{B} \cong \mathbb{C}\otimes\mathbb{H}$ denote the biquaternion algebra.
Any field-valued geometric object $\mathcal{X}$ (e.g.\ tetrad $E_\mu$, metric $\mathcal{G}_{\mu\nu}$, curvature $\mathcal{R}_{\mu\nu}$)
takes values in $\mathbb{B}$ and can be expanded in an 8-real-dimensional basis.

\paragraph{Real-sector (GR) projection.}
We define a general \emph{projection functional} $P$ acting on $\mathbb{B}$-valued objects by selecting a real 4D observational slice.
For the metric sector this induces
\begin{equation}
g^{(P)}_{\mu\nu} \;:=\; \Re\!\Big(P\big[\mathcal{G}_{\mu\nu}\big]\Big).
\label{eq:gP_def}
\end{equation}
In particular, the default observational metric corresponds to $P=\mathrm{Id}$,
so that $g_{\mu\nu} := \Re(\mathcal{G}_{\mu\nu})$.

\paragraph{Automorphisms vs.\ physical channel-switching.}
We distinguish two classes of transformations on $\mathbb{B}$:

\begin{itemize}
\item \textbf{(A) Allowed automorphisms (gauge/basis changes):}
$\;\varphi\in\mathrm{Aut}(\mathbb{B})$ such that $\varphi$ preserves the involution and the scalar part:
\begin{equation}
\varphi(x^\dagger)=\varphi(x)^\dagger,\qquad \mathrm{Sc}(\varphi(x))=\mathrm{Sc}(x),
\qquad \forall x\in\mathbb{B}.
\label{eq:aut_conditions}
\end{equation}
These transformations represent internal relabelings (changes of biquaternionic basis)
and therefore do \emph{not} change the physical GR-sector metric extracted by \eqref{eq:gP_def},
up to ordinary diffeomorphisms.

\item \textbf{(B) Channel permutations (multiplex sectors):}
$\;\pi$ denotes a \emph{component permutation} (including possible scalar--vector reshuffling)
acting on the 8 real components of a chosen representation of $\mathbb{B}$.
In general, $\pi\notin\mathrm{Aut}(\mathbb{B})$ and therefore corresponds to a \emph{physically distinct projection choice}.
Such permutations define alternative channels
\begin{equation}
P_\pi \;:=\; P\circ \pi,
\qquad
g^{(\pi)}_{\mu\nu} \;:=\; \Re\!\Big(P_\pi[\mathcal{G}_{\mu\nu}]\Big).
\label{eq:pi_channel_def}
\end{equation}
\end{itemize}

\paragraph{Allowed permutations.}
A permutation $\pi$ is declared \emph{allowed} (i.e.\ a dynamically viable multiplex channel)
iff the induced 4D metric $g^{(\pi)}_{\mu\nu}$ satisfies all of the following constraints:

\begin{enumerate}
\item \textbf{Lorentzian signature constraint:}
Lorentz invariance in the observed sector is not imposed as an independent axiom but emerges as a stability requirement selecting the admissible projection $P_\star$.
\begin{equation}
\mathrm{sign}\big(g^{(\pi)}_{\mu\nu}\big) = (+,-,-,-)
\quad\text{(or the chosen convention)},
\qquad
\det g^{(\pi)} \neq 0.
\label{eq:lorentz_constraint}
\end{equation}

\item \textbf{Compatibility constraint (projected metric from tetrads):}
Let the fundamental tetrads satisfy $\mathcal{G}_{\mu\nu}=\mathrm{Sc}(E_\mu E_\nu^\dagger)$.
Then the projected metric must be reproducible as a real-sector slice of this construction:
\begin{equation}
g^{(\pi)}_{\mu\nu}
=
\Re\!\Big(
P_\pi\big[\mathrm{Sc}(E_\mu E_\nu^\dagger)\big]
\Big),
\label{eq:proj_from_tetrad}
\end{equation}
ensuring that the channel does not introduce an ad-hoc metric unrelated to the biquaternionic geometry.

\item \textbf{Stability / integrity constraint (self-repair / ECC principle):}
Define an \emph{integrity functional} $\mathcal{I}[P]$ that penalizes projection choices which generate
nonphysical instabilities (e.g.\ excessive imaginary-norm leakage, curvature blow-up, or loss of hyperbolicity).
A minimal generic choice is
\begin{equation}
\mathcal{I}[P]
:= \int d^4x\,\sqrt{|g^{(P)}|}\;
\Big(
\alpha\,\|\Im(P[\mathcal{G}])\|^2
+\beta\,\|\Im(P[\mathcal{T}])\|^2
+\gamma\,\|\Im(P[\mathcal{R}])\|^2
\Big),
\label{eq:integrity_functional}
\end{equation}
with nonnegative weights $\alpha,\beta,\gamma$ and any suitable norm $\|\cdot\|$ on components.
Then $\pi$ is allowed only if it is \emph{near-minimizing}:
\begin{equation}
\mathcal{I}[P_\pi] \le \mathcal{I}[P_{\mathrm{Id}}] + \varepsilon,
\label{eq:allowed_pi_integrity}
\end{equation}
for a small tolerance $\varepsilon$ that sets the admissible multiplex bandwidth.
\end{enumerate}

\paragraph{The selected projection $P_\star$.}
We define the \emph{physically realized} projection as the integrity-minimizing choice within the allowed class:
\begin{equation}
P_\star \;:=\; \arg\min_{P\in\mathcal{P}_{\mathrm{adm}}}\; \mathcal{I}[P],
\qquad
\mathcal{P}_{\mathrm{adm}} := \{\,P_\pi \;|\; \pi \text{ satisfies } \eqref{eq:lorentz_constraint}--\eqref{eq:allowed_pi_integrity}\,\}.
\label{eq:Pstar_def}
\end{equation}
The observed spacetime metric is then
\begin{equation}
g_{\mu\nu} \;:=\; g^{(P_\star)}_{\mu\nu} \;=\; \Re\!\Big(P_\star[\mathcal{G}_{\mu\nu}]\Big).
\label{eq:observed_metric_Pstar}
\end{equation}

\paragraph{Superselection of the projection.}
The projection $P_\star$ is treated as a superselection choice fixed by global stability and is not a dynamical field.
Local fluctuations of $\mathcal{G}$, $\Theta$, $\mathcal{T}$ or $\mathcal{R}$ do not induce transitions between distinct projections $P_\pi$.
Any hypothetical ``projection switching'' mechanism (if considered) must be modeled by an explicit additional dynamical sector and is outside the scope of this appendix.

\paragraph{Why this removes projection underdetermination.}
The projection operator $P_\star$ is not arbitrary: it is uniquely selected by the extremal principle of minimizing the integrity functional $\mathcal{I}[P]$ within the class of admissible projections. The admissibility constraints—Lorentzian signature, tetrad compatibility, and integrity stability—ensure that $P_\star$ corresponds to a physically viable observational channel. This variational selection mechanism eliminates the freedom in choosing projections and anchors the theory to empirical reality.

\paragraph{Remark (multiplex gravitational leakage).}
If the fundamental dynamics is defined at the biquaternionic level (e.g.\ $\mathcal{G}=\kappa\mathcal{T}$),
then sectors with different allowed $\pi$ may remain electromagnetically decoupled
while potentially contributing to the same underlying biquaternionic invariants entering $\mathcal{I}[P]$.
This suggests a possible mechanism for gravitational ``leakage'' across channels without arbitrary mixing.
However, quantitative galaxy-scale predictions (e.g., rotation curves, lensing profiles) are not yet derived.
...
where:
\begin{itemize}
\item $g_{\mu\nu} = \text{Re}(\mathcal{G}_{\mu\nu})$ is the projected/observed metric
...
\begin{equation}
\mathcal{L}_{\text{dark}} = \mathcal{L}_{\text{dark}}[\text{Im}(\mathcal{G}_{\mu\nu}), \text{QIm}(\mathcal{G}_{\mu\nu}), \dots]
\end{equation}
...
\textbf{Note:} These are tentative suggestions; quantitative predictions for galaxy-scale phenomena (rotation curves, lensing profiles) have not yet been computed from first principles.
...
If dark matter couples to imaginary metric components, it may exhibit:
\begin{itemize}
\item No electromagnetic interactions (doesn't couple to $A_{\mu}$)
\item Gravitational interactions (couples to $g_{\mu\nu}$)
\item Novel self-interactions through $\text{Im}(\mathcal{G}_{\mu\nu})$
\end{itemize}

This could potentially contribute to an effective gravitational source term that might address small-scale structure anomalies (core-cusp problem, etc.).
However, explicit rotation curve and lensing calculations from UBT first principles remain to be performed.
...
A critical constraint on multiverse theories is to prevent \textbf{arbitrary mixing} between physically distinct sectors. If sectors (channels) with different allowed $\pi$ could interact gravitationally in an unconstrained manner, this would:
\begin{itemize}
...
\begin{itemize}
\item \textbf{Direct metric coupling}: $g_{\mu\nu}^{(\pi_1)} h^{\mu\nu}_{(\pi_2)}$ for $\pi_1 \neq \pi_2$ (violates gauge invariance)
\item \textbf{Matter transfer}: $T_{\mu\nu}^{(\pi_1)} \to T_{\mu\nu}^{(\pi_2)}$ (violates energy conservation)
\item \textbf{Arbitrary phase mixing}: $\Theta_{\pi_1} \Theta_{\pi_2}^\dagger$ without physical justification (violates automorphism structure)
\end{itemize}
...
\begin{equation}
\langle \mathcal{T}_{\mu\nu} \rangle_{\text{total}} = \sum_{\pi \in \mathcal{P}_{\text{adm}}} \omega_\pi \langle T_{\mu\nu}^{(\pi)} \rangle
\end{equation}
where $\omega_\pi$ are weight factors determined by the integrity functional and $\mathcal{P}_{\text{adm}}$ is the set of admissible projections.
...
\begin{equation}
\langle \mathcal{O} \rangle = \sum_\pi |\psi_\pi|^2 \langle \mathcal{O} \rangle_\pi + \sum_{\pi_1 \neq \pi_2} \psi_{\pi_1}^* \psi_{\pi_2} \langle \mathcal{O} \rangle_{\pi_1\pi_2}
\end{equation}

However, decoherence rapidly suppresses off-diagonal terms $\langle \mathcal{O} \rangle_{\pi_1\pi_2}$ for macroscopically distinct sectors.

\paragraph{3. Topological Winding Contributions}

If the biquaternionic manifold has non-trivial topology, sectors may be connected by topological invariants. Using differential form notation:
\begin{equation}
W[\mathcal{G}] = \int_{\mathbb{B}^4} \text{Tr}(\mathcal{R} \wedge \mathcal{R})
\end{equation}
where $\mathcal{R}$ is the biquaternionic curvature 2-form and the trace is over gauge indices.
...
This is automatically satisfied in UBT if cross-sector effects arise only through shared biquaternionic invariants, since these are suppressed by the imaginary component scales or through the integrity functional minimization.

\paragraph{Connection to $P_\star$ Selection}

The integrity functional $\mathcal{I}[P]$ defined in Eq.~\eqref{eq:integrity_functional} already incorporates constraints on cross-sector leakage through the norm terms on imaginary parts. The minimization principle that selects $P_\star$ thus automatically minimizes unphysical cross-sector mixing while allowing the controlled gravitational leakage described above.
...
\begin{enumerate}
\item \textbf{Structure:} $\mathbb{B}^4$ (32D) projects to $M^4$ (4D) via operator $P_\star$
\item \textbf{Formalization:} Projection operator $P_\star$ selected by extremal principle minimizing integrity functional
\item \textbf{Automorphisms vs. Sectors:} Clear distinction between gauge-equivalent transformations and physically distinct channels
\item \textbf{Cross-Sector Constraints:} Gravitational effects between sectors mediated only through shared biquaternionic invariants; arbitrary mixing prohibited
...
\item Complete enumeration of all physically distinct allowed permutations $\pi$
\end{itemize}
