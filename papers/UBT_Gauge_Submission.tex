...
The colour-charge lattice theorem (Gap~C2) is proved [L1] via the
topological U(1)$_{\rm EM}$ integrality condition on $S^1_\psi$,
yielding all Standard Model hypercharge assignments from first principles.
A conditional derivation of $\sin^2\theta_W=3/8$ at the unification
scale follows from generator norm ratios [L1], with renormalization
group running to $\sin^2\theta_W(M_Z)\approx 0.231$ [STD].
The SU(2) Scherk--Schwarz twist $\Theta(\psi+2\pi R)=\sigma_3\Theta\sigma_3$
yields $N_{\mathrm{eff}}=12$ \Lone\ for the charged off-diagonal components,
giving $B_0=8\pi$ \Lone\ and strengthening the alpha-track candidate formula.
\end{abstract}
...
connecting the winding partition function to Ramanujan's CM value.}\footnote{An SU(2) Scherk--Schwarz twist $\Theta(\psi+2\pi R_\psi)=\sigma_3\Theta\sigma_3$ yields $N_{\rm eff}=12$ for the charged off-diagonal components, giving $B_0=8\pi$ [L1]; see \texttt{research\_tracks/quantum\_ubt/su2\_twist\_neff12.tex}.}
...
See \texttt{DERIVATION\_INDEX.md \S psi-modes} and \texttt{canonical/su3\_derivation/}.
\end{proof}
...
\medskip
\textbf{Update 2026-05-12}: The EW1+RG route is upgraded from
\emph{dead end} to \emph{conditional [L1]}. Generator norm ratios
give $\sin^2\theta_W^{\rm GUT}=3/8$ [L1] given the first-principles
hypercharge assignments (Gap~C2, [L1]); see
\texttt{research\_tracks/EW/weinberg\_angle\_ew1\_rg.tex}.
$N_{\mathrm{eff}}=12$ \Lone\ via the SU(2) Scherk--Schwarz twist
$\Theta(\psi+2\pi R)=\sigma_3\Theta\sigma_3$ strengthens the EW1+RG route.
Full $M_Z$ running remains conditional on $R_\psi$ scale closure
