...
satisfies the Regge-Wheeler equation, and the even-parity graviton satisfies
the Zerilli equation (GAP-Z closed; see \texttt{canonical/gr\_closure/zerilli\_derivation.tex}).
...
The even-parity (Zerilli) equation has been derived subsequently in a companion file and will appear in a follow-up note.
\end{abstract}
...
\begin{tcolorbox}[colback=green!4!white,colframe=green!55!black,
  title=GAP-Z: Zerilli Equation\ \PROVED\ [\Lone]]
\textbf{Proved}: Both polarisation sectors are now derived from linearised UBT.

\smallskip
\textbf{Odd-parity} (Regge-Wheeler): Theorem~\ref{thm:rw} (this paper).

\smallskip
\textbf{Even-parity} (Zerilli): The Zerilli equation
\[
  \left[\frac{\mathrm{d}^2}{\mathrm{d}r_*^2} + \omega^2 - V_\mathrm{Z}(r)\right]
  \Psi_\mathrm{Z} = 0
\]
is derived via the Chandrasekhar two-potential transformation applied to
the even-parity ($P_\psi$-even) linearised UBT field equation.
See \texttt{canonical/gr\_closure/zerilli\_derivation.tex} (2026-05-10).

\textbf{Isospectrality}: The Chandrasekhar-Detweiler isospectrality theorem
(same QNM spectrum for both polarisations) holds by the Darboux relation
...
7a & Zerilli equation (even-parity) & Proved \Lone & GAP-Z, \S\ref{sec:gapz} \\
