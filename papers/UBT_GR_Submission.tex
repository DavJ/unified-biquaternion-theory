...
\title{\textbf{Induced Lorentzian Metrics from Biquaternion-Valued Fields:\\
       ...
       and the Variational Obstruction}}
...
% ---- Abstract ----------------------------------------------------------------
\begin{abstract}
We construct a candidate spacetime metric as the real-valued bilinear
$g_{\mu\nu} = \langle\partial_\mu\Theta,\partial_\nu\Theta\rangle_\eta/\calN$
of a biquaternion-valued field $\Theta(q,\tau)$ over complexified time
$\tau = t + i\psi$, and analyse the extent to which the dynamics of the
Unified Biquaternion Theory (UBT) field equation
$\nabla^\dagger\nabla\Theta = \kappa\,\mathcal{T}$ reproduces general
relativity. We prove: (i) $g_{\mu\nu}[\Theta]$ is a symmetric covariant
tensor (transformation law derived, not assumed); (ii) for admissible
fields --- those satisfying a non-degeneracy condition and a Lorentzian
real-sector projection --- the induced metric has signature $(-,+,+,+)$
(conditional signature theorem); (iii) the Levi-Civita connection,
curvature, and, upon adjoining the Einstein--Hilbert term for the induced
metric, the Einstein field equations follow by standard variational
arguments. We identify precisely the variational obstruction (GAP-10) to
deriving the Einstein equations from the $\Theta$-dynamics alone: since
$g = g[\Theta]$, stationarity under the induced variations
$\delta g[\delta\Theta]$ has not been shown to imply the full Einstein
equations, and this equivalence remains the central open problem of the
programme. A spherically symmetric ansatz $\Theta_0$ reproduces the
spatial Schwarzschild metric in isotropic coordinates,
$g_{ij} = \Psi^4\delta_{ij}$, verified numerically to relative error
$< 10^{-15}$; the temporal component $g_{tt} = -\Phi^2$ follows from the
complex-time relation $\partial_\psi\Theta_0 = i\Phi\Theta_0$, whose
dynamical derivation from the field equation remains open (GAP-U2).
Linearised odd- and even-parity perturbations reduce to the
Regge--Wheeler and Zerilli equations conditional on the identification
$\delta(\nabla^\dagger\nabla\Theta) \to \delta G_{\mu\nu}$, which is
stated as an explicit bridge assumption. We situate the construction
within the Hermitian-gravity and biquaternion literature and state all
assumptions, conventions, and open gaps explicitly.
\end{abstract}
...
\begin{enumerate}
  \item \textbf{Induced-metric construction.}
        The candidate metric $g_{\mu\nu}$ is defined as a real-valued bilinear of
        $\partial_\mu\Theta$; its tensor transformation law is \emph{proved}, not
        assumed (Theorem~\ref{thm:metric}).
        Whether the UBT dynamics selects Einstein dynamics for this metric is the
        identified open problem (GAP-10).
  ...
  \item \textbf{Five-step chain with an identified obstruction.}
        The chain
        $\Theta \to g \to \Gamma \to R \to G_{\mu\nu} = 8\pi G\,T_{\mu\nu}$
        is constructed explicitly; steps~1--4 are proved, and step~5 holds upon
        adjoining the Einstein--Hilbert action for the induced metric.
        The equivalence of this variation with variation over $\Theta$ alone is
        open (GAP-10) and is stated as such.

  \item \textbf{No additional fit parameters.}
        After fixing Newton's constant $G$, the Lorentzian projection, the
        bilinear form, and the normalisation convention $\calN$, the
        Schwarzschild reconstruction introduces no further adjustable parameters.
...
Einstein equations & Variational, with identified obstruction (GAP-10) [\Lone\ cond.] & --- & Einstein--Cartan, first-order \\
Complexified time $\tau=t+i\psi$ & Yes (compact $\psi$) & No & No \\
Schwarzschild recovery & Spatial verified $<10^{-15}$; temporal conditional (GAP-U2) & --- & PG form demonstrated \\
...
Theorems~\ref{thm:metric}--\ref{thm:rw} hold as stated with
their explicit hypotheses. GAP-10 directly conditions the \emph{interpretation}
of the main result: without control of the image of the variational map
$\delta\Theta \mapsto \delta g_{\mu\nu}[\delta\Theta]$, stationarity of the
total action under available variations has not been shown to imply the
full Einstein equations. GAP-U2 conditions the temporal Schwarzschild
sector; GAP-B conditions the perturbation reductions.
...
This is a classical variational-equivalence question, prior
to any quantum considerations.
\end{tcolorbox}

\begin{tcolorbox}[colback=red!4!white,colframe=red!55!black,
  title={GAP-U2: Temporal Schwarzschild Closure\ \OPEN},breakable]

\paragraph{GAP-U2: Temporal Schwarzschild closure.}
The relation $\partial_\psi\Theta_0 = i\Phi(r)\Theta_0$ produces
$g_{tt} = -\Phi(r)^2$. A phase-based realization is excluded: it is
inconsistent with the flat-background field equation and, on compact
$\psi$, with single-valuedness (see
\texttt{research\_tracks/gap\_u2/gap\_u2\_lapse\_mechanism.md}). A
connection-based mechanism has been identified and verified exactly:
with $D_\psi\Theta = \partial_\psi\Theta + iqA_\psi\Theta$, the vacuum
field equation for the $\mathrm{U}(1)_\psi$ connection on the induced
spatial metric $g_{ij} = \Psi^4\delta_{ij}$ is
$\partial_r(r^2\Psi^2 A_\psi') = 0$, whose solution with
$A_\psi(\infty) = 1$ and charge $M$ is exactly
$A_\psi = 1 - M/(r+M/2) \equiv \Phi(r)$ (verified symbolically,
\texttt{tools/verify\_gap\_u2\_lapse.py}). Closure of the remaining
normalization condition $q^2|\Theta_0|^2/\calN = 1$ reduces to the
bilinear specification of GAP-U1; the gap therefore remains OPEN
pending GAP-U1.
\end{tcolorbox}

\begin{tcolorbox}[colback=red!4!white,colframe=red!55!black,
  title={GAP-B: Perturbation Bridge\ \OPEN},breakable]

\paragraph{GAP-B: Perturbation bridge.}
The reduction of linearised UBT perturbations to the Regge--Wheeler
and Zerilli equations uses the identification
$\delta(\nabla^\dagger\nabla\Theta) \to \delta G_{\mu\nu}$.
This bridge has not been derived from the linearised UBT dynamics;
it is stated as an explicit assumption.
Deriving it from the first principles of the field equation is an open problem.
\end{tcolorbox}
...
\S\ref{sec:gaps}; GAP-U2 and GAP-B additionally condition the temporal
Schwarzschild and perturbation sectors respectively (see \S\ref{sec:gaps}).
...
\[
  ...
  G_{\mu\nu} = 8\pi G\,T_{\mu\nu}
\]
is complete with an identified obstruction: steps~1--4 are proved at the [L1]
level; step~5 holds upon adjoining the Einstein--Hilbert action for the induced
metric (\Lone\ conditional on GAP-10 for the $\Theta$-only formulation).
UBT contains standard GR as a derivable sector: the metric and Lorentzian
signature are derived, not postulated; the spatial Schwarzschild metric is
reproduced to floating-point precision; and the Regge-Wheeler equation follows
from the perturbation chain, conditional on the bridge assumption GAP-B.
...
\begin{tcolorbox}[colback=blue!3!white,colframe=blue!55!black,
  title={T1\_GR Proof Status: Steps 1--4 Proved; Steps 5, 6c, 7a Conditional}]
\begin{center}
...
5 & Einstein eqs $G_{\mu\nu}=8\pi GT_{\mu\nu}$ & \Lone\ cond.\ (GAP-10 for $\Theta$-only) & step3\_einstein\_with\_matter.tex \\
