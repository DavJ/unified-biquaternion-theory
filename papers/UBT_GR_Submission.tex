...
Regge--Wheeler and Zerilli reductions no longer rest on an assumed bridge:
linearising the fiber-closure equation around a fiber-free vacuum background
yields $\delta G_{\mu\nu}=0$ for the induced perturbation, so the reductions
are conditional only on the fiber-free Schwarzschild representer
(GAP-B subsumed by GAP-10R).
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Regge--Wheeler and Zerilli reductions (conditional).}
        ...
        Regge--Wheeler and Zerilli equations, conditional only on the existence of a fiber-free stationary
        representation of the background: the former bridge identification is
        now \emph{derived} by linearising the fiber-closure equation
        (\texttt{canonical/gr\_closure/linearised\_fiber\_closure.tex});
        GAP-B is thereby subsumed by GAP-10R.
\end{enumerate}
...
conditional on GAP-B, now derived from the linearised fiber closure
(conditional on the fiber-free Schwarzschild representer, GAP-10R).
GAP-Z is therefore at level [L1 cond.\ given GAP-B$\to$GAP-10R].
\end{tcolorbox}
...
Schwarzschild sector by the canonical $\Theta$ equation; GAP-B is conditionally resolved: the bridge is derived from the linearised
fiber closure, conditional on the fiber-free Schwarzschild representer
...
\paragraph{GAP-B: Perturbation bridge --- CLOSED CONDITIONALLY.}
Linearising the exact pure-$\Theta$ vacuum equation
$\nabla_\nu(G^{\mu\nu}E_\mu)=0$ around a regular, fiber-free, stationary
vacuum background gives, using $\bar G_{\mu\nu}=0$, the linearised Bianchi
identity, and the Gauss step, the normal equation
$\delta G^{\mu\nu}\bar B_{\mu\nu}=0$; injectivity of the closure map at a
fiber-free point yields $\delta G_{\mu\nu}=0$ for the induced perturbation
(\texttt{canonical/gr\_closure/linearised\_fiber\_closure.tex}).  The
Regge--Wheeler and Zerilli equations follow by the standard harmonic
decomposition.  The single remaining condition is the fiber-free
Schwarzschild representer (the GAP-10R/GAP-U2$\Theta$ instance); GAP-B
carries no independent logical weight.
\end{tcolorbox}
...
\begin{itemize}
\item Even-parity (Zerilli) graviton equation derived [L1 cond.\ given GAP-B$\to$GAP-10R]
  (\texttt{canonical/gr\_closure/zerilli\_derivation.tex}).
...
chain remains conditional on GAP-B, now subsumed by GAP-10R.
...
6c & Regge-Wheeler equation & \Lone\ cond.\ (GAP-B$\to$GAP-10R) & linearised GR chain \\
7a & Zerilli equation (even-parity) & \Lone\ cond.\ (GAP-B$\to$GAP-10R) & \texttt{zerilli\_derivation.tex} \\
