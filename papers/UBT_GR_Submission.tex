...
\begin{align}
  ...
  \calN &:= -\langle \partial_0\Theta,\, \partial_0\Theta \rangle_\eta \,,
  \quad \text{where } \langle A, B\rangle_\eta
  := \mathrm{Re}[\mathrm{Tr}(A\,\eta_{\mathcal{B}}\,B^\dagger)]\,, \\
  g_{\mu\nu} &:= \frac{\re[\Tr(\calG_{\mu\nu})]}{\calN}.
\end{align}
Here $\eta_{\mathcal{B}} = \mathrm{diag}(-1,+1,+1,+1)$ is the
Minkowski metric operator embedded in $\mathcal{B}$, inducing
an \emph{indefinite} inner product on the timelike sector.
...
so $\calN > 0$ and $g_{00} = -1 < 0$ consistently.
\end{definition}
