...
$g_{tt} = -\Phi(r)^2$ but has not been derived from the UBT field
equation $\nabla^\dagger\nabla\Theta = 0$ and appropriate boundary
conditions; the verification script \texttt{tools/verify\_schwarzschild\_theta.py}
reports this sector \textbf{OPEN}.
The spatial Schwarzschild metric $g_{ij} = \Psi^4\delta_{ij}$ is unaffected.
\end{tcolorbox}
...
GAP-U1 & Uniqueness of metric bilinear (alt.\ forms of $\langle\cdot,\cdot\rangle_\eta$); see \texttt{reviews/external\_review\_2026-07\_gpt.md} & Medium \\
\bottomrule
\end{tabular}
\end{center}
