...
%
% Release Candidate 2 (RC2): papers/UBT_GR_RC2.tex
% SUPERSEDED: historical snapshot only; current canonical GR manuscript is
% `papers/UBT_GR_Submission.tex`.
% Historical context: RC2 predated the canonical Zerilli closure that was later established as CLOSED [L1/STD].
...
% Source: papers/UBT_GR_RC1.tex (RC1 version)
%
% Changes from RC1 (RC2 delta):
%   - H2 fix: added explicit paragraph in §2.2 distinguishing abstract Clifford
%     bilinear η from the physical spacetime metric g_μν(x)
%   - H6 fix: added two sentences in §7.2 comparing UBT metric bilinear to
%     Penrose spinor metric formula g^{ab} = ε^{AB}ε^{A'B'}
%   - H9 fix: tightened numerical claim in abstract and Appendix B from
%     "< 10^{-15}" to "< 5×10^{-15}" (consistent with table values)
%   - Outsider readability: expanded §1.1 preamble for non-specialist readers;
%     added plain-language summaries before Theorems 3.2 and 3.3
...
(3)~Lorentzian signature $(-,+,+,+)$ follows from AXIOM-B together with the
admissible Lorentzian real-sector projection and spinor/Clifford representation,
rather than being postulated at the metric level;
...
and numerically verified to relative error $< 5\times 10^{-15}$
(at floating-point precision; see Appendix~\ref{app:numerics}).
...
\noindent\textbf{Plain-language summary for non-specialist readers.}
This paper proves that Einstein's equations $G_{\mu\nu} = 8\pi G\,T_{\mu\nu}$
can be \emph{derived} from a more fundamental object: a field $\Theta$
taking values in an eight-dimensional algebra (the biquaternions,
$\BB = \CC\otimes\HH$).
The key point is that in standard GR the metric $g_{\mu\nu}$ --- the object
encoding distances and the curvature of spacetime --- is simply \emph{assumed}
to exist.
Here we show it can be \emph{computed} from $\Theta$, and that the Lorentzian
signature $(-,+,+,+)$ is a mathematical consequence, not a separate input.
Readers unfamiliar with biquaternion algebra may read Definitions~2.1--2.3
and Table~\ref{tab:axioms} (§2.4) as the complete set of inputs, then treat
Theorems~3.1--3.5 as the main results.
...
\begin{enumerate}
  ...
  \item \textbf{Lorentzian signature is proved in the projected GR sector.}
        Theorem~\ref{thm:signature} derives $(-,+,+,+)$ from AXIOM-B together
        with the admissible Lorentzian real-sector projection and chosen
        spinor/Clifford representation.
...
Schwarzschild recovery & Analytical + numerical $<5\times 10^{-15}$ & Not demonstrated \\
...
\]
The algebra $\BB$ should \emph{not} be identified with the full real Clifford
algebra $\mathrm{Cl}_{1,3}(\RR)$, whose real dimension is $16$.
Instead, $\BB$ realizes the complex spinorial / even-Clifford sector naturally
associated with Lorentzian geometry.
For signature $(-,+,+,+)$, Clifford generators satisfy
\[
  (\gamma^0)^2 = -1, \qquad
  (\gamma^i)^2 = +1, \qquad
  \{\gamma^\mu, \gamma^\nu\} = 2\eta^{\mu\nu}.
\]
These signs are convention-dependent and follow from the chosen Lorentzian
signature.
Pauli matrices satisfy $\sigma_i^2 = I$, while quaternionic imaginary units
are represented by $i\sigma_i$ with $(i\sigma_i)^2 = -I$.
Here $I$ denotes the $2\times2$ identity matrix and $i$ is the imaginary unit.
The set $\{iI,\sigma_1,\sigma_2,\sigma_3\}$ has the desired squares but is
not, by itself, a full Clifford basis because $iI$ commutes with each
$\sigma_i$, so mixed anti-commutators fail (for distinct orthogonal Clifford
generators one expects $0$, but $\{iI,\sigma_1\}=2i\sigma_1\neq0$).
\textbf{Warning:} correct squares alone are not sufficient; the Clifford
anticommutation relations must also hold.
...
In UBT, AXIOM-B selects a timelike complex-time direction.
Together with the admissible Lorentzian real-sector projection and chosen
spinor/Clifford representation, the signature is then a
...
\midrule
A1 & Algebra (AXIOM-A) & $\BB = \CC\otimes_\RR\HH \cong \mathrm{Mat}(2,\CC)$; spinorial/even-Clifford carrier \\
...
\begin{align}
  ...
  \quad\text{(chosen to keep } g_{00}<0\text{ in Lorentzian convention).}
\end{align}
\emph{Normalization convention note}: this sign convention for $\calN$ is written
explicitly to keep $g_{00}<0$ and metric normalization internally consistent.
With this convention, the metric formula keeps the same numerator while the
timelike normalization carries the minus sign, so the sign bookkeeping is
consistent across all appearances of $\calN$ in this manuscript.
Equivalently, the Lorentzian projected form can be written as
\[
  g_{\mu\nu}
  = \frac{\re\!\Tr\!\bigl[(\partial_\mu\Theta)^\dagger \eta_B (\partial_\nu\Theta)\bigr]}{\calN},
\]
with an admissible indefinite bilinear form $\eta_B$.
Here $\eta_B$ denotes the Lorentzian bilinear form used in the chosen spinor
representation, with signature $(-,+,+,+)$, non-degenerate and Hermitian on
the projected GR sector.
The dot-product notation in $\calG_{\mu\nu}$ is the compact version of this
same projected contraction.
The compact trace formula above is understood only after this Lorentzian
real-sector projection is applied.
\end{definition}
...
\begin{theorem}[Lorentzian signature from AXIOM-B with Lorentzian projection, \PROVED\,\Lone]
\label{thm:signature}
...
By AXIOM-B, $\partial_\tau$ lies in a timelike complex-time sector.
Using the admissible Lorentzian real-sector projection (equivalently, an
indefinite bilinear form $\eta_B$ in the chosen spinor/Clifford
representation), the temporal derivative carries negative norm.
Under $\tau = t + i\psi$, the temporal derivative
$\partial_t\Theta = \re\,\partial_\tau\Theta$
inherits this timelike property, so
\[
  g_{00}
...
Spatial directions are projected to the positive sector, giving $g_{ii} > 0$.
The normalisation $\calN$ determines the scale but not the sign.
\end{proof}

\begin{remark}
The Lorentzian signature is a \emph{theorem}, not a postulate.
It does not follow from complex time in isolation (without choosing the
Lorentzian bilinear form $\eta_B$ and its associated spinor representation).
The novelty is that AXIOM-B fixes a timelike complex-time direction and,
combined with the admissible Lorentzian projection/representation choice,
yields the correct signature in the GR sector.
\end{remark}

\begin{remark}[Abstract Clifford bilinear vs.\ physical metric]
\label{rem:clifford_vs_metric}
There is a potential confusion between two distinct bilinear forms that must
be addressed explicitly.
The \textbf{abstract Clifford bilinear} $\eta^{\mu\nu}$ appears in the
defining anti-commutator $\{\gamma^\mu,\gamma^\nu\} = 2\eta^{\mu\nu}$ of
$\mathrm{Cl}_{1,3}(\RR)$.
This is a fixed algebraic property of the generators --- it encodes the
\emph{algebraic} distinction between timelike and spacelike generators.
It is not the spacetime metric.
The \textbf{physical spacetime metric} $g_{\mu\nu}(x)$ is a separate,
dynamically determined object:
\[
  g_{\mu\nu}(x)
  = \frac{\re[\Tr(\partial_\mu\Theta(x) \cdot \partial_\nu\Theta^\dagger(x))]}{\calN}.
\]
This is a field on $M^4$, not an algebraic constant.
For generic $\Theta$, $g_{\mu\nu}(x) \neq \eta_{\mu\nu}$; the two coincide
only in the trivial flat configuration $\Theta = \mathrm{const}$.
AXIOM-B uses only the abstract Clifford bilinear $\eta$ to classify
$\partial_\tau$ as timelike; Theorem~\ref{thm:signature} then proves that
the dynamical metric $g_{\mu\nu}(x)$ inherits the Lorentzian signature
as a consequence.
\end{remark}
...
error $< 5\times 10^{-15}$ via \texttt{tools/verify\_schwarzschild\_theta.py}
...
\begin{tcolorbox}[colback=green!4!white,colframe=green!45!black,
  title=Zerilli equation (even-parity graviton): CLOSED [L1/STD]]
Zerilli/even-parity perturbations are closed at [L1/STD] for vacuum
Schwarzschild modes with $\ell \ge 2$, using
\texttt{canonical/gr\_closure/zerilli\_derivation.tex}.
The closure relies on standard Chandrasekhar/RWZ perturbation theory
after the UBT metric-chain reduction.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=green!4!white,colframe=green!45!black,
  title=GAP-Z: Zerilli Equation\ CLOSED\ [L1/STD]]
\textbf{Proved}: The Regge-Wheeler equation (odd-parity graviton) is derived
from linearised UBT (Theorem~\ref{thm:rw}).

\smallskip
\textbf{Closed at [L1/STD]}: The Zerilli equation for even-parity perturbations:
\[
  \left[\frac{\mathrm{d}^2}{\mathrm{d}r_*^2} + \omega^2 - V_\mathrm{Z}(r)\right]
  \Psi_\mathrm{Z} = 0.
\]
is obtained for vacuum Schwarzschild modes with $\ell \ge 2$ by
standard Chandrasekhar/RWZ machinery after the UBT metric-chain reduction;
see \texttt{canonical/gr\_closure/zerilli\_derivation.tex}.

\textbf{Scope note}: this closure is standard-theory [STD] after reduction;
it does not claim a non-standard even-parity mechanism beyond RWZ.
\end{tcolorbox}
...
\begin{itemize}
  \item \textbf{Twistor theory}~\cite{penrose1976,penrose_rindler1984}:
        ...
        In the two-spinor formalism, the metric is expressed as
        $g^{ab} = \varepsilon^{AB}\varepsilon^{A'B'}$, where $\varepsilon$ is
        the two-dimensional antisymmetric tensor of the spin space.
        The UBT bilinear formula
        $g_{\mu\nu} = \re[\Tr(\partial_\mu\Theta \cdot \partial_\nu\Theta^\dagger)]/\calN$
        is structurally different: $\Theta$ takes values in the eight-real-dimensional
        algebra $\BB$, not the two-complex-dimensional spin space, and the Lorentzian
        signature is a \emph{theorem} (AXIOM-B) rather than an assumption built into
        the spin space.
...
3 & Signature $(-,+,+,+)$ from AXIOM-B + projection & Proved \Lone & step3\_signature\_theorem.tex \\
...
Theorem~\ref{thm:signature} (Lorentzian signature from AXIOM-B with
Lorentzian projection).

\paragraph{Step A1: Spinorial/Clifford representation data.}
The biquaternion algebra satisfies
$\BB = \CC\otimes_\RR\HH \cong \mathrm{Mat}(2,\CC)$.
It should not be identified with the full real Clifford algebra
$\mathrm{Cl}_{1,3}(\RR)$ (real dimension $16$).
Rather, $\BB$ provides the complex spinorial/even-Clifford carrier.
For signature $(-,+,+,+)$ the Clifford generators satisfy
\[
  (\gamma^0)^2 = -1, \qquad
  (\gamma^i)^2 = +1, \qquad
  \{\gamma^\mu,\gamma^\nu\} = 2\eta^{\mu\nu}.
\]
Pauli matrices satisfy $\sigma_i^2 = I$, while quaternionic units are
represented by $i\sigma_i$ with $(i\sigma_i)^2=-I$.
Here $I$ denotes the $2\times2$ identity matrix and $i$ is the imaginary unit.
The tuple $\{iI,\sigma_1,\sigma_2,\sigma_3\}$ matches the square signs but is
not itself a full Clifford basis because $iI$ commutes with each $\sigma_i$
(e.g.\ $\{iI,\sigma_1\}=2i\sigma_1\neq0$);
correct squares alone are insufficient without full anticommutation.

\paragraph{Step A2: AXIOM-B constraint and projection choice.}
AXIOM-B states that $\partial_\tau$ lies in a timelike complex-time sector.
Concretely: $\partial_\tau = \partial_t + i\partial_\psi$ satisfies
$\langle\partial_\tau,\partial_\tau\rangle_\eta < 0$.
To transfer this sign information to the spacetime metric, one must also use
an admissible Lorentzian real-sector projection (equivalently an indefinite
bilinear form $\eta_B$ in the chosen representation, non-degenerate and
Hermitian in the projected GR sector).
...
This follows in the GR sector from AXIOM-B together with the admissible
Lorentzian projection/representation structure.
...
All errors are below $5\times 10^{-15}$; typical values are below $5\times10^{-16}$
(close to double-precision machine epsilon $\approx 2.2\times10^{-16}$).
