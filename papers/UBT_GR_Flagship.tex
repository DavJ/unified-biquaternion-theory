...
% SUPERSEDED draft: this file is not the current GR submission manuscript.
% Current canonical GR manuscript: `papers/UBT_GR_Submission.tex`.
% Deliverable: papers/UBT_GR_Flagship.tex — FLAGSHIP superseded draft retained for publication provenance
% Historical target venues (for this superseded draft): Journal of Mathematical Physics / Classical and Quantum Gravity
% Source draft: research_tracks/T1_GR/ubt_gr_paper.tex
% Theorem result document: research_tracks/T1_GR/GR_theorem_result.tex
% Theorem chain: research_tracks/T1_GR/theorem_chain.tex
% Assumptions: research_tracks/T1_GR/assumptions.md
% Gaps: reports/GR_final_gap_checklist.md
% Reviewer responses: reports/GR_reviewer_objections_and_answers.md
% Figures: papers/UBT_GR_Figures/
...
\title{\textbf{General Relativity as a Real-Projected Limit\\
       of Unified Biquaternion Theory}\\[8pt]
  \large Flagship Paper — T1\_GR Track}
...
UBT action.
The SU(2) Scherk--Schwarz twist yields $N_{\mathrm{eff}}=12$ [L1]
for the charged sector, and the Weinberg angle
$\sin^2\theta_W=3/8$ at the unification scale follows conditionally
from generator norm ratios [L1].
...
\noindent\textit{Canonical sources: \texttt{canonical/fields/biquaternion\_algebra.tex},
\texttt{canonical/fields/theta\_field.tex},
\texttt{canonical/THEORY/axioms/core\_assumptions.tex}.}
...
\]
satisfying the UBT field equation (the ``T-shirt equation''):
\[
...
Table~\ref{tab:axioms}; see also \texttt{assumptions.md}~\cite{jaros2026_canonical}.
...
\noindent\textit{Canonical sources:
\texttt{canonical/gr\_closure/GR\_chain\_summary.tex},
\texttt{canonical/gr\_closure/step1\_metric\_bridge.tex},
\texttt{canonical/gr\_closure/step2\_nondegeneracy.tex},
...
\medskip
\noindent\textit{Full proof:
\texttt{canonical/gr\_closure/step1\_metric\_bridge.tex}~\cite{jaros2026_canonical}.}
\end{proof}
...
\noindent\textit{Full proof:
\texttt{canonical/gr\_closure/step2\_nondegeneracy.tex},
Theorem~1~\cite{jaros2026_canonical}.}
\end{proof}
...
\texttt{canonical/t\_munu/step3\_einstein\_with\_matter.tex};
$T_{\mu\nu}$ properties:
\texttt{canonical/geometry/stress\_energy.tex}~\cite{jaros2026_canonical}.}
\end{proof}
...
\noindent\textit{Canonical sources:
\texttt{canonical/geometry/biquaternionic\_vacuum\_solutions.tex}~\S3,
\texttt{canonical/gr\_closure/GR\_chain\_summary.tex}~\S{Gravitational Sector}.}
...
Full obstruction maps are in \texttt{proof\_gap\_list.md}~\cite{jaros2026_canonical}.
...
\medskip
\noindent\textit{Full proof with matrix computations:
\texttt{canonical/gr\_closure/step3\_signature\_theorem.tex}~\cite{jaros2026_canonical}.}
...
The following table summarises anticipated reviewer objections with references
to the specific preemptive actions taken in this paper.
Full responses are in \texttt{reviewer\_attack\_responses.md}~\cite{jaros2026_canonical}.

\begin{center}
\small
\begin{tabular}{llll}
\toprule
\textbf{Attack} & \textbf{Severity} & \textbf{Handled by} & \textbf{Location in paper} \\
\midrule
GR-1: metric not unique & MAJOR
  & GAP-10 open-problem statement
  & \S\ref{sec:gaps} \\
GR-2: signature postulated & MODERATE
  & Theorem~\ref{thm:signature}
  & \S\ref{sec:step3}, App.~\ref{app:signature} \\
GR-3: ansatz reverse-engineered & MODERATE
  & Numerical verification + uniqueness
  & \S\ref{sec:schwarzschild}, App.~\ref{app:numerics} \\
GR-4: Zerilli missing & MODERATE
  & GAP-Z open statement
  & \S\ref{sec:rw}, \S\ref{sec:gapz} \\
GR-5: not new & MODERATE
  & Novelty table + comparison
  & Tables~\ref{tab:novelty},~\ref{tab:comparison} \\
GR-6: overclaiming ``unified'' & MINOR
  & Scope statement
  & \S\ref{sec:discussion} \\
GR-X1: too many axioms & MINOR
  & Axiom count table
  & Table~\ref{tab:axioms} \\
GR-X2: notation inconsistent & MINOR$\to$MOD
  & Unified notation throughout
  & This document \\
GR-X3: no arXiv & MINOR
  & Submit early draft
  & (action item) \\
\bottomrule
\end{tabular}
