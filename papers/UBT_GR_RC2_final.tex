...
The full Clifford algebra has $\dim_\RR = 16$; the odd elements
(vectors, pseudovectors, pseudoscalar) lie outside $\BB$ as an algebra
but are accessible via the spinor representation.
For the classical GR sector, the even subalgebra suffices:
Lorentz boosts and rotations are grade-2 elements (bivectors),
and the metric bilinear is grade-0 valued.
\end{remark}
...
\begin{align}
  \calN := -\langle \partial_0\Theta,\partial_0\Theta \rangle_\eta > 0,
  \qquad
  g_{\mu\nu} := \frac{\langle \partial_\mu\Theta,\partial_\nu\Theta \rangle_\eta}{\calN}.
\end{align}
Here the trace is evaluated with respect to the \emph{indefinite}
Clifford inner product $\langle A, B\rangle_\eta :=
\mathrm{Re}[\mathrm{Tr}(A \cdot \eta_{\mathcal{B}} \cdot B^\dagger)]$,
where $\eta_{\mathcal{B}}$ denotes an external Lorentzian bilinear form / representation-level
metric operator on the selected four-dimensional real projection subspace associated with $\BB$.
Choosing $\calN := -\langle \partial_0\Theta, \partial_0\Theta\rangle_\eta > 0$
(positive by AXIOM-B) gives $g_{00} < 0$ consistently.
\end{definition}
...
(i) \emph{Gauge structure and three generations from UBT}
(T2\_GAUGE track), deriving $\mathrm{SU}(3)\times\mathrm{SU}(2)_L\times\mathrm{U}(1)_Y$
and fermion hypercharges; and
(ii) \emph{One-loop $\beta$-function coefficient from biquaternion NCG}
(T3\_ALPHA track), establishing $B_0 = 8\pi$ [L1].
...
3 & Signature $(-,+,+,+)$ from AXIOM-B + admissible Lorentzian projection & Proved \Lone & step3\_signature\_theorem.tex \\
