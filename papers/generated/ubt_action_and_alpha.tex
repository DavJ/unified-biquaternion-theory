...
$SU(3)_c$ & \textbf{Formally derived} & Theorems G.A--G.D: $\mathfrak{su}(3)$ from traceless phase fiber; quarks in $\mathbf{3}$, gluons in $\mathbf{8}$; decoupling proved \\
$G_\text{SM}$ full & Formally derived & Direct-product structure: left-right commutativity (Theorem~G.D) \\
\bottomrule
\end{tabular}
\end{center}
...
The two major open problems blocking a fully parameter-free derivation are:
\begin{enumerate}
...
For the color sector, Appendix~G (Theorems~G.A--G.D) provides a complete formal derivation:
$\mathfrak{su}(3)$ is isomorphic to traceless Hermitian $3\times3$ matrices (Theorem~G.A);
quarks transform in the fundamental $\mathbf{3}$ (Proposition~G.B);
gluons transform in the adjoint $\mathbf{8}$ (Theorem~G.C);
and the $\mathrm{SU}(3)_c\times\mathrm{SU}(2)_L\times\mathrm{U}(1)_Y$ direct-product structure
follows from left-right commutativity (Theorem~G.D).
Confinement (the mass gap problem) remains open, consistent with its status as a Clay
