...
\begin{abstract}
\noindent\textcolor{ubtblue}{\textbf{Exploratory Investigation:}} The fine structure constant $\alpha \approx 1/137$ might emerge from geometric structures in the Unified Biquaternion Theory (UBT). We explore how biquaternionic time topology and gauge quantization could relate to $\alpha^{-1} \approx 137$, though this remains an open problem requiring further theoretical development. See disclaimer above for critical limitations.
\end{abstract}
...
\begin{enumerate}
\item \textbf{Complex Time}: Spacetime includes biquaternionic time $\tau = t + i\psi$, where $\psi$ is an imaginary phase coordinate.
...
on a compact biquaternionic time manifold.\\[0.5em]
