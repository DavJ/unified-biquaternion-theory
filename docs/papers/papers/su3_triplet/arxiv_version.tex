...
\begin{enumerate}
  \item \textbf{Dynamical symmetry — not proved.}
    Does $\mathrm{SU}(3)_{V_c}$ extend to a symmetry of a natural
    kinetic operator on $V_c$-valued fields?  The global invariance of the
    free d'Alembertian kinetic term is proved (see companion document
    \texttt{research\_tracks/associative\_su3/global\_invariance.tex}),
    but extension to an \emph{interacting} or \emph{curved-background}
    kinetic operator is unresolved.
  \item \textbf{Local gauge extension — candidate only.}
    A local (spacetime-dependent) gauge $\mathrm{SU}(3)$ requires a
    Yang-Mills gauge field $A_\mu$ and has been formulated as a candidate
    construction (see \texttt{research\_tracks/associative\_su3/%
    local\_gauge\_extension.tex}).  It is \emph{not} an independently
    derived consequence of the algebra; its physical interpretation and
    connection to QCD are open problems.
