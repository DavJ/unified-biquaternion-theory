...
\begin{enumerate}
  ...
  \item \textbf{Open Problem~D}: [CLOSED 2026-03-06]
    Gap~C1 for chirality derivation closed: the $W^\pm$ vertex is
    $P_\psi$-odd because $S[\Theta]$ contains no $W_R$ coupling
    (see \texttt{step3\_gap\_C1\_resolution.tex}).
    Parity violation follows from $\psi$-circle orientation alone.
\end{enumerate}
