...
% Historical material only. Current canonical status is in canonical/alpha/ALPHA_MASTER_STATUS.md:
% alpha is not derived, alpha_bare^{-1}=137 is conditional, alpha^{-1}=137.036 is not derived, Gap G137-B remains open.
...
\noindent\textbf{LEGACY / SUPERSEDED ALPHA CONTENT:} This report is historical.
For canonical alpha status, use \texttt{canonical/alpha/ALPHA\_MASTER\_STATUS.md}.
Current canonical truth: alpha is not derived, \(\alpha_{\text{bare}}^{-1}=137\) is conditional,
\(\alpha^{-1}=137.036\) is not derived, and Gap G137-B remains open.
