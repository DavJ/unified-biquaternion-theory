...
% Purpose: Expository note — distinguish algebra automorphisms from carrier/module symmetries in UBT.
...
% ---- document metadata ----
\title{%
  Algebraic and Carrier Symmetries in UBT:\\[4pt]
  \large What follows from $\mathbb{C}\otimes\mathbb{H}$ and what remains dynamical}
...
The Lorentz/spin symmetry is tied directly to the biquaternionic algebra.
The involution structure also selects useful internal carriers.  However,
$\mathrm{SU}(3)$ color and the global $\mathrm{U}(1)$ phase act as
carrier/module symmetries rather than algebra automorphisms, and their
canonical dynamical selection remains an open part of the gauge programme.
...
This is the first instance of the algebra-first principle: the Lorentz/spin
symmetry is read off from the algebraic structure.  By contrast, a global
$\mathrm{U}(1)$ phase and an $\mathrm{SU}(3)$ action on an associated
$\mathbb{C}^3$ carrier are module/carrier symmetries, not algebra
automorphisms: multiplying every algebra element by a phase does not preserve
products, and a generic $\mathrm{SU}(3)$ carrier rotation does not preserve the
quaternionic square relations.
...
\begin{itemize}
  ...
  \item A complex rank-three carrier selected by the $\mathbb{Z}_2^3$
    involution structure.  A unitary $\mathrm{SU}(3)$ action can be introduced
    on this carrier, but is not an automorphism of the underlying algebra and
    is not yet derived from the canonical action.
\end{itemize}
...
\begin{keybox}
\sloppy
  The algebra fixes the Lorentz/spin automorphism structure, while its
  involutions select internal carrier spaces.  The present UBT gauge programme
  introduces unitary actions on those carriers and must still derive their
  Yang--Mills dynamics, chirality selection, and couplings from the canonical
  action.  In particular, $\mathrm{SU}(3)$ is not a subgroup of
  $\mathrm{Aut}_{\mathbb{C}}(\mathbb{C}\otimes\mathbb{H})$.
\end{keybox}
...
\begin{keybox}
  The components of $E_\mu$ are entries in a single algebraic object, so
  Lorentz/frame transformations must respect the multiplication table.  Internal
  color and phase transformations instead act on associated carriers/modules.
  Establishing how those carrier symmetries and their dynamics arise from the
  finalized canonical action is a separate, explicitly open step.
\end{keybox}
...
\begin{tabular}{>{\raggedright\arraybackslash}p{5.0cm}
  ...
  Carrier $V_c\cong\mathbb{C}^3$ from involutions; unitary dynamics postulated
  & {[L0 proved / dynamics open]}
