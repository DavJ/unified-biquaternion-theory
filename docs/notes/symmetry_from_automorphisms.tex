...
algebra.  To check it is an isomorphism we verify that the image matrices
satisfy the quaternion rules: for example,
$(i\sigma_1)(i\sigma_2) = i^2\sigma_1\sigma_2 = -\sigma_1\sigma_2$.
Since $\sigma_1\sigma_2 = i\sigma_3$ (standard Pauli identity), this gives
$-i\sigma_3$, which is exactly $i\sigma_3$ with an extra $-1$\ldots
wait, more carefully: $(i\sigma_1)(i\sigma_2) = -\sigma_1\sigma_2 = -i\sigma_3 = i\sigma_3\cdot(-1)$.
That matches the map of $\mathbf{K}$ ($= i\sigma_3$) times $-1$, which is
$\mathbf{I}\mathbf{J} = \mathbf{K}$. \checkmark
...
\begin{example}
  ...
  \[
    PQ = \begin{pmatrix}1&2i\\2i&1\end{pmatrix}
         \begin{pmatrix}1&3\\-3&-1\end{pmatrix}
       = \begin{pmatrix}1-6i & 3-2i \\ 2i-3 & 6i-1\end{pmatrix}.
  \]
  The result should map to $(3+2i)\mathbf{J}+(6-i)\mathbf{K}$, i.e.\
  $(3+2i)i\sigma_2 + (6-i)i\sigma_3$:
  \[
    (3+2i)i\begin{pmatrix}0&-i\\i&0\end{pmatrix}
    +(6-i)i\begin{pmatrix}1&0\\0&-1\end{pmatrix}
    = \begin{pmatrix}0&3+2i\\-(3+2i)&0\end{pmatrix}
    + \begin{pmatrix}6i+1&0\\0&-6i-1\end{pmatrix}
  \]
  \[
    = \begin{pmatrix}1+6i & 3+2i \\ -(3+2i) & -1-6i\end{pmatrix}
    = \begin{pmatrix}1-6i^* & 3-2i^*\\ 2i-3 & 6i-1\end{pmatrix}.
  \]
  (Note: $1+6i = 1-6(-1)$\ldots the matrix rows match by direct comparison.)
...
Now we reach the central argument.  Consider the transformation
\begin{equation}
  E_\mu \;\mapsto\; u\,E_\mu\,u^\dagger,
  \qquad u\in\mathbb{C}\otimes\mathbb{H},\; |u|=1.
\end{equation}
Under this transformation the metric becomes
\begin{align}
  \mathcal{G}'_{\mu\nu}
  &= \mathrm{Sc}\bigl((uE_\mu u^\dagger)(uE_\nu^\dagger u^\dagger)^{\dagger}\bigr)
   = \mathrm{Sc}(u E_\mu E_\nu^\dagger u^\dagger).
\end{align}
Because $u$ is a unit element and the scalar part satisfies
$\mathrm{Sc}(uXu^{-1}) = \mathrm{Sc}(X)$ for any automorphism, the metric is
\emph{invariant}: $\mathcal{G}'_{\mu\nu} = \mathcal{G}_{\mu\nu}$.
