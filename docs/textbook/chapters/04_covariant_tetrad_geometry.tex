...
\subsection{A correct calculation can answer the wrong architectural question}
...
\subsection{No-new-field reduction and its curved-space no-go}
...
This does not close curved GR.  The same involution-compatible derivative makes
a nondegenerate torsion-free solution
$D_\mu\Theta=\sqrt{\mathcal N_0}E_\mu$ imply a vector field satisfying
\[
 \boxed{\nabla_\mu V^\nu=\delta_\mu{}^\nu,}
 \qquad \mathcal L_Vg=2g.
\]
Schwarzschild with nonzero mass has no such proper homothety.  The pure pair is
therefore closed kinematically and closed as a no-go for generic curved GR.
A viable completion must derive a relative term
$P_\mu X-XQ_\mu$ beyond the pure spin lift and prove that it is composite or
auxiliary rather than a new propagating field.  A common central constant does
not help because it cancels.

\section{Conditional dynamical subclosures}
\subsection{The algebraic Cartan torsion equation}
...
\subsection{Fixed-set propagation of the Lorentz slice}
...
\subsection{Imaginary-time metric stability}
A vertical flow
\[
...
\subsection{Augmented holonomy for prescribed curved coefficients}
...
\subsection{The conditional Einstein--Lambda endpoint}
...
\begin{enumerate}
 ...
 \item GAP-10I-PAIR-KIN: the pure compatible pair reduces to the one spin
       connection;
 \item GAP-10I-PAIR-GR: that pure pair is a concurrent-vector no-go for generic
       curved GR and Schwarzschild with nonzero mass;
...
\begin{enumerate}
 \item GAP-10T-DYN: derive the selected first-order branch, spin current,
       normalization and possible extra torsion invariants from canonical UBT;
 \item GAP-10I-2S/CURVED: derive a nontrivial relative bimodule action as a
       composite or auxiliary nonpropagating variable, then establish
       self-consistent on-shell selection, regularity and global continuation
       of $(\Theta,E,A,B,T)$;
