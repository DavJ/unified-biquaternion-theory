...
\begin{description}
  \item[Gap G3-k (Kac-Moody level $k=1$).]
    ...
    Earlier drafts motivated $k=1$ using a $p=139$ mirror-sector argument; that
    interpretation has been moved to the speculative extension and is not used
    here as a canonical derivation.  A direct derivation of $k = 1$ from
    $\mathcal{S}[\Theta]$ remains unavailable. In particular, the canonical
    norm condition $\|\Theta\|^2 = 1$ gives $r^2 = 1$ and
    $k = 2\pi r^2 = 2\pi \notin \mathbb{Z}$, which is inadmissible
    \textbf{[Dead End]}.
  ...
  \item[Gap B (correction factor $R \approx 1.114$).]
    ...
    A new candidate arises from the additive reformulation $\Delta B = B - B_{\mathrm{base}}
    \approx 4.74$: the value $\Delta B = 3\pi/2 \approx 4.712$ ($0.6\%$ error) combines
    the modular weight $k_{\mathrm{mod}} = 3/2$ [L0] with the SSB half-period
    $\theta_W^{\mathrm{eff}} = \pi$ [L1] as $\Delta B = k_{\mathrm{mod}} \times \theta_W^{\mathrm{eff}}$.
    Algebraic derivation from $\mathcal{S}[\Theta]$ is pending
...
The FPE-EL equivalence holds in all sectors (U(1), SU(2), SU(3))
by the $\mathrm{Sc}$-projection property: $\mathrm{Sc}[\mathrm{Im}(\mathbb{H})] = 0$
\textbf{[Proved L0]}.  Specifically, the commutator correction
$[\mathbf{e}_a,\mathbf{e}_b] \in \mathrm{Im}(\mathbb{H})$ is annihilated by
$\mathrm{Sc}[\cdot]$, so the FPE derived from
$\mathcal{S}[\Theta] = \int\mathrm{Sc}[(\nabla\Theta)^\dagger(\nabla\Theta)]$
is algebraically equivalent to the Euler--Lagrange equation in all gauge sectors
(see \texttt{canonical/qm\_emergence/step4\_fpe\_equivalence.tex},
Theorem~thm:fpe\_all\_sectors).
