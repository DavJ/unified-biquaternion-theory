...
has its minimum at $\theta_W = \pi/2$ in this effective description.
Earlier language invoking a $p=139$ mirror sector has been moved to the
speculative extension and is not used here as a canonical derivation.  This gives
\begin{equation}
...
\begin{enumerate}
  \item \textbf{Zerilli potential (even-parity)} \OPEN\ \Ltwo:
        ...
        \textit{v72 — Two-component approach}: The resolution has been
        identified and set up explicitly (see
        \texttt{research\_tracks/research/graviton\_schwarzschild.tex}~§7.3):
        \begin{itemize}
          \item Two-component even-parity Ansatz:
                $\Theta_1^{\text{even}} =
                  [H_0(r)\mathbf{1} + H_2(r)\boldsymbol{e}_r\boldsymbol{e}_r]
                  Y_{2m}\,e^{-i\omega t}$
                (plus angular component $K(r)$, eliminated by Zerilli gauge).
          \item Linearising $\nabla^\dagger\nabla\Theta = 0$ around the
                Schwarzschild background yields a coupled $2\times 2$ ODE
                for $(H_0, H_2)$.
          \item The Zerilli master variable
                $Z = 2r^2H_0/(2r+3M) + \cdots$
                formally decouples the system into the Zerilli equation.
          \item \textbf{Pending}: explicit extraction of the off-diagonal
                coupling coefficients from the UBT action and verification
                that the master equation reproduces $V_Z(r)$ exactly.
        \end{itemize}
        Status: \OPEN\ [\Ltwo] — structural identification complete;
        explicit coefficient computation not yet done.
        If the two-component computation succeeds: Zerilli from UBT
        Proved [L1] — major result for the second paper.

  \item \textbf{Higgs sector and Yukawa couplings}:
        The Higgs potential $V(|\Theta|^2) = \lambda(|\Theta|^2-v^2)^2$ is
        a Motivated Conjecture.  Earlier $p=139$ mirror-sector motivation has been
        moved to the speculative extension.  The quartic coupling
