...
The matching of $V_{\text{eff}}$ to the standard Regge-Wheeler potential
is an open problem at \Ltwo; see Section~\ref{sec:open}.
...
\begin{enumerate}
  \item \textbf{Regge-Wheeler coefficient matching} \OPEN\ \Ltwo:
        The dispersion relation $k^2 + V_{\text{eff}}(r) = 0$
        (\eqref{eq:dispersion_curved}) is structurally identical to the
        Regge-Wheeler equation
        \begin{equation}
          \left[\frac{d^2}{dr_*^2} + \omega^2 - V_{\text{RW}}(r)\right]
          \psi_{\text{RW}} = 0,\quad
          V_{\text{RW}} = \left(1-\frac{2M}{r}\right)\!\!
            \left[\frac{\ell(\ell+1)}{r^2} - \frac{6M}{r^3}\right],
          \label{eq:regge_wheeler}
        \end{equation}
        but the explicit coefficient matching (converting between isotropic
        and Schwarzschild coordinates and including angular mode
        decomposition) remains open at \Ltwo.
