...
\paragraph{Note on Complex Time Approximation in QED.}
\textbf{Justification for using complex time $\tau = t + i\psi$}: QED in UBT utilizes the complex time approximation because: (1) The electromagnetic field is Abelian, satisfying $[\Theta_i, \Theta_j] \approx 0$ for QED components, and (2) For typical QED processes at accessible energies, the vector components satisfy $\|\mathbf{v}\|^2 \ll |\psi|^2$. The full biquaternionic time structure would be required only for extremely high-energy QED processes coupled to strong gravitational fields or in rotating reference frames. See Appendix~\ref{sec:biquaternion_vs_complex_time} for the complete criterion.
