...
\begin{resultbox}
  ...
  Status: CLEAN [L0] — cohomological argument from $G_{\mathrm{SM}}\hookrightarrow\mathcal{B}$.
  No reference to $\alpha$ or $B_{\mathrm{required}}$.
\end{resultbox}
...
weight $k = 3/2$ under $\tau\to-1/\tau$.  The one-loop effective potential
coefficient $B_{\mathrm{base}}$ equals $k \times N_{\mathrm{eff}}$ in the standard
CFT identification (Gap G3-k), which gives:
\begin{equation}
  B_{\mathrm{base}} = k \times N_{\mathrm{eff}} = \frac{3}{2} \times N_{\mathrm{eff}}
  \;\Rightarrow\;
  B_{\mathrm{base}} = N_{\mathrm{eff}}^{3/2}
  \text{ when } k = \tfrac{1}{2}\sqrt{N_{\mathrm{eff}}}.
\end{equation}
For $N_{\mathrm{eff}} = 12$: $k = \tfrac{1}{2}\sqrt{12} = \sqrt{3} \approx 1.73$.
The relation $B_{\mathrm{base}} = N_{\mathrm{eff}}^{3/2}$ with exponent $3/2$
corresponds to $d_{\mathrm{eff}} = 3$:
\begin{equation}
  d_{\mathrm{eff}}^{(\mathrm{D2})} = 3 \quad [\text{from modular weight 3/2}].
\end{equation}
...
D2 & Modular weight of $\vartheta_3^3(\tau)$ & 3 & CLEAN [L0] \\
...
the winding-mode attractor:
\begin{equation}
  n^* = \sqrt{B/2} = \sqrt{12^{3/2}/2} = \sqrt{12\sqrt{3}} \approx \sqrt{20.78} \approx 4.56.
  \label{eq:nstar_raw}
\end{equation}

\begin{gapbox}
\textbf{Open gap}: The value $n^* \approx 4.56$ from the bare formula does not
yet reproduce $\alpha^{-1} \approx 137$.  A correction factor $R$ from the
renormalisation group evolution and the $S^1_\psi$ radius calibration is still
required.  These correspond to Assumptions A9, A10, A12 in the assumptions audit
...
$d_{\mathrm{eff}}$ from D2
  & Modular weight of $\vartheta_3^3$
  & 3 & CLEAN [L0] \\
...
\midrule
$n^*$ (bare)
  & $\sqrt{B/2}$; correction factor $R$ still open
  & $\approx 4.56$ & Incomplete \\
\bottomrule
\end{tabular}
\end{center}
...
Only after establishing both quantities is $B = 12^{3/2} \approx 41.57$
computed.  The remaining gap is the $R$-factor and the precise form of
$n^*\to\alpha^{-1}$ identification.
