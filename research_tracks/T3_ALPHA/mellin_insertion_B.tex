...
Byl proveden numerický scan všech kombinací tvaru
$$
B = 12^{3/2} [\eta(i)^a \vartheta_3(i)^b \vartheta_2(i)^c \vartheta_4(i)^d]^{1/4}
$$
pro $a+b+c+d=1$ a $a,b,c,d$ racionální v rozsahu $[-2,2]$.

\textbf{Výsledek:} Nejlepší dosažený poměr (ratio) je $0.907$ (mock theta), žádná kombinace nedává $\mathrm{ratio} < 0.001$.

\begin{statusbox}
Systematický scan kombinací $(\eta,\vartheta_2,\vartheta_3,\vartheta_4)$: \textbf{NO-GO}\\
Nejlepší dosažený poměr: $0.907$ (mock theta)\\
Žádná kombinace nedává $\mathrm{ratio} < 0.001$\\
Gap G137-B: \textbf{[NO-GO]}
\end{statusbox}
...
\textbf{Current status}: Gap G137-B is \textbf{OPEN}.
The numerical observation $B_{\rm phenom}\approx 46.28$ at
$0.0066\%$ accuracy remains \textbf{[OBS]}.

\textbf{Alpha is NOT DERIVED.}

\textbf{Next candidate} (if any): string-inspired modular bootstrap
via mock modular forms or Rademacher expansion of the partition
function. No timebox active; this is explicitly a long-term open problem.
\end{tcolorbox}
