...
% Status: Gap G137-B OPEN — five tested routes logged; Eisenstein E4 is NO-GO
% Date: 2026-05-18
...
\begin{abstract}
This note audits the open step in Gap G137-B: deriving the coefficient
$B \approx 46.28$ in $V_{\mathrm{eff}}(n) = n^2 - Bn\ln n$ from the UBT
action. Five explicit routes are now on record:
T-duality [NO-GO], orbifold [NO-GO], direct $M^4$ functional determinant
[NO-GO, residual factor $\sim 1.14\times 10^4$], Casimir $T^3$ [NO-GO,
residual factor $\sim 9.0\times 10^3$], and Eisenstein $E_4(i)$ [NO-GO,
short by $\sim 8.0\%$]. The coefficient source remains open.
Gap G137-B status: \textbf{OPEN}. Alpha: \textbf{NOT DERIVED}.
\end{abstract}
...
Fyzikální původ z $S[\Theta]$: \textbf{[OPEN — Gap G137-B NARROWED]}\\
...
Gap G137-B status: \textbf{NARROWED} (Casimir route explored, closed)\\
Alpha: \textbf{NOT DERIVED}
\end{statusbox}
...
Status: Gap G137-B is \textbf{OPEN}. The numerical coincidence
...
Gap G137-B: \textbf{NARROWED — one MC step remaining}\\
\textbf{Alpha is NOT DERIVED.}
\end{statusbox}
\end{tcolorbox}
...
Mock modular route closes Gap G137-B: \textbf{[NO-GO \#6]}\\
...
%──────────────────────────────────────────────────────────────────────────────
\section*{Gap G137-B --- Formal Downgrade Notice (2026-05-19)}
%──────────────────────────────────────────────────────────────────────────────

\begin{tcolorbox}[colback=red!3!white, colframe=red!50!black,
  title={Gap G137-B --- Formal Downgrade Notice (2026-05-19)}]
\textbf{Five routes tested, all NO-GO:}
\begin{enumerate}
\item T-duality map $\Theta\to\tilde\Theta$: \textbf{[NO-GO]}
\item Orbifold $Z_{\rm orb} = (\vartheta_3+\vartheta_4)/(2\eta)$:
      \textbf{[NO-GO]} ($Z_{\rm orb}=1.302\neq\sqrt{2}$)
\item Direct M$^4$ $\zeta$-regularisation:
      \textbf{[NO-GO]} (residual factor $1.14\times10^4$)
\item Casimir energy on $T^3$:
      \textbf{[NO-GO]} (residual factor $9.0\times10^3$)
\item Eisenstein $E_4(i)$ modular route:
  \textbf{[NO-GO]} (ratio $0.920$)
\item Mock theta $f(e^{-\pi})$:
  \textbf{[NO-GO]} (ratio $0.907$)
\end{enumerate}

\textbf{New result}: The combination
$\eta(i)^{-2}\vartheta_3(0|i)\vartheta_4(0|i)^2 = 2\eta(i)$
is an algebraic identity [\textbf{L0}], so
$B = 12^{3/2}(2\eta(i))^{1/4}$ [\textbf{L0}].
The open problem is now precisely:
\emph{derive why the combination $\eta^{-2}\vartheta_3\vartheta_4^2$
arises from $S[\Theta]$ on $T^3$ at $\tau=i$}.

\textbf{Current status}: Gap G137-B is \textbf{NARROWED}
(not NO-GO). \textbf{Alpha is NOT DERIVED.}
\end{tcolorbox}
\end{tcolorbox}

\section{Minimal remaining open problem}
\label{sec:minimal-open}
%────────────────────────────────────────────────────────────────────

Z algebraické identity $\S$\ref{sec:algebraic-identity}:
\[
B = 12^{3/2}(2\eta(i))^{1/4}
= 12^{3/2}\bigl[\eta(i)^{-2}\vartheta_3(0|i)\vartheta_4(0|i)^2
\bigr]^{1/4}
\quad [\mathbf{L0}].
\]

Kombinace $\eta^{-2}\vartheta_3\vartheta_4^2$ má přirozený
fyzikální smysl: při $\tau=i$ je to součin partiční funkce
periodického sektoru ($\vartheta_3/\eta$, pro pole $a,d$),
antiperiodického sektoru ($\vartheta_4/\eta$, pro pole $b,c$)
a normalizačního faktoru $1/\eta^2$.

\paragraph{Minimální chybějící teorém.}
Zbývá dokázat, že efektivní potenciál $V_{\rm eff}(n)$
odvozený z jednosmyčkové akce $S[\Theta]$ na $T^3=S^1_\psi
\times S^1_\chi\times S^1_\xi$ při $\tau=i$ dává:
\[
V_{\rm eff}(n) = n^2 - B\,n\ln n + \mathcal{O}(n),
\]
kde $B$ je koeficient identifikovaný s
$12^{3/2}[\eta^{-2}\vartheta_3\vartheta_4^2]^{1/4}=12^{3/2}(2\eta)^{1/4}$.

Konkrétně: ukázat, proč se v jednosmyčkovém výpočtu
$\mathrm{Tr}[\ln P_{\rm eff}]$ na $T^3$ při $\tau=i$
přirozeně objeví kombinace
$\eta(i)^{-2}\cdot\vartheta_3(0|i)\cdot\vartheta_4(0|i)^2$,
a nikoli jiná kombinace modulárních forem.

\begin{tcolorbox}[colback=blue!3!white, colframe=blue!50!black,
  title={Gap G137-B --- Minimal Missing Theorem (2026-05-20)}]
\textbf{Proved [L0]}: $B = 12^{3/2}(2\eta(i))^{1/4}$
via identity $\eta^{-2}\vartheta_3\vartheta_4^2=2\eta$ at $\tau=i$.

\textbf{Open}: Why does the combination
$\eta^{-2}\vartheta_3\vartheta_4^2$ arise from
$\mathrm{Tr}[\ln P_{\rm eff}]$ on $T^3$ at $\tau=i$?

Candidate physical mechanism:
The twisted boundary conditions (SU(2) twist, [L1])
select sectors $\vartheta_3$ (periodic, neutral fields $a,d$),
$\vartheta_4$ (anti-periodic, charged fields $b,c$),
and $1/\eta^2$ from the heat-kernel determinant normalisation.
This combination $= 2\eta$ at $\tau=i$ is a structural consequence
of the self-dual point.

\textbf{Status}: Gap G137-B is \textbf{NARROWED}.
\textbf{Alpha is NOT DERIVED.}
\end{tcolorbox}
