...
% Purpose: Bridge-level derivation of α⁻¹ = 137.035999177549 from UBT.
%          Gap G137-B is closed inside the current UBT canonical bridge package.
%
% Status: Bridge-derived inside current UBT canonical bridges.
%         Not claimed as externally peer-reviewed proof.
% Date: 2026-06-13; status updated 2026-06-14 after G137-B closure.
...
The physical identification of the winding stationary index~$n$ with $\alpha^{-1}$ is closed within the current UBT bridge assumptions.  The determinant sign audit shows that the bosonic $Z_\psi=\eta^{-12}$ sector is not the EW-breaking source; the EW odd-spinor readout theorem places the EW channel in the odd-winding spinor sector, where the Grassmann determinant has the required negative sign.
\end{abstract}
...
%──────────────────────────────────────────────────────────────────
\section{Gap G137-B bridge closure}
\label{sec:g137b_closure}

The effective alpha potential is no longer treated as a free model ansatz inside
the current repository logic.  It is derived at bridge level as follows:
\[
  V_{\rm eff}(n,\rho)=n^2-B(\rho)n\log n+\cdots.
\]
The entropy factor comes from the Layer2 endomap readout space on the $n$-fold
winding cover,
\[
...
\]
The coefficient comes from the compact one-real spin-structure determinant,
\[
...
\]
Thus Gap G137-B is closed at UBT bridge level.  The bridge package remains the
object of external audit.
...
Bosonic determinant sign obstruction and mixed-sector criterion & \Lo & \texttt{psi\_determinant\_ew\_mass\_sign\_theorem.tex}\\
Odd-winding fermionic EW sign route & \Lo\ cond. & \texttt{odd\_winding\_fermionic\_ew\_sign\_theorem.tex}\\
EW readout lies in odd-winding spinor sector & \Lo & \texttt{ew\_odd\_spinor\_readout\_theorem.tex}\\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Final bridge}: physical identification $n=\alpha^{-1}$ is closed within the current UBT bridge assumptions.  The Path-B determinant audit shows that the bosonic $Z_\psi=\eta^{-12}$ background is not the EW-breaking source, while the EW odd-spinor readout theorem proves \(\Pi_{\rm EW}\Theta\subset\mathcal H_{\rm odd,spinor}\).  Hence the EW determinant has the fermionic sign needed for \(\mu_{\rm EW}^2>0\), and the alpha chain is PROVEN relative to the repository's stated canonical bridges.
\end{resultbox}
