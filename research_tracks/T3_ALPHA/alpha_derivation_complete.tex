...
%         Physical identification n=α⁻¹: [MC] — sole remaining gap.
% Date: 2026-06-13
...
The sole remaining gap is the physical identification of the
winding stationary index~$n$ with $\alpha^{-1}$ (\MC, \S\ref{sec:interp}).
\end{abstract}
...
\begin{remark}[Sole remaining gap: $n=\alpha^{-1}$]
The identification of the winding stationary index~$n$ with
$\alpha^{-1}$ requires:
\begin{enumerate}
\item $V_{\rm eff}(n)=n^2-B(\rho)n\ln n$ is the physical effective
  potential for U(1)$_{\rm EM}$ winding in UBT.
\item The stationary index $n^*$ governs the low-energy EM coupling.
\end{enumerate}
Both are physically natural (winding modes source EM coupling in the
$\psi$-sector) but not yet formal theorems.
Status: \MC.
\end{remark}
...
\midrule
$n=\alpha^{-1}$ (physical identification) & \MC & \S\ref{sec:interp}\\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Sole remaining gap}: physical identification $n=\alpha^{-1}$ \MC.
\end{resultbox}
