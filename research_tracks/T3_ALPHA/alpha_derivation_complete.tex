...
The physical identification of the winding stationary index~$n$ with $\alpha^{-1}$ is closed as a conditional chain.  The determinant sign audit shows that the explicit bosonic $Z_\psi=\eta^{-12}$ sector alone gives the wrong sign if directly used for EW breaking; the primary Path-B resolution is to place the EW doublet readout in the canonical odd-winding spinor sector, where the Grassmann determinant has the required negative sign.
\end{abstract}
...
\textbf{Final bridge}: physical identification $n=\alpha^{-1}$ is closed as a conditional chain.  The Path-B determinant audit shows that the bosonic $Z_\psi=\eta^{-12}$ background alone is not the EW-breaking source.  The primary route to PROVEN status is now the odd-winding readout theorem \(\Pi_{\rm EW}\Theta\subset\mathcal H_{\rm odd,spinor}\), which gives the required fermionic determinant sign.
\end{resultbox}
