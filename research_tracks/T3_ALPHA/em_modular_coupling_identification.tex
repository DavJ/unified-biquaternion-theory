...
\section{Remaining conditional assumption}

The only remaining conditional statement is structural, not numerical:
\[
  \boxed{
  \text{UBT EM projection} = \text{compact unit-charge }U(1)_{\rm EM}
  \text{ line bundle.}
  }
\]
This is the correct place for further work on the electroweak projection
\(SU(2)_L\times U(1)_Y\to U(1)_{\rm EM}\).  However, once that projection is
accepted, the identification \(n=\alpha^{-1}\) follows from standard Maxwell
normalisation.
