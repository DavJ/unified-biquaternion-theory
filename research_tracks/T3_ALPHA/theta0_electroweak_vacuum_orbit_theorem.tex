...
postulate the shape of \(\Theta_0\).  It only needs the standard symmetry-breaking condition, made explicit in
\texttt{theta0\_nonzero\_minimum\_theorem.tex}:
\[
  V_{\rm EW}=V_0-\mu_{\rm EW}^2\Phi^\dagger\Phi+\lambda_{\rm EW}(\Phi^\dagger\Phi)^2,
  \qquad \lambda_{\rm EW}>0,\quad \mu_{\rm EW}^2>0.
\]
...
\[
  ...
  n=\alpha^{-1}.
\]
The remaining gap is not the EM projection or charge normalisation.  The
non-zero-minimum theorem closes the low-energy potential step; the remaining UV
question is the derivation of the symmetry-breaking sign \(\mu_{\rm EW}^2>0\)
from the full canonical \(S[\Theta]\).
\end{resultbox}
