...
%
% Status: LEGACY GAP NOTE.  Superseded as current alpha status by
%         alpha_derivation_complete.tex and
%         theta0_uv_sign_from_psi_casimir_candidate.tex.
%         Gap A remains useful historically, but it is no longer the sole
%         current alpha gap.  Current status: complete conditional chain;
%         remaining UV tasks are m_{0,EW}^2=0 and Gamma_psi>0 from S[Theta].
...
%──────────────────────────────────────────────────────────────────
\begin{abstract}
Legacy note: Gap~A was previously identified as the sole remaining unproved step in an earlier UBT alpha route:
...
theorem. Status: \MCp\ as a historical gap analysis.  The current alpha route is maintained in \texttt{alpha\_derivation\_complete.tex}; after the compact-$\psi$ UV-sign candidate, the remaining tasks are the UV derivations of $m_{0,\rm EW}^2=0$ and $\Gamma_\psi>0$ from canonical $S[\Theta]$.
