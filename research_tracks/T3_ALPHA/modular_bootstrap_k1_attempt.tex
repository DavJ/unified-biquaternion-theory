...
$V_{\mathrm{eff}}(n) = n^2 - B\,n\ln n$ effective potential &
  CLEAN [L1] given $B$ & \texttt{canonical/appendices/appendix\_alpha\_geometry.tex} \\
Stationarity $2n^* = B(\ln n^* + 1)$ (transcendental) &
  CLEAN [L1] given $B$ & same, corrected 2026-05-02 \\
...
\begin{enumerate}
  ...
  \item Resolution: $c = 3/2$ rather than $c = 12$ (only 3 effective bosons,
    one per $\mathrm{Im}(\mathbb{H})$ direction, where $\mathrm{Im}(\mathbb{H}) = \mathrm{span}\{i,j,k\}$).
\end{enumerate}
...
5 & $B = \mu(\Gamma_0(p))/3\big|_{p=137} = 46$ & $46$ (exact) & Modular route [MC/OPEN] \\
5b & Prime minimiser: $q = 137$ at $B = 46$ & $137$ & CLEAN [L0] \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Note}: The correction-factor route (step 5 previously: $n^* = \sqrt{B_{\mathrm{base}}/2}$
with $B \to B_{\mathrm{base}} \cdot R^2$, $R \approx 1.114$) is
\textbf{abandoned}.  The modular prime-attractor route replaces it:
$B = \mu(\Gamma_0(137))/3 = 46$ is an exact arithmetic invariant that places the
discrete prime minimiser of $V(q;\,B)$ at $q = 137$ ([L0]).
The identification of $B$ with $\mu(\Gamma_0(p))/3$ from $S[\Theta]$ is Gap G-Bmod
(\textbf{Open}); see \texttt{canonical/alpha/modular\_prime\_attractor\_theorem.tex}.

The prime value $n^* = 137$ is stable (prime) under the prime-stability
argument and is \textbf{fully proved at [L0] given $B = 46$}.
