...
\end{equation}
where $f(\lambda) := \frac{1}{2}\ln\lambda$ is the standard one-loop determinant
contribution (from $\log\det(-\nabla^2) = \Tr\ln(-\nabla^2) = \sum_\lambda \ln\lambda$,
giving a $\frac{1}{2}\ln\lambda$ contribution per eigenvalue $\lambda$ of $-\nabla^2$).
...
\midrule
\textbf{Path B resolves Gap G137-B} & \textbf{NO} (as of 2026-05-10) \\
\bottomrule
\end{tabular}
\end{center}
