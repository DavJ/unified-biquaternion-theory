...
The term $\beta_{\kappa_\psi}$ in Eq.~\eqref{eq:beta_kappa} is \textbf{unique to UBT}. It arises from loops involving $\partial_\psi \Theta$ and is proportional to the winding number squared $n_\psi^2$. 

\textbf{Symbolic form}:
\begin{equation}
\beta_{\kappa_\psi} = \frac{n_\psi^2}{16\pi^2 R_\psi^2} \kappa_\psi^2
\label{eq:beta_kappa_symbolic}
\end{equation}

\textbf{Numerical evaluation} (using calibrated $n_\psi = 137$):
\begin{equation}
\beta_{\kappa_\psi} \bigg|_{n_\psi=137} \approx \frac{18769}{16\pi^2 R_\psi^2} \kappa_\psi^2 \approx 1191 \, \frac{\kappa_\psi^2}{R_\psi^2}
\end{equation}

Note: The numerical coefficient $1191 \approx 137^2 / (16\pi^2)$ depends on the \textbf{phenomenological choice} $n_\psi = 137$, not a derived quantity.
\end{remark}
...
\section{RG Flow Status: LOCKED}

\subsection{Audit of Numeric Insertions}

We audit all explicit numerical values appearing in RG equations:
...
\hline
$n_\psi = 137$ & Calibrated to $\alpha^{-1}(m_e) = 137.036$ & \textbf{PHENOMENOLOGICAL} \\
$n_\psi^2 = 18769$ & Computed from $n_\psi = 137$ & Derived (from calibration) \\
$M_{\text{Pl}} \sim 10^{19}$ GeV & Planck mass (measured) & \textbf{FIXED} \\
$H_0 \sim 70$ km/s/Mpc & Hubble constant (measured) & \textbf{FIXED} \\
$\Delta H_0/H_0 \sim 0.08$ & Hubble tension (observed) & \textbf{FIXED} \\
$\alpha^{-1}(m_e) = 137.036$ & Fine-structure constant (CODATA) & \textbf{FIXED} \\
$\alpha^{-1}(m_Z) = 127.95$ & Running to Z-pole (measured) & \textbf{FIXED} \\
$b_1 = -41/6$ (U(1)) & SM β-function coefficient & \textbf{DERIVED} \\
$b_2 = -19/6$ (SU(2)) & SM β-function coefficient & \textbf{DERIVED} \\
$b_3 = -7$ (SU(3)) & SM β-function coefficient & \textbf{DERIVED} \\
\hline
\end{tabular}
\caption{Classification of all numerical values in RG flow equations. FIXED = observational input; DERIVED = computed from theory; PHENOMENOLOGICAL = fitted/calibrated.}
\label{tab:parameter_classification}
\end{table}

\textbf{Key finding}: All appearances of $137$ or $137^2$ are traced to the calibration $n_\psi = 137$. No independent numeric insertions exist.

\subsection{Parameter Budget with Classification}

\begin{definition}[UBT RG Parameter Budget]
\textbf{Fitted parameters} (adjusted to match observations):
\begin{enumerate}
    \item $\kappa_{\psi,0}$: Chronofactor normalization at reference scale — \textbf{FREE}
    \item $D_\psi$: Phase diffusion rate — \textbf{FREE}
    \item $\Lambda_{\RG}$: UV RG scale — \textbf{FREE}
\end{enumerate}
Total fitted: \textbf{3 FREE parameters}.

\textbf{Derived quantities} (predicted from fits + UBT structure):
\begin{enumerate}
    \item $\kappa(z)$: Latency coefficient as function of redshift — \textbf{DERIVED}
    \item $\Delta H_0 / H_0$: Hubble tension magnitude — \textbf{DERIVED}
    \item $\mu_\psi = n_\psi / R_\psi$: Phase sector cutoff — \textbf{DERIVED}
    \item $\beta_{\kappa_\psi}$: β-function for chronofactor — \textbf{DERIVED}
\end{enumerate}

\textbf{Fixed by observation} (not free):
\begin{enumerate}
    \item $\alpha^{-1}(m_Z) = 127.95$ — \textbf{FIXED (measured)}
    \item $M_{\text{Pl}} = 1.22 \times 10^{19}$ GeV — \textbf{FIXED (measured)}
    \item $H_0 \approx 70$ km/s/Mpc — \textbf{FIXED (measured)}
\end{enumerate}

\textbf{Phenomenological calibration}:
\begin{enumerate}
    \item $n_\psi = 137$ — \textbf{PHENOMENOLOGICAL} (calibrated to match $\alpha^{-1}(m_e) \approx 137.036$)
    \item $R_\psi$ — Constrained by $n_\psi$ and cosmological fits — \textbf{PHENOMENOLOGICAL}
\end{enumerate}
\end{definition}
...
\hline
FREE (fitted) & 3 & $\kappa_0, D_\psi, \Lambda_{\RG}$ \\
DERIVED & 4 & $\kappa(z), \Delta H_0/H_0, \mu_\psi, \beta_{\kappa}$ \\
FIXED (measured) & 3 & $\alpha^{-1}(m_Z), M_{\text{Pl}}, H_0$ \\
PHENOMENOLOGICAL & 2 & $n_\psi=137, R_\psi$ \\
\hline
\textbf{Total parameters} & \textbf{3} & (excluding fixed observables) \\
\hline
\end{tabular}
\caption{Complete parameter budget for UBT RG flow. Only 3 free parameters are fitted; all others are either measured, derived, or calibrated.}
