...
\begin{statusbox}
\textbf{Update 2026-06-14, beta-closure.}  The odd-spinor threshold coefficients
$b_1=33/5$, $b_2=1$, $b_3=-3$ are derived in
\texttt{weinberg\_angle\_odd\_spinor\_threshold\_beta\_theorem.tex}.  Combining
these coefficients with the UBT hypercharge boundary, the compact-scale bridge,
and the alpha/QED running bridge gives the $Z$-pole closure in
\texttt{weinberg\_angle\_mz\_bridge\_closure.tex}.  The Weinberg angle is now
bridge-proven inside the current UBT assumptions; external acceptance still
requires defending those bridges.
\end{statusbox}
