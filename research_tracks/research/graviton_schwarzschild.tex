...
Regge-Wheeler coefficient matching (full, with angular modes) & \OPEN\ [L2] \\
\hline
\end{tabular}
\end{center}

\textbf{Conclusion}: Coefficient matching is incomplete because the UBT
linearisation has not yet been decomposed into spin-2 spherical harmonics.
The structural correspondence ($k^2 + V_{\text{eff}} = 0$ vs.\
Regge-Wheeler) is established \Lone, but the coefficient values do not
agree until the angular decomposition is included.  This is consistent
with a matching expected to emerge after step~1 above — the $\ell(\ell+1)$
centrifugal term will appear once spin-weighted harmonics are introduced.
Gap status: \OPEN\ [L2].
