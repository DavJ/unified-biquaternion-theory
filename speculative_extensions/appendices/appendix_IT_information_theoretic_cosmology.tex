...
\subsection{Hierarchy of Multiplexing: Structural and Orthogonal Layers}

We propose that the UBT manifold utilizes at least two distinct layers of multiplexing to maintain the isolation of the 16 parallel 4D channels within the 8D biquaternionic space.

\begin{enumerate}
    \item \textbf{Layer 1: Structural Slot Multiplexing (TDM/CDM):} 
    The primary division of the $2^8$ state space into 16 logical segments of $2^4$ states each. This defines the addressable boundaries of each individual universe.
    
    \item \textbf{Layer 2: Orthogonal Prime Multiplexing (Spread Spectrum):} 
    To eliminate Inter-Channel Interference (ICI) and crosstalk, each channel is modulated using a unique spreading code based on prime number sequences. In this framework, fundamental dimensionless constants are interpreted as prime-based orthogonal identifiers. 
\end{enumerate}

\subsubsection{The Fine Structure Constant as a Channel ID}

The fine structure constant ($\alpha^{-1} \approx 137$) is identified as a prime-number basis for the ``Channel 1'' (our universe) spreading sequence. The primality of 137 ensures maximum orthogonality against other potential channels in the $GF(2^8)$ manifold. 

\begin{equation}
\text{Orthogonality} \implies \int \Psi_p \cdot \Psi_q = 0 \quad \text{for } p \neq q \in \mathbb{P}
\end{equation}

Where $p, q$ are the prime bases of different multiplexed layers. This multi-layered approach explains the ``tuning'' of the universe as a standard engineering requirement for high-SNR (Signal-to-Noise Ratio) digital communication over a 3-qubit quantum bus.
