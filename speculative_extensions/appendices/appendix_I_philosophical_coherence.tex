...
\paragraph{Metric convention.} Throughout this appendix, the fundamental geometric object is the biquaternionic metric $\mathcal{G}_{\mu\nu}$. Any appearance of the physical metric $g_{\mu\nu}$ should be understood as the real projection $g_{\mu\nu} := \text{Re}(\mathcal{G}_{\mu\nu})$, which couples to ordinary matter and reproduces Einstein's field equations in the appropriate limit.
...
\paragraph{Step 2: Metric reduction.} The biquaternionic metric $\mathcal{G}_{\mu\nu}(q,\tau)$ reduces to:
\begin{equation}
\mathcal{G}_{\mu\nu}(x,t) \big|_{\psi=0} = g_{\mu\nu}(x,t) := \text{Re}(\mathcal{G}_{\mu\nu}(x,t)) \quad \text{(real Lorentzian metric)}
\end{equation}
