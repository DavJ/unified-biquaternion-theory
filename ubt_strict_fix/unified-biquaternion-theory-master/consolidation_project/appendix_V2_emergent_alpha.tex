...
\textbf{WARNING:️ CRITICAL DISCLAIMER: FINE-STRUCTURE CONSTANT WARNING:️}

This appendix discusses the fine-structure constant $\alpha$ within UBT. \textbf{Critical limitations:}
\begin{itemize}
\item This is \textbf{NOT an ab initio derivation} from first principles
\item Discrete parameters (N=10, modulus choices) are not uniquely determined by theory
\item Represents postdiction (explaining known data), not true prediction
\item Circular dependencies remain (electron mass used to fix parameters)
\item This is one of UBT's \textbf{most significant open challenges}
\end{itemize}

See \texttt{UBT\_SCIENTIFIC\_STATUS\_AND\_DEVELOPMENT.md} for detailed discussion.
\end{minipage}}
\vspace{1em}
