...
\textbf{Where:}
\begin{itemize}
\item $\varepsilon$ is the (dimensionless) warp amplitude of $A(\psi)$ via $Z^{\star} = \!\int e^{-2A} d\psi = L\,\Izero(2\varepsilon)$, hence $\Lambda Z^{\star} = (2\pi/L)\cdot Z^{\star} = 2\pi\,\Izero(2\varepsilon)$.
...
We used only: (i) the circle identity $\Lambda=2\pi/L$, $Z^{\star}=L\Izero(2\varepsilon)$, (ii) the heat-kernel $a_2$ coefficient in $D=5$, and (iii) the invariant trace $\Tr(Q^2)$ instead of any sum over fields. No ``choice of content'' appears.
