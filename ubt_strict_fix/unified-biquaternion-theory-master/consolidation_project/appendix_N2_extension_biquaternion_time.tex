...
In the biquaternion algebra $\mathbb{C} \otimes \mathbb{H}$, this represents the temporal component of the full biquaternionic field $\Theta(q, T_B)$.
...
The complex time approximation $\tau = t + i\psi$ used extensively in UBT has a direct correspondence with A. H. Chamseddine's Hermitian gravity formulation (1982)~\cite{Chamseddine2005,Chamseddine2006}. This subsection establishes the formal mathematical mapping and clarifies when the Hermitian limit is admissible.
...
\paragraph{Justification for Complex Time Usage in UBT.}
Given the Chamseddine correspondence, the complex time approximation $\tau = t + i\psi$ is \textbf{justified} in UBT derivations when:
\begin{enumerate}
\item The physical system is in the weakly-coupled regime: $g_{\text{eff}}^2 \ll 1$
\item Field commutators are negligible: $\|\mathcal{C}\| \ll \|\Theta\|^2$ (cf.~Eq.~\eqref{eq:commutator_norm})
\item The theory exhibits an emergent $U(1,3)$ symmetry consistent with Hermitian gravity
\item The vector imaginary component is small: $\|\mathbf{v}\|^2 \ll |\psi|^2$ (cf.~Eq.~\eqref{eq:complex_criterion})
\end{enumerate}

When all four conditions are satisfied, the complex time formalism provides an accurate and computationally tractable approximation, with corrections controlled by the parameters $[\Theta_i,\Theta_j]/\|\Theta\|^2$ and $\|\mathbf{v}\|^2/|\psi|^2$.
